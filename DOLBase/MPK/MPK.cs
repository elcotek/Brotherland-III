	...
	public class MPK
	{
		...
		public void Write(string fname)
		{
			...
			using (var dirmem = new MemoryStream(_files.Count * MPKFileHeader.MaxSize))
			{
				...
				foreach (MPKFile file in _files.Values)
				{
					...
					using (var wrtr = new BinaryWriter(dirmem, Encoding.UTF8, leaveOpen: true))
					{
			...
			{
				using (var wrtr = new BinaryWriter(filemem, Encoding.UTF8, leaveOpen: true))
				{
			...
			{
				using (var wrtr = new BinaryWriter(fileStream, Encoding.UTF8, leaveOpen: true))
				{
					wrtr.Write(Magic);
					wrtr.Write((byte)2);
					wrtr.Write(buf, 0, 16);
		...
		public void Read(string fname)
		{
			...
			try
			{
				...
				rdr = new BinaryReader(File.OpenRead(fname), Encoding.UTF8);
			...
			using (var directory = new MemoryStream(buf))
			{
				...
				using (var files = new MemoryStream(buf))
				{
					...
					while (directory.Position < directory.Length && files.Position < files.Length)
					{
						...
						BinaryReader hdrRdr = null;

						try
						{
							using (var hdrStream = new MemoryStream(buf))
							{
								hdrRdr = new BinaryReader(hdrStream, Encoding.UTF8);
