...
namespace DOL.FTP
{
    ...
    public class FTPConnection
	{
		...
		private TcpClient CreateDataClient()
		{
			...
			var ep = new
               // public IPEndPoint(long address, int port);
                ...
                IPEndPoint(Convert.ToUInt32(GetLocalAddressList()), port);
		...
		private void SetDataPort(int portNumber)
		{
			...
			lock (_tcpClient)
			{
				tempMessageList = SendCommand("PORT "
				                              + GetLocalAddressList().ToString().Replace(".", ",")
				                              + "," + iPortHigh + "," + iPortLow);
        ...
        public static IEnumerable<string> GetLocalAddressList()
        {
            ...
            return (from ip in Dns.GetHostEntry(Dns.GetHostName()).AddressList select ip.ToString()).ToList();
            //return (from ip in Dns.GetHostEntry(Dns.GetHostName()).AddressList where ip.AddressFamily == AddressFamily.InterNetwork select ip.ToString()).ToList();
        }
