...
namespace DOL.Database
{
    ...
    public sealed class DataTableHandler
    {
        ...
        public DataTableHandler(Type ObjectType)
        {
            ...
            var elementBindings = new List<ElementBinding>();

            foreach (var member in members)
            {
                var binding = new ElementBinding(member);
                if (binding.IsDataElementBinding)
                {
            ...
            foreach (var bind in ElementBindings)
            {
                ...
                if (bind.DataElement != null)
                {
                    ...
                    {
                        Table.Constraints.Add(new UniqueConstraint($"U_{TableName}_{bind.ColumnName}", column));
                    }

                    // Store Indexes for definition after table
                    if (!string.IsNullOrEmpty(bind.DataElement.IndexColumns))
                    {
                        var indexCols = bind.DataElement.IndexColumns.Split(',')
                        ...
                        indexes.Add($"I_{TableName}_{bind.ColumnName}", indexCols);
                    }
                    else if (bind.DataElement.Index)
                    {
                        indexes.Add($"I_{TableName}_{bind.ColumnName}", new[] { bind });
                    }
            ...
            Table.ExtendedProperties.Add("INDEXES", indexes.ToDictionary(kv => kv.Key,
                kv => kv.Value.Select(bind => Table.Columns[bind.ColumnName]).ToArray()));
            ...
            foreach (var bind in multipleUnique)
            {
                ...
                Table.Constraints.Add(new UniqueConstraint($"U_{TableName}_{bind.ColumnName}",
        ...
        public DataObject GetPreCachedObject(object key)
        {
            ...
            return _precache.TryGetValue(key, out DataObject obj) ? obj : null;
        }
        ...
        public bool DeletePreCachedObject(object key)
        {
            ...
            return _precache.TryRemove(key, out DataObject dummy);
        }
