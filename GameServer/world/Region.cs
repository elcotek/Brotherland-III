...
namespace DOL.GS
{
    ...
    public class Region
    {
        ...
        /// </summary>
        private readonly Dictionary<ushort, IArea> m_Areas;
        ...
        public bool OnZonePoint(GamePlayer player, ZonePoint zonePoint)
        {
            return true;
        }
        ...
        public bool IsToA
        {
            get
            {
                switch (m_regionData.Id)
                {
                    //case 27://Isle
                    case 30://Celestius
                    case 130://new frontiers
                    case 73://cathal valley
                        return true;
                    default:
                        return false;
                }
        ...
        /// </summary>
        public bool IsRegionDivingEnabled
        {
        ...
        /// </summary>
        /// <returns></returns>
        public AbstractGameKeep CreateGameKeep()
        {
            return new GameKeep();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public AbstractGameKeep CreateGameKeepTower()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public GameKeepComponent CreateGameKeepComponent()
        {
        ...
        /// <summary>
        /// Determine if current time is between 12AM and 6AM, can be used for conditional spells.
        /// </summary>
        public bool IsNightTime
        {
            get
            {
                ...
                if (IsAM && hour >= 11) //Special Handling for Midnight.
                    m_isNightTime = true;
        ...
        public ConcurrentDictionary<GameNPC, int> MobsRespawning
        {
            get
            {
        ...
        protected IEnumerable GetInRadius(Zone.eGameObjectType type, int x, int y, int z, ushort radius, bool withDistance)
        {
            ...
            {
                List<GameObject> res = startingZone.GetObjectsInRadius(type, x, y, z, radius, new List<GameObject>());
                uint sqRadius = (uint)radius * radius;

                // Verwendung von ConcurrentBag für thread-sichere Sammlung
                ConcurrentBag<GameObject> concurrentRes = new ConcurrentBag<GameObject>(res);

                foreach (var currentZone in m_zones)
                {
                    if (currentZone != startingZone && currentZone.TotalNumberOfObjects > 0 && CheckShortestDistance(currentZone, x, y, sqRadius))
                    {
                        var objects = currentZone.GetObjectsInRadius(type, x, y, z, radius, res);
                        foreach (var obj in objects)
                        {
                            concurrentRes.Add(obj); // Füge zur regulären Sammlung hinzu
                        }
                    }
                }

                IEnumerable tmp;
                // Logik für withDistance bleibt unverändert
                if (withDistance)
                {
                    switch (type)
                    {
                        case Zone.eGameObjectType.ITEM:
                            tmp = new ItemDistanceEnumerator(x, y, z, concurrentRes.ToList());
                            break;
                        case Zone.eGameObjectType.NPC:
                            tmp = new NPCDistanceEnumerator(x, y, z, concurrentRes.ToList());
                            break;
                        case Zone.eGameObjectType.PLAYER:
                            tmp = new PlayerDistanceEnumerator(x, y, z, concurrentRes.ToList());
                            break;
                        case Zone.eGameObjectType.DOOR:
                            tmp = new DoorDistanceEnumerator(x, y, z, concurrentRes.ToList());
                            break;
                        default:
                ...
                {
                    tmp = new ObjectEnumerator(concurrentRes.ToList());
                }
                return tmp;
            }
            else
