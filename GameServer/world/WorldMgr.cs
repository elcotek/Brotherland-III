...
namespace DOL.GS
{
    ...
    public static class WorldMgr
    {
        ...
        /// </summary>
        public const int MAX_EXPFORKILL_DISTANCE = 4096;
        /// <summary>
        ...
        /// </summary>
        public const int VISIBILITY_DISTANCE = 6700;//3600WORLD_VISIBILITY_DISTANCE;
        /// <summary>
        ...
        private static readonly object m_syncTeleport = new object();
        ...
        public static void KeepHealTimercheck()
        {
            ...
            foreach (AbstractGameKeep keep in keepList)
            {
                if (keep is GameKeep || keep is GameKeepTower)
                {
                    foreach (GameKeepComponent keepComponent in keep.SentKeepComponents)
                    {
                        ...
                        {
                            if (keepComponent.HealthPercent < 100)// && keepComponent.InCombat == false)
                            {
                                
                                keepComponent.Repair((int)(keepComponent.MaxHealth * 0.01));//1% all 2 hours
                                //log.Error("Worldtimer heal Keep components!");
                            }
                        }
                    }
                }
        ...
        public static void WorldTimercheck()
        {

            //log.Error("CheckKeeps!!");
            ...
            CheckPlayersInBattlegrounds();




        }
        public static void CheckPlayersInBattlegrounds()
        {
            foreach (GameClient client in GetAllPlayingClients())
            {
                ...
                LoginRealmSwitchtimer(client); //RvR LoginTimer

                if (client.Player.CurrentRegion.IsBG)
                {
                    GamePlayer player = client.Player as GamePlayer;

                    //check for battleground caps
                    Battleground bg = KeepMgr.GetBattleground(player.CurrentRegionID);
                    if (bg != null)
                    {
                       
                        if (player.InCombat == false && (player.Level > bg.MaxLevel || player.RealmLevel >= bg.MaxRealmLevel))
                        {
        ...
        #region Realm switch Timer

        static void LoginRealmSwitchtimer(GameClient Client)
        {
                     

            if (Client != null)
            {
                //log.ErrorFormat("Login Timer Start! Player: {0}", Client.Player.Name);

                IList<Account> PlayerAccountName = GameServer.Database.SelectObjects<Account>("`AccountID` = @AccountID", new QueryParameter("@AccountID", Client.Account.AccountID));
                ...
                if (PlayerAccountName != null)
                {
                    foreach (Account db in PlayerAccountName)
                    {
                        ...
                        if (Client.Player.CurrentRegion.IsRvR && Client.Account.PrivLevel == 1 && db.LoginWaitTime <= DateTime.Now && (string.IsNullOrEmpty(Client.Account.AccountID) == false) && db.IsOtherRealmAllowed == false)
                        {
                            
                            db.LoginWaitTime = DateTime.Now;
                            //now set the last login Realm
                            //while(db.LoginWaitTime.Minute != time && db.AccountID == client.Account.AccountID) ///test raus
                            if (db.AccountID == Client.Account.AccountID)
                            {
                                
                                for (int i = 0; i < (byte)Client.Player.Realm; i++)
                                {
                                    // log.Error("login zeit eingetragen");
                                    // log.Error("login realm eingetragen");
                                    db.Realm = (byte)Client.Player.Realm;
                                    Client.Account.Realm = (byte)Client.Player.Realm;
                                    //log.ErrorFormat("Account Realm: {0}", client.Account.Realm);
                                    db.LastLoginRealm = (byte)Client.Player.Realm;
                                    Client.Account.LastLoginRealm = (byte)Client.Player.Realm;

                                    db.LastLogin = DateTime.Now;

                                    if (Properties.LOGIN_OTHER_RELAM_SWITCH_TIMER_MINUTES > 0)
                                    {
                                        db.LoginWaitTime = DateTime.Now.AddMinutes(Properties.LOGIN_OTHER_RELAM_SWITCH_TIMER_MINUTES);
                                    }
                                    if (Properties.LOGIN_OTHER_RELAM_SWITCH_TIMER_HOURS > 0)
                                    {
                                        db.LoginWaitTime = DateTime.Now.AddHours(Properties.LOGIN_OTHER_RELAM_SWITCH_TIMER_HOURS);
                                    }

                                    //Mindestens 10 Minuten timer
                                    if (Properties.LOGIN_OTHER_RELAM_SWITCH_TIMER_HOURS <= 0 && Properties.LOGIN_OTHER_RELAM_SWITCH_TIMER_HOURS <= 0)
                                    {
                                        db.LoginWaitTime = DateTime.Now.AddMinutes(10);
                                    }

                                    Client.Account.LoginWaitTime = db.LoginWaitTime;
                                }

                                GameServer.Database.SaveObject(db);
                                Client.Player.SaveIntoDatabase();

                                // DateTime LoginWaitTime = db.LoginWaitTime;

                                Client.Player.Out.SendMessage(LanguageMgr.GetTranslation(Client.Account.Language, "GamePlayer.RealmTimerStatus", db.LoginWaitTime), eChatType.CT_Important, eChatLoc.CL_SystemWindow);

                            }
                        }
                        else
                        {
                            //log.ErrorFormat("Account LoginWaitTime: {0}", db.LoginWaitTime);
                            if (db.Realm == (byte)Client.Player.Realm && db.LoginWaitTime <= DateTime.Now)
                            {

                                for (int i = 0; i < (byte)db.Realm; i++)
                                {
                                    //Remove the timer from Player
                                    db.Realm = (byte)0;
                                    Client.Account.Realm = (byte)0;
                                    //log.ErrorFormat("Account Realm now: {0}", client.Account.Realm);
                                ...
                                Client.Player.Out.SendMessage(LanguageMgr.GetTranslation(Client.Account.Language, "GamePlayer.RealmTimerStatusOFF"), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            }

                        }
                    }
                   // PlayerAccountName.Clear();

                }

            }
        }
        #endregion Login Timer
        ...
        public static bool EarlyInit(out RegionData[] regionsData)
        {
            ...
            {
                List<string> list = ServerProperties.Properties.DEBUG_LOAD_REGIONS.SplitCSV(true);
                for (int i = 0; i < list.Count; i++)
                {
                    string loadRegion = list[i];
            ...
            {
                Mob mob = mobList[i];
                if (!mobsByRegionId.TryGetValue(mob.Region, out List<Mob> list))
                {
            ...
            {
                foreach (KeyValuePair<string, ConfigElement> entry in regionCfg.Children)
                {
                    ...
                    if (dbRegion == null)
                    {
                        ...
                        {
                            RegionID = (ushort)config[ENTRY_REG_ID].GetInt(),
                            Name = entry.Key,
                            Description = config[ENTRY_REG_DESC].GetString(),
                            IP = config[ENTRY_REG_IP].GetString(),
                            Port = (ushort)config[ENTRY_REG_PORT].GetInt(),
                            Expansion = config[ENTRY_REG_EXPANSION].GetInt(),
                            HousingEnabled = config[ENTRY_REG_HOUSING_ENABLE].GetBoolean(false),
                            DivingEnabled = config[ENTRY_REG_DIVING_ENABLE].GetBoolean(false),
                            WaterLevel = config[ENTRY_REG_WATER_LEVEL].GetInt()
                        };
            ...
            foreach (var data in from DBRegions dbRegion in GameServer.Database.SelectAllObjects<DBRegions>()
                                 let data = new RegionData
                                 {
                                     Id = dbRegion.RegionID,
                                     Name = dbRegion.Name,
                                     Description = dbRegion.Description,
                                     Ip = dbRegion.IP,
                                     Port = dbRegion.Port,
                                     Expansion = dbRegion.Expansion,
                                     HousingEnabled = dbRegion.HousingEnabled,
                                     DivingEnabled = dbRegion.DivingEnabled,
                                     WaterLevel = dbRegion.WaterLevel,
                                     ClassType = dbRegion.ClassType,
                                     IsFrontier = dbRegion.IsFrontier
                                 }
                                 select data)
            {
                hasFrontierRegion |= data.IsFrontier;
                if (!mobsByRegionId.TryGetValue(data.Id, out List<Mob> mobs))
                    data.Mobs = new Mob[0];
                else
            ...
            {
                foreach (KeyValuePair<string, ConfigElement> entry in zoneCfg.Children)
                {
                    ...
                    if (checkZone == null)
                    {
                        ...
                        {
                            Height = config[ENTRY_ZONE_HEIGHT].GetInt(0),
                            Width = config[ENTRY_ZONE_WIDTH].GetInt(0),
                            OffsetY = config[ENTRY_ZONE_OFFY].GetInt(0),
                            OffsetX = config[ENTRY_ZONE_OFFX].GetInt(0),
                            Name = config[ENTRY_ZONE_DESC].GetString(""),
                            RegionID = (ushort)config[ENTRY_ZONE_REGIONID].GetInt(0),
                            ZoneID = config[ENTRY_ZONE_ZONEID].GetInt(0),
                            WaterLevel = config[ENTRY_ZONE_WATER_LEVEL].GetInt(0),
                            DivingFlag = 0
                        };
            ...
            IList<Zones> list1 = GameServer.Database.SelectAllObjects<Zones>();
            for (int i = 0; i < list1.Count; i++)
            {
                Zones dbZone = list1[i];
                ZoneData zoneData = new ZoneData
                {
                    Height = (byte)dbZone.Height,
                    Width = (byte)dbZone.Width,
                    OffY = (byte)dbZone.OffsetY,
                    OffX = (byte)dbZone.OffsetX,
                    Description = dbZone.Name,
                    RegionID = dbZone.RegionID,
                    ZoneID = (ushort)dbZone.ZoneID,
                    WaterLevel = dbZone.WaterLevel,
                    DivingFlag = dbZone.DivingFlag,
                    IsLava = dbZone.IsLava
                };
        ...
        private static void PingCheck(object sender)
        {
            try
            {
                IList<GameClient> list = GetAllClients();
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (log.IsWarnEnabled)
                                    log.Warn("Hard timeout for client " + client.Account.Name + " (" + client.ClientState.ToString() + ")");
                                GameServer.Instance.Disconnect(client);
                            }
                        }
                    }
        ...
        private static void RelocateRegions()
        {
            ...
            while (m_relocationThread != null && m_relocationThread.IsAlive)
            {
                try
                {
                    ...
                    foreach (var region in from Region region in regionsClone
                                           where region.NumPlayers > 0 && (region.LastRelocationTime + Zone.MAX_REFRESH_INTERVAL) * 10 * 1000 < DateTime.Now.Ticks
                                           select region)
                    {
                        region.Relocate();
                    }
        ...
        private static void DayReset(object sender)
        {
            ...
            foreach (var client in from GameClient client in GetAllPlayingClients()
                                   where client.Player != null && client.Player.CurrentRegion != null && client.Player.CurrentRegion.UseTimeManager
                                   select client)
            {
                client.Out.SendTime();
            }
        ...
        /// <returns></returns>
        public static long GetCurrentGameTime(GamePlayer player)
        {
        ...
        public static void RegisterZone(ZoneData zoneData, ushort zoneID, ushort regionID, string zoneName, int xpBonus, int rpBonus, int bpBonus, int coinBonus, byte realm)
        {
            ...
            if (region == null)
            {
                if (log.IsWarnEnabled)
                {
                    log.ErrorFormat("Could not find Region {0} for Zone {1}", regionID, zoneData.Description);
                }
                return;
            }
        ...
        /// <summary>
        /// Get the Zone Description of a Region id
        /// </summary>
        /// <param name="redionID"></param>
        /// <returns>the Region Descripton or not avaible</returns>
        public static string GetZoneDescription(ushort regionID)
        {

            if (m_zones.TryGetValue(regionID, out Zone zone))
                return zone.Description.ToString();
        ...
        {
            if (m_regions.TryGetValue(regionID, out Region reg))
                return reg;
		...
		public static Zone GetZone(ushort zoneID)
        {
            ...
            {
                log.ErrorFormat("Trying to access inexistent ZoneID {0}", zoneID);
        ...
        public static GameObject[] GetobjectsFromRegion(ushort regionID, Type objectType)
        {
            ...
            GameObject[] gameObject = (GameObject[])reg.Objects.Where(obj => obj != null).OfTypeAndToArray(objectType);
            return gameObject;
        }
        ...
        public static GameObject[] GetObjectsByNameFromRegion(string name, ushort regionID, eRealm realm, Type objectType)
        {
            ...
            GameObject[] gameObject = (GameObject[])reg.Objects.Where(obj => obj != null && obj.Realm == realm && obj.Name == name).OfTypeAndToArray(objectType);
            return gameObject;
        }
        ...
        {
            GameObject[] gameObject = (GameObject[])m_regions.Values.Select(reg => GetObjectsByNameFromRegion(name, reg.ID, realm, objectType))
        ...
        {
            GameNPC[] gameNPC = (GameNPC[])GetObjectsByNameFromRegion(name, regionID, realm, typeof(GameNPC));
            return gameNPC;
        }
        ...
        {
            if (!m_regions.TryGetValue(region, out Region reg))
        ...
        public static int GetDistance(IPoint3D pointObj1, IPoint3D pointObj2)
        {
            Point3D targetPoint1;
            Point3D targetPoint2;

            if (pointObj1 == null || pointObj2 == null || (pointObj1 as GameObject).CurrentRegion != (pointObj2 as GameObject).CurrentRegion)
                return -1;

            targetPoint1 = new Point3D(pointObj1.X, pointObj1.Y, pointObj1.Z);
            targetPoint2 = new Point3D(pointObj2.X, pointObj2.Y, pointObj2.Z);


            return GetDistance(targetPoint1, targetPoint2);
        }
        ...
        /// <returns>The distance in units or -1 if they are not the same Region</returns>
        public static int GetDistance(IPoint3D pointObj1, IPoint3D pointObj2, double zfactor)
        {
            Point3D targetPoint1;
            Point3D targetPoint2;
            ...
            return GetDistance(targetPoint1, targetPoint2, zfactor);
        }
        /// <summary>
        ...
        public static int GetDistance(int x1, int y1, int z1, int x2, int y2, int z2)
        {
            ...
            targetPoint1 = new Point3D(x1, y1, z1);
            targetPoint2 = new Point3D(x2, y2, z2);

            long xdiff = (long)targetPoint1.X - targetPoint2.X;
            long ydiff = (long)targetPoint1.Y - targetPoint2.Y;
            long zdiff = (long)targetPoint1.Z - targetPoint2.Z;
        ...
        public static int GetDistance(int x1, int y1, int z1, int x2, int y2, int z2, double zfactor)
        {
            ...
            long xdiff = (long)targetPoint1.X - targetPoint2.X;
            long ydiff = (long)targetPoint1.Y - targetPoint2.Y;
            long zdiff = (long)((targetPoint1.Z - targetPoint2.Z) * zfactor);
        ...
        /// <returns>The distance</returns>
        public static int GetDistance(IPoint3D obj, int x, int y, int z)
        {
            Point3D targetPoint1;
            Point3D targetPoint2;

            targetPoint1 = new Point3D(obj.X, obj.Y, obj.Z);
            targetPoint2 = new Point3D(x, y, z);
        ...
        public static bool CheckDistance(int x1, int y1, int z1, int x2, int y2, int z2, int radius)
        {
            ...
            return CheckSquareDistance(targetPoint1, targetPoint2, radius * radius);
        }
        ...
        public static bool CheckDistance(IPoint3D obj, IPoint3D obj2, int radius)
        {
            ...
            targetPoint1 = new Point3D(obj.X, obj.Y, obj.Z);
            targetPoint2 = new Point3D(obj2.X, obj2.Y, obj2.Z);

            return CheckDistance(targetPoint1, targetPoint2, radius);
        }
        ...
        private static bool CheckSquareDistance(IPoint3D obj, IPoint3D obj2, int squareRadius)
        {
            ...
            targetPoint1 = new Point3D(obj.Z, obj.Y, obj.Z);
            targetPoint2 = new Point3D(obj2.Z, obj2.Y, obj2.Z);

            return CheckSquareDistance(targetPoint1, targetPoint2, squareRadius);
        }
        ...
        public static GameClient GetClientByPlayerName(string playerName, bool exactMatch, bool activeRequired)
        {
            ...
            {
                return GuessClientByPlayerNameAndRealm(playerName, 0, activeRequired, out int x);
            }
        }
        ...
        {
            return GetNPCsCloseToSpot(regionid, (int)pos.X, (int)pos.Y, (int)pos.Z, radiusToCheck, withDistance);
        }
        public static IEnumerable GetNPCsCloseToSpot(ushort regionid, Vector3 pos, ushort radiusToCheck)
        { 
           return GetNPCsCloseToSpot(regionid,(int)pos.X, (int)pos.Y, (int)pos.Z, radiusToCheck, false);
        }
        ...
        public static BaseInstance CreateInstance(ushort requestedID, ushort skinID, Type instanceType)
        {
            ...
            if (data == null)
            {
                log.Error("Data for region " + skinID.ToString() + " not found on instance create!");
                return null;
            }
            ...
            //GameTimer.TimeManager time = m_regionTimeManagers[FastMath.Abs(requestedID % (GameServer.Instance.Configuration.CPUUse * 2) - GameServer.Instance.Configuration.CPUUse) % GameServer.Instance.Configuration.CPUUse];
            ...
            if (list == null)
            {
                log.Warn("No zones found for given skinID on instance creation, " + skinID.ToString());
                return null;
            }
        ...
        {
            //Remove the region
            m_regions.TryRemove(instance.ID, out Region reg);
