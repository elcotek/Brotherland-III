...
namespace DOL.GS
{
    public class MinotaurRelic : GameStaticItem
	{
		...
		public override bool Interact(GamePlayer player)
		{
            ...
            if (player.Level < 45)
            {
                player.Out.SendMessage("You cannot pickup " + GetName(0, false) + "You must be level 45 and above to take the relic!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return false;
            }
            if (this.Owner != null)
			{
		...
		protected virtual void SetHandlers(GamePlayer player, bool start)
		{
			if (start)
			{
                ...
                GameEventMgr.AddHandler(player, GamePlayerEvent.EnemyKilled, new DOLEventHandler(EnemyKilledGainBPRP));
			...
			else
			{
                ...
                GameEventMgr.RemoveHandler(player, GamePlayerEvent.EnemyKilled, new DOLEventHandler(EnemyKilledGainBPRP));
        ...
        protected void EnemyKilledGainBPRP(DOLEvent e, object sender, EventArgs args)
        {
            long realmPoints;

            if (sender is GamePlayer && args is EnemyKilledEventArgs)
            {
                GamePlayer player = sender as GamePlayer;
                EnemyKilledEventArgs arg = args as EnemyKilledEventArgs;

                if (player.MinotaurRelic == null || arg == null) return;

                if (arg.Target is GamePlayer && arg.Target.Realm != player.Realm)
                {

                    realmPoints = (25 + (int)player.Level * 2);
                    if (realmPoints > 0)
                    player.RealmPoints += realmPoints;
                    player.BountyPointsAndMassage(50);
                    player.UpdatePlayerStatus();
                    player.Out.SendMessage("You gain extra realm points " + realmPoints + " and 50 extra bps from your " + GetName(0, false) + " relic!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                }
            }
        }
