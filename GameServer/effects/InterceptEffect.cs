    ...
    public class InterceptEffect : StaticEffect, IGameEffect
	{
		...
		public void Start(GameLiving interceptor, GameLiving intercepted)
		{
			...
			if (!interceptor.IsWithinRadius(intercepted, InterceptAbilityHandler.INTERCEPT_DISTANCE))
			{
				if (interceptor is GamePlayer)
					((GamePlayer)interceptor).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)interceptor).Client.Account.Language, "Effects.InterceptEffect.YouAttemtInterceptYBut", intercepted.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
				if (intercepted is GamePlayer && interceptor is GamePlayer)
					((GamePlayer)intercepted).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)intercepted).Client.Account.Language, "Effects.InterceptEffect.XAttemtInterceptYouBut", interceptor.GetName(0, true)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
			}
			else
			{
				if (interceptor is GamePlayer)
					((GamePlayer)interceptor).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)interceptor).Client.Account.Language, "Effects.InterceptEffect.YouAttemtInterceptY", intercepted.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
				if (intercepted is GamePlayer && interceptor is GamePlayer)
					((GamePlayer)intercepted).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)intercepted).Client.Account.Language, "Effects.InterceptEffect.XAttemptInterceptYou", interceptor.GetName(0, true)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
			}
		...
		public override void Cancel(bool playerCancel)
		{
			...
			if (playerCancel)
			{
				if (InterceptSource is GamePlayer)
					((GamePlayer)InterceptSource).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)InterceptSource).Client.Account.Language, "Effects.InterceptEffect.YouNoAttemtInterceptY", InterceptTarget.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
				if (InterceptTarget is GamePlayer && InterceptSource is GamePlayer)
					((GamePlayer)InterceptTarget).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)InterceptTarget).Client.Account.Language, "Effects.InterceptEffect.XNoAttemptInterceptYou", InterceptSource.GetName(0, true)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
			}
		}
			...
			get
			{
				if (Owner is GamePlayer)
				{
					if (m_interceptSource != null && m_interceptTarget != null)
						return LanguageMgr.GetTranslation(((GamePlayer)Owner).Client.Account.Language, "Effects.InterceptEffect.InterceptedByName", m_interceptTarget.GetName(0, false), m_interceptSource.GetName(0, false));
					return LanguageMgr.GetTranslation(((GamePlayer)Owner).Client.Account.Language, "Effects.InterceptEffect.Name");
				}
				return "";
			}
		}
        ...
        public override IList<string> DelveInfo(GamePlayer player)
        {
			
			{
                ...
                {
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Effects.InterceptEffect.InfoEffect"),
                    " ",
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Effects.InterceptEffect.XInterceptingY", InterceptSource.GetName(0, true), InterceptTarget.GetName(0, false))
                };
