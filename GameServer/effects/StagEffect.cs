        ...
        */
		...
		public override void Start(GameLiving living)
		{
			...
			if (living is GamePlayer)
			{
				...
				(living as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((living as GamePlayer).Client.Account.Language, "Effects.StagEffect.HuntsSpiritChannel"), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
			}
		}
		...
		public override void Stop()
		{
			...
			if (m_owner is GamePlayer)
			{
				...
				(m_owner as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((m_owner as GamePlayer).Client.Account.Language, "Effects.StagEffect.YourHuntsSpiritEnds"), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
			}
		}
		...
		/// Name of the effect
		/// </summary>
		public override string Name { get { return LanguageMgr.GetTranslation(((GamePlayer)Owner).Client.Account.Language, "Effects.StagEffect.Name"); } }
		...
		public override IList<string> DelveInfo(GamePlayer player)
        {
			
			{
                ...
                {
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Effects.StagEffect.InfoEffect")
                };
				...
				if (seconds > 0)
				{
					...
					if (seconds > 60)
						delveInfoList.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "Effects.DelveInfo.MinutesRemaining", (seconds / 60), (seconds % 60).ToString("00")));
					else
						delveInfoList.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "Effects.DelveInfo.SecondsRemaining", seconds));
				}
