...
namespace DOL.GS.Effects
{
    ...
    public class GameEffectPlayerList : GameEffectList
    {
        ...
        {
            if (changedEffect != null && changedEffect.Icon == 0 || changedEffect == null)
                return;
            ...
            try
            {
                lock (m_changedLock) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
                {
                    ...
                    {
                        //log.ErrorFormat("Add effect:{0}", changedEffect.Name);
            ...
            catch
            {
                ...
                {
                    log.ErrorFormat("Error: in m_changedEffects.Add(changedEffect); changedEffect name: {0}", changedEffect.Name);
                }
                else
                    log.ErrorFormat("Error: in m_changedEffects.Add(changedEffect); changedEffect was null");
            }
            finally
            {
                //log.ErrorFormat("Add effect ok:{0}", changedEffect.Name);
                base.OnEffectsChanged(changedEffect);
            }
        }
