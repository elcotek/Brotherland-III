        ...
        public virtual void Cancel(bool playerCanceled)
        {
            if (playerCanceled && HasNegativeEffect)
            {
                if (Owner is GamePlayer)
                    ((GamePlayer)Owner).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)Owner).Client.Account.Language, "Effects.StaticEffect.YouCantRemoveThisEffect"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            Stop();
        }
