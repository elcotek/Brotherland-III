...
namespace DOL.GS.Effects
{
    ...
    public class GameSpellEffect : IGameEffect, IConcentrationEffect
    {
        ...
        {
            get { return (SpellHandler != null && !SpellHandler.HasPositiveEffect) && IsExpired && m_timer != null && m_timer.IsAlive; }
        }
        ...
        public virtual void Cancel(bool playerCanceled)
        {
            lock (m_LockObject)
            {
                ...
                if (playerCanceled && ((SpellHandler != null && !SpellHandler.HasPositiveEffect) || ImmunityState && Spell.Target != "Self" && Spell.Target != "Group"))
                {
                    GamePlayer player = Owner as GamePlayer;
                    if (player != null)
                        player.Out.SendMessage(LanguageMgr.GetTranslation((Owner as GamePlayer).Client.Account.Language, "Effects.GameSpellEffect.CantRemoveEffect"), eChatType.CT_System, eChatLoc.CL_SystemWindow);

                    return;
                }
               
                // Can't Cancel Immunity Effect from Alive Living
                if (SpellHandler != null && !SpellHandler.HasPositiveEffect && Spell.SpellType != "Disease")
                {
        ...
        protected virtual void StartTimers()
        {
            ...
            if ((Duration > 0 || PulseFreq > 0) && Spell.SpellType != "SummonMobPet")
            {
            ...
            //Mob Pet respawn
            else if (Owner != null && Owner.IsAlive && Owner is GameLiving && Spell.SpellType == "SummonMobPet")
            {
                int numberofpets = Spell.ResurrectMana;

                for (int i = 0; i < numberofpets; i++)
                {
                    if (numberofpets != Owner.PetCount)
                        Owner.PetCount++;


                    else if (Owner.PetCount > Spell.ResurrectMana)
                    {
                        Owner.PetCount--;
                        //return;
                    }
        ...
        {
            if (m_timer != null && Spell.SpellType != "SummonMobPet")
            {
            ...
            else if (Owner.IsAlive && Owner != null && Owner is GameLiving && Spell.SpellType == "SummonMobPet")
            {
                int numberofpets = 0;
                numberofpets = Owner.PetCount;
                ...
                for (int i = 0; i < numberofpets; i++)
                {
                    if (Owner.PetCount > 0)
                    {
                        ...
                        if (Owner != null && Owner.IsAlive && Spell.Target == "Self")
                        {
                            m_timer.Stop();
                            Owner.Die(Owner);
                            m_timer = null;
                        }
                        else if (Owner.PetCount > 0)
                        {
                ...
                // Console.Out.WriteLine("StopTimers: Body have now a count of pets = {0} ", Owner.PetCount);
            }
        }
        ...
        public virtual IList<string> DelveInfo(GamePlayer player)
        {
            ...
            {
                IList<string> list = m_handler.DelveInfo(player);
                ...
                if (seconds > 0)
                {
                    list.Add(" "); //empty line
                    if (seconds > 60)
                        list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "Effects.DelveInfo.MinutesRemaining", (seconds / 60), (seconds % 60).ToString("00")));
                    else
                        list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "Effects.DelveInfo.SecondsRemaining", seconds));
                }
        ...
        public PlayerXEffect GetSavedEffect()
        {
            if (this.RestoredEffect && this.RemainingTime > 5000)
            {
                PlayerXEffect eff = new PlayerXEffect
                {
                    ...
                    IsHandler = true,
                    Var1 = this.RestoreVars[0],
                    Var2 = this.RestoreVars[1],
                    Var3 = this.RestoreVars[2],
                    Var4 = this.RestoreVars[3],
                    Var5 = this.RestoreVars[4],
                    Var6 = this.RestoreVars[5],
                    SpellLine = this.SpellHandler.SpellLine.KeyName
                };
                return eff;
            }
            if (m_handler != null)
        ...
        protected sealed class PulsingEffectTimer : GameTimer
        {
            ...
            public PulsingEffectTimer(GameSpellEffect effect)
                : base(effect.m_owner.CurrentRegion.TimeManager)
            {
                m_effect = effect ?? throw new ArgumentNullException("effect");
            }
