    ...
    public class TripleWieldEffect : TimedEffect
	{
		...
		protected void EventHandler(DOLEvent e, object sender, EventArgs arguments)
		{
            ...
            AttackData ad = new AttackData
            {
                ...
                WeaponSpeed = atkArgs.AttackData.WeaponSpeed
            };

            GamePlayer owner = attacker as GamePlayer;
			if (owner != null) {
				owner.Out.SendMessage(LanguageMgr.GetTranslation(owner.Client.Account.Language, "Effects.TripleWieldEffect.MBHitsExtraDamage", target.GetName(0, false), ad.Damage), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
				GamePlayer playerTarget = target as GamePlayer;
				if (playerTarget != null) {
                    playerTarget.Out.SendMessage(LanguageMgr.GetTranslation(owner.Client.Account.Language, "Effects.TripleWieldEffect.XMBExtraDamageToYou", attacker.GetName(0, false), ad.Damage), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
				}
			}
		...
		public override string Name { get { return LanguageMgr.GetTranslation(((GamePlayer)Owner).Client.Account.Language, "Effects.TripleWieldEffect.Name"); } }
		...
		public override IList<string> DelveInfo(GamePlayer player)
        {
			
			{
				...
				list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "Effects.TripleWieldEffect.InfoEffect"));
