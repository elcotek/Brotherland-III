...
namespace DOL.GS.Spells
{
    ...
    public abstract class AbstractBuffShear : SpellHandler
    {
        ...
        /// The chance Value
        /// </summary>
        protected int _lowerValueHitChance;
        ...
        /// <summary>
        /// Calcutate the hit Chance if the buff has highter Value than the Shear.
        /// </summary>
        /// <param name="buffval">Buff Value</param>
        /// <param name="shearval">Shear Value</param>
        /// <returns></returns>
        public virtual bool CalculateShearHitchance(int buffval, int shearval)
        {
            int CustomaddValue = ServerProperties.Properties.CUSTOM_BUFFSHEAR_VALUE;
            if (CustomaddValue > 50)
            {
                CustomaddValue = 50;
            }
            ...
            switch (shearval)
            {
                ...
                //lower shear vales are 30, 45, 65, 75
                //buff values are 45, 65, 75, 150
                case 30:
                    {
                        if (buffval == 45)
                            _lowerValueHitChance = 5+ CustomaddValue;
                        if (buffval == 65)
                            _lowerValueHitChance = 3+ CustomaddValue;
                        if (buffval == 75)
                            _lowerValueHitChance = 2+ CustomaddValue;
                        if (buffval == 150)
                            _lowerValueHitChance = 1+ CustomaddValue;
                        break;
                    }
                case 45:
                    {
            ...
            return Util.Chance(_lowerValueHitChance);
        }
        ...
        public override void OnDirectEffect(GameLiving target, double effectiveness)
        {
            ...
            foreach (GameSpellEffect effect in target.EffectList.GetAllOfType<GameSpellEffect>())
            {
                ...
                if (effect.Spell.SpellType == ShearSpellType)
                {
                    ...
                    else if ((effect.Owner != effect.SpellHandler.Caster || effect.Spell.IsShearable) && effect.Spell.Value > Spell.Value && CalculateShearHitchance((int)effect.Spell.Value, (int)Spell.Value))
                    {
                        SendEffectAnimation(target, 0, false, 1);
                        effect.Cancel(false);
                        MessageToCaster("Your spell rips away some of your target's enhancing magic.", eChatType.CT_Spell);
                    ...
                    else
                    {
                        ...
                        MessageToCaster("The target's connection to their enhancement was too strong for you to remove.", eChatType.CT_SpellResisted);
                    }
