	...
	public class RealmLore : SpellHandler
	{
		public override bool CheckBeginCast(GameLiving selectedTarget)
        {
			...
			if (selectedTarget is GameNPC)
			{
                ...
                {
                    //MessageToCaster("This spell works only on players.", eChatType.CT_SpellResisted);
                    (m_caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((m_caster as GamePlayer).Client.Account.Language, "SpellHandler.WorksOnlyOnPlayers"), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                }
                    return false;
			}
