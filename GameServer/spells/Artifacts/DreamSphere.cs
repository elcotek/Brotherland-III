        ...
        public override void OnEffectStart(GameSpellEffect effect)
        {
            ...
            if (player is GamePlayer && player.Group != null)
            {
                foreach (GamePlayer players in player.Group.GetPlayersInTheGroup())
                {
                    ...
                    foreach (GameSpellEffect Effect in players.EffectList.GetAllOfType<GameSpellEffect>())
                    {
                        if (Effect.SpellHandler.Spell.SpellType.Equals("ShadesOfMist") ||
                            Effect.SpellHandler.Spell.SpellType.Equals("TraitorsDaggerProc") ||
                            Effect.SpellHandler.Spell.SpellType.Equals("MaddeningScalars") ||
                            ...
                            Effect.SpellHandler.Spell.SpellType.Equals("AlvarusMorph"))
                        {
                            players.Out.SendMessage("You already have an active morph!", DOL.GS.PacketHandler.eChatType.CT_SpellResisted, DOL.GS.PacketHandler.eChatLoc.CL_ChatWindow);
                            return;
                        }
                    }
                    //if (players == null) return;
                    //GameEventMgr.AddHandler(player, GamePlayerEvent.TakeDamage, new DOLEventHandler(LivingTakeDamage));
                    GameEventMgr.AddHandler(players, GamePlayerEvent.AttackedByEnemy, new DOLEventHandler(LivingTakeDamage));
                }
            }
        }
        public override int OnEffectExpires(GameSpellEffect effect, bool noMessages)
        {
            GamePlayer players = effect.Owner as GamePlayer;
            if (players == null) return base.OnEffectExpires(effect, noMessages);
            //GameEventMgr.RemoveHandler(player, GamePlayerEvent.TakeDamage, new DOLEventHandler(LivingTakeDamage));
            GameEventMgr.RemoveHandler(players, GamePlayerEvent.AttackedByEnemy, new DOLEventHandler(LivingTakeDamage));
            return base.OnEffectExpires(effect, noMessages);
        }
