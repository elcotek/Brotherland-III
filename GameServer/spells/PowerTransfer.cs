        ...
        public override void OnDirectEffect(GameLiving target, double effectiveness)
        {
            ...
            else
            {
                ...
                owner.Out.SendMessage(String.Format("You transfer {0} power to {1}!",
                    powerHealed.ToString(), target.Name), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);

                if (target is GamePlayer)
                    (target as GamePlayer).Out.SendMessage(String.Format("{0} transfers {1} power to you!",
                        owner.Name, powerHealed.ToString()), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
            }
        }
