...
namespace DOL.GS.Spells
{
    ...
    /// </summary>
    public abstract class AOEAbstractCCSpellHandler : ImmunityEffectSpellHandler
    {
        ...
        public AOEAbstractCCSpellHandler(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell, line) { }
    }

    /// <summary>
    /// Mezz
    /// </summary>
    [SpellHandlerAttribute("AOEMesmerize")]
    public class AOEMesmerizeSpellHandler : AbstractCCSpellHandler
    {
        /// <summary>
        /// Defines a logger for this class.
        /// </summary>
        ...
        private void CheckSpellLOS(GamePlayer player, ushort response, ushort targetOID)
        {
            ...
            if ((response & 0x100) == 0x100)
            {
                try
                {
                    ...
                    if (target != null && target.ObjectState == GameLiving.eObjectState.Active)
                    {
                        ...
                        StartMezEffects(target, effectiveness);
            ...
            else
            {
                if (Caster is GamePlayer)
                {
                    (Caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((Caster as GamePlayer).Client.Account.Language, "ArrowSpellHandler.YourTargetIsNotInView"), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                }
            }
        }
        ...
        public virtual void StartMezEffects(GameLiving target, double effectiveness)
        {



            GameSpellEffect mezblock = SpellHandler.FindEffectOnTarget(target as GamePlayer, "CeremonialBracerMezz");

            //Ceremonial Bracer Stun Feedback
            if (mezblock != null && target != null)
            {

                {
                    ...
                    if (target is GamePlayer)
                    {
                        (target as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((target as GamePlayer).Client.Account.Language, "CCSpellHandler.YourItemEffectInterceptsTheMez"), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                    }
            ...
            {
                //cancel effect if target is not in range and not mezed
                if (target != null && Caster.IsWithinRadius(target, this.Spell.Range) == false && target.IsMezzed == false)
                {
                    ...
                    if (Caster is GamePlayer)
                    {
                        ...
                        (Caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((Caster as GamePlayer).Client.Account.Language, "CCSpellHandler.YouAreFarAwayFromTarget"), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                        (Caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((Caster as GamePlayer).Client.Account.Language, "CCSpellHandler.StopPlayingSong"), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                    }
                    return;
                }
        ...
        // constructor
        public AOEMesmerizeSpellHandler(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell, line) { }
    }
    /// <summary>
    /// Stun
    /// </summary>
    [SpellHandlerAttribute("AOEStun")]
    public class AOEStunSpellHandler : AbstractCCSpellHandler
    {
        /// <summary>
        /// Defines a logger for this class.
        /// </summary>
        ...
        protected override GameSpellEffect CreateSpellEffect(GameLiving target, double effectiveness)
        {
            ...
            GameSpellEffect aoemes = SpellHandler.FindEffectOnTarget(target, "AOEMesmerize");
            if (aoemes != null && target.IsMezzed)
            {
            ...
            if ((response & 0x100) == 0x100)
            {
                try
                {
                    ...
                    if (target != null && target.ObjectState == GameLiving.eObjectState.Active)
                    {
                        ...
                        StartStunEffects(target, effectiveness);
        ...
        public virtual void StartStunEffects(GameLiving target, double effectiveness)
        {
            ...
            if (Spell.SpellType.ToLower() != "stylestun")
            {
                ...
                //Ceremonial message and interupt
                GameSpellEffect stunblock = SpellHandler.FindEffectOnTarget(target, "CeremonialBracerStun");
                if (stunblock != null && target != Caster)
                {
        ...
        // constructor
        public AOEStunSpellHandler(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell, line) { }
    }
}
