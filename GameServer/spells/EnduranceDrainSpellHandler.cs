	...
	public class EnduranceDrainSpellHandler : SpellHandler
	{
		...
		public override void OnDirectEffect(GameLiving target, double effectiveness)
		{
			...
			if (target is GamePlayer)
			{
                ...
                (target as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((target as GamePlayer).Client.Account.Language, "SpellHandler.CasterSteelEnduranceFromTarget", end, m_caster.Name), eChatType.CT_YouWereHit, eChatLoc.CL_SystemWindow);
            }
		...
		{
            //MessageToCaster(string.Format("You steal {1} for {0} endurance!", target.Name, end), eChatType.CT_YouHit);
            if (m_caster is GamePlayer)
            {
                (m_caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((m_caster as GamePlayer).Client.Account.Language, "SpellHandler.YouSteelTargetforValueEnduranceFromTarget", end, target.Name), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
            }
            if (end > 0) 
			{
