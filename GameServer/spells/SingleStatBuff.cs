...
namespace DOL.GS.Spells
{
    ...
    public abstract class SingleStatBuff : PropertyChangingSpell
    {
        ...
        private static bool IsStatBuff(Spell spell)
        {
        ...
        public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
        {
            ...
            if (Caster is GamePlayer && IsStatBuff(Spell) && (Spell.Level > 0 && ((GamePlayer)Caster).IsCharcterClass(eCharacterClass.Savage) == false))
            {
                effectiveness = 0.75; // This section is for self bulfs, cleric buffs etc.
                if (Spell.Level > 0 || Spell.IsItemSpell)
                {
                    _spelllevel = Spell.Level;
                    _speclevel = specLevel;

                    if (Spell.IsItemSpell)
                    {
                       //log.Error("is form IsItemSpell!");
                           _speclevel = target.Level;
                           _spelllevel = target.Level;
                    }

                    //log.ErrorFormat("Buffbunus aktive, Spell Level: {0} Spec Level: {1}", _spelllevel, _speclevel);
                    effectiveness += (_speclevel - 1.0) * 0.5 / _spelllevel;
    ...
    /// Combat speed buff
    /// </summary>
    [SpellHandlerAttribute("CastSpeedBuff")]
    public class CastSpeedBuff : SingleStatBuff
    {
        ...
        // constructor
        public CastSpeedBuff(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell, line) { }
    }
    ...
    /// Block chance buff
    /// </summary>
    [SpellHandlerAttribute("BlockBuff")]
    public class BlockChanceBuff : SingleStatBuff
    {
        public override eProperty Property1 { get { return eProperty.BlockChance; } }

        // constructor
        public BlockChanceBuff(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell, line) { }
    }
