...
namespace DOL.GS.Spells
{
    ...
    public class Grapple : MasterlevelHandling
    {
        ...
        {
            if (selectedTarget is GamePlayer == false || selectedTarget is GameNPC || selectedTarget == null || Caster is GameNPC || selectedTarget == Caster)
            {
                MessageToCaster("This spell works only on enemy players.", eChatType.CT_SpellResisted);
                return false;
            }
                       
            if (selectedTarget.EffectList.GetOfType<SpeedOfSoundEffect>() != null || selectedTarget.EffectList.GetOfType<ChargeEffect>() != null)
            {
                (Caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((Caster as GamePlayer).Client.Account.Language, "CCSpellHandler.TargetIsMovingTooFast", selectedTarget.Name), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
            }
    ...
    public class ThrowWeaponSpellHandler : DirectDamageSpellHandler
    {
        ...
        public override void DamageTarget(AttackData ad, bool showEffectAnimation, bool wasBlocked)
        {
            InventoryItem weapon = null;
            weapon = ad.Weapon;
        ...
        public override void FinishSpellCast(GameLiving target)
        {
            ...
            else
            {
                ...
                Caster.DisableSkill(Spell, 10000);
            }
        }
        ...
        public override AttackData CalculateDamageToTarget(GameLiving target, double effectiveness)
        {
            ...
            if (ad.AttackResult == GameLiving.eAttackResult.HitUnstyled || ad.AttackResult == GameLiving.eAttackResult.HitStyle)
            {
                ...
                ad.Modifier = (int)(damage * (ad.Target.GetResist(ad.DamageType) + SkillBase.GetArmorResist(armor, ad.DamageType, ad.Target)) * -0.01);
            ...
            else
            {
                ...
                int attackSpeed = player.AttackSpeed(weapon); // player.AttackWeapon.SPD_ABS;//    player.AttackSpeed(weapon);
...
namespace DOL.GS.PropertyCalc
{
    ...
    public class KeepDamagePercentCalculator : PropertyCalculator
    {
        ...
        {
            int percent = 0;
            percent += living.BaseBuffBonusCategory[(int)property];
            if (percent > 100)
                percent = 100;
