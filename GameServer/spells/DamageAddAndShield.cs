...
namespace DOL.GS.Spells
{
	...
	public class DamageAddSpellHandler : AbstractDamageAddSpellHandler
	{
		...
		protected override void EventHandler(DOLEvent e, object sender, EventArgs arguments)
		{
			...
			//only Melee attack
			if (atkArgs.AttackData.IsMeleeAttack == false && atkArgs.AttackData.DamageType != eDamageType.Crush && atkArgs.AttackData.DamageType != eDamageType.Slash && atkArgs.AttackData.DamageType != eDamageType.Thrust)
				return;
			...
			//if (ad.DamageType == eDamageType.Crush || ad.DamageType == eDamageType.Slash || ad.DamageType == eDamageType.Thrust)
			//{
	...
	public class DamageShieldSpellHandler : AbstractDamageAddSpellHandler
	{
		...
		protected override void EventHandler(DOLEvent e, object sender, EventArgs arguments)
		{
			...
			double constante = ServerProperties.Properties.SHIELDBUFF_DAMAGE_ADD;


			double shielddamage = damageResisted + (constante * (target.Level / 50));
			if (shielddamage < 0)
				shielddamage = 0;
			...
			ad.Attacker = attacker;
			ad.Target = target;
			ad.Damage = (int)(damage + (damageResisted + shielddamage));
			ad.Modifier = (int)damageResisted;
			ad.DamageType = Spell.DamageType;
			ad.SpellHandler = this;
			ad.AttackType = AttackData.eAttackType.Spell;
