...
namespace DOL.GS.Spells
{
    ...
    /// 
    /// </summary>
    [SpellHandlerAttribute("AOEHeal")]
    public class AOEHealSpellHandler : SpellHandler
    {
        ...
        // constructor
        public AOEHealSpellHandler(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell, line) { }
                    ...
                    if (target != null && target.ObjectState == GameLiving.eObjectState.Active)
                    {
                        ...
                        foreach (GameLiving healTarget in targets)
                        {
                            if (healTarget != null && healTarget == target)
                            {
                                ...
                                StartHealEffects(healTarget, target, effectiveness);
        ...
        public virtual bool StartHealEffects(GameLiving healTargets, GameLiving Target, double effectiveness)
        {
...

            {
                ...
                if (healTargets.IsDiseased)
                {
                    if (m_caster is GamePlayer)
                    {
                        // MessageToCaster("Your target is diseased!", eChatType.CT_SpellResisted);
                        ((GamePlayer)m_caster).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)m_caster).Client.Account.Language, "HealSpellHandler.Execute.YourTargetIsDiseased", healTargets.Name), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                    }
                    heal >>= 1;
                }
                ...
                if (SpellLine.KeyName == GlobalSpellsLines.Item_Effects)
                {
                    healed |= ProcHeal(healTargets, heal);
                }
                else
                {
                    healed |= HealTarget(healTargets, heal);
                }
            }

            // group heals seem to use full power even if no heals
            if (!healed && Spell.Target.ToLower() == "realm")
                m_caster.Mana -= PowerCost(Target) * (int)DOL.GS.ServerProperties.Properties.HEAL_POWERCOST_AOE_MULTIPLICATOR >> 1; // only 1/2 power if no heal
            else
                m_caster.Mana -= PowerCost(Target) * (int)DOL.GS.ServerProperties.Properties.HEAL_POWERCOST_AOE_MULTIPLICATOR;
            ...
            if (Spell.Pulse == 0)
            {
                ...
                //foreach (GameLiving healTarget in targets)
                if (healTargets.IsAlive)
                    SendEffectAnimation(healTargets, 0, false, healed ? (byte)1 : (byte)0);
            }
        ...
        {
            if (Caster.ObjectState != GameLiving.eObjectState.Active || Caster == null || target == null || target.ObjectState != GameLiving.eObjectState.Active) return false;
