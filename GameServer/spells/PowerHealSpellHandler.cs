    ...
    public class PowerHealSpellHandler : SpellHandler
    {
        ...
        public virtual bool HealTarget(GameLiving target, int amount)
        {
            ...
            if (heal == 0)
            {
                if (Spell.Pulse == 0)
                {
                    if (target == m_caster) ((GamePlayer)m_caster).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)m_caster).Client.Account.Language, "PowerHealSpellHandler.SendCastMessage.PowerFull"), eChatType.CT_SpellResisted, eChatLoc.CL_ChatWindow);
                    //MessageToCaster("Your power is full.", eChatType.CT_SpellResisted);
                    else ((GamePlayer)m_caster).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)m_caster).Client.Account.Language, "PowerHealSpellHandler.SendCastMessage.TargetPowerFull", target.GetName(0, true)), eChatType.CT_SpellResisted, eChatLoc.CL_ChatWindow);
                }
                return false;
            }
