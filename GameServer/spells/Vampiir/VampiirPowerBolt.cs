		...
		public override bool CheckBeginCast(GameLiving selectedTarget)
		{
			if (Caster.InCombat == true)
			{
                if (Caster is GamePlayer)
                {
                    ...
                    (Caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((Caster as GamePlayer).Client.Account.Language, "VampiirPowerBolt.CanNotCastInCombat"), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                }
                    return false;
			}
		...
		public override void FinishSpellCast(GameLiving target)
		{
			if (target is Keeps.GameKeepDoor || target is Keeps.GameKeepComponent)
			{
                if (m_caster is GamePlayer)
                {
                    ...
                    (m_caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((m_caster as GamePlayer).Client.Account.Language, "VampiirPowerBolt.NoEffectOnKeepComponents"), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                }
                    return;
			}
			...
			protected override void OnTick()
			{
                ...
                if (target is GameNPC || target.Mana > 0)
                {
                    ...
                    if (target is GamePlayer)
                    {
                        ...
                        (target as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((target as GamePlayer).Client.Account.Language, "VampiirPowerBolt.TakesPower", caster.Name, power), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);

                    }

                    if (caster is GamePlayer)
                    {
                        (caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((caster as GamePlayer).Client.Account.Language, "VampiirPowerBolt.YouReceivePowerFrom", power, target.Name), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                    ...
                    if (caster is GamePlayer)
                {
                    ...
                    (caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((caster as GamePlayer).Client.Account.Language, "VampiirPowerBolt.YouReceiveNoPowerFrom", target.Name), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                }
