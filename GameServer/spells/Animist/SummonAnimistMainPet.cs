    ...
    public override bool CheckBeginCast(GameLiving selectedTarget)
    {
      if(Caster is GamePlayer && Caster.ControlledBrain != null)
      {
                if (m_caster is GamePlayer)
                {
                    ...
                    (m_caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((m_caster as GamePlayer).Client.Account.Language, "SummonAnimistMainPetSpellHandler.YouAlreadyHaveACharmedCreature"), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                }
                    return false;
      }
