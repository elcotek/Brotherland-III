...
namespace DOL.GS
{
    ...
    public sealed class WeatherManager
    {
        ...
        public bool ChangeWeather(ushort regionId, Action<RegionWeather> change)
        {
            ...
            lock (LockObject)
            {
                if (!RegionsWeather.TryGetValue(regionId, out RegionWeather weather))
                {
                    return false;
                }
        ...
        private void OnRegionStart(DOLEvent e, object sender, EventArgs arguments)
        {
            ...
            if (region != null && !region.IsDungeon && region.ID != 230 && region.ID != 247)
            {
        ...
        {
            Region region = sender as Region;
            RegionPlayerEventArgs args = arguments as RegionPlayerEventArgs;
        ...
        public void UnRegisterRegion(Region region)
        {
            ...
            lock (LockObject)
            {
                if (RegionsWeather.TryGetValue(region.ID, out RegionWeather weather))
                {
