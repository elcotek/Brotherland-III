	...
	public class TaskCommandHandler : AbstractCommandHandler, ICommandHandler
	{
		public void OnCommand(GameClient client, string[] args)
		{
			...
			if (args.Length > 1)
			{
				if (args[1] == "abort")
				{
					if (client.Player.QuestTask != null && client.Player.QuestTask.TaskActive)
						client.Player.QuestTask.ExpireTask();
				}
			}
			else
			{
				...
				if (player.QuestTask != null)
					player.QuestTask.CheckTaskExpired();
				
				AbstractTask task = player.QuestTask;
