...
namespace DOL.GS.Commands
{
	...
	public class AdviceCommandHandler : AbstractCommandHandler, ICommandHandler
	{
		public void OnCommand(GameClient client, string[] args)
		{
			...
			{
				client.Player.Out.SendMessage("You have been muted and are not allowed to speak in this channel.", eChatType.CT_Advise, eChatLoc.CL_SystemWindow);
				return;
			}
			...
			foreach (GameClient playerClient in WorldMgr.GetAllClients())
			{
				...
				if ((playerClient.Player.Realm == client.Player.Realm) ||
					...
					playerClient.Out.SendMessage(LanguageMgr.GetTranslation(client, "Scripts.Players.Advice.Advice", getRealmString(client.Player.Realm), client.Player.Name, msg), eChatType.CT_Advise, eChatLoc.CL_ChatWindow);
				   //playerClient.Player.Advisor 
			}
		}
