...
namespace DOL.GS.Commands
{
    ...
    public class GuildCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        ...
        //Keep claim Trget Level
        public static byte keepUpgradelevel = 0;
        public static long costforLevel = 0;
                    ...
                    case "remove":
                        {
                            ...
                            if (obj is GamePlayer)
                            {
                                ...
                                //log.Error("entferne online Player");
                            }
                            else
                            {
                                plyName = ch.Name;
                                ...
                                //log.Error("entferne offline Player");
                                GuildMgr.RemovePlayerFromAllGuildPlayersListBYID(client.Player, ch.ObjectId);
                            }
                    ...
                    case "list":
                        {
                            ...
                            {
                                if (guild.Realm == client.Player.Realm)
                                {
                                    if (guild.MemberOnlineCount > 0)
                                    {
                                        if (guild.Name != "Brotherland Staff") //no Server staff listening!
                                        {
                    ...
                    case "form":
                        {
                            if (!IsNearRegistrar(client.Player))
                            {
                                ...
                                client.Out.SendMessage(LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Player.Guild.MustNearAGuildRegistrar"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }
                            if (args.Length < 3)
                            {
                            ...
                            {
                                client.Out.SendMessage(LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Player.Guild.GuildNameTooLong"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                //client.Out.SendMessage("Sorry, your guild name is too long.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }
                            if (guildname.Length <= 6 || string.IsNullOrEmpty(guildname))
                            {
                                client.Out.SendMessage(LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Player.Guild.GuildNameMustContain6"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                //client.Out.SendMessage("Sorry, the guild name must contain at least 6 and characters.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }

                            System.Text.RegularExpressions.Regex pattern = new System.Text.RegularExpressions.Regex(@"^[A-Za-z…È\s]+$"); // erlaubte Zeichen
                            if (pattern.IsMatch(guildname.Trim()) == false)
                            {
                                client.Out.SendMessage(LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Player.Guild.GuildNameMustName"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                //client.Out.SendMessage("Sorry, the guild name can only contain letters and spaces.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }
                    ...
                    case "promote":
                        {
                            ...
                            if ((newrank >= guildRank && guildRank != 0) || (newrank < 0) || (newrank > 9))
                            {
                    ...
                    case "who":
                        {
                            ...
                            if (args.Length == 6 && args[2] == "window")
                            {
                                ...
                                if (Int32.TryParse(args[3], out int sortTemp) && Int32.TryParse(args[4], out int page) && Byte.TryParse(args[5], out byte showTemp) && sortTemp >= -7 && sortTemp <= 7)
                                {
                    ...
                    case "alliance":
                        {
                            ...
                            if (client.Player.Guild.AllianceId != null && string.IsNullOrEmpty(client.Player.Guild.AllianceId) == false)
                            {
                    ...
                    case "aremove":
                        {
                            ...
                            //Raus:client.Player.Guild.AllianceId != client.Player.Guild.AllianceId  neu:client.Player.Guild.GuildID != client.Player.Guild.alliance.Dballiance.DBguildleader.GuildID
                            if (client.Player.Guild.GuildID != client.Player.Guild.alliance.Dballiance.DBguildleader.GuildID || !client.Player.Guild.HasRank(client.Player, Guild.eRank.Leader))
                            {
                    ...
                    case "claim":
                        {
                            ...
                            if (keep.Region == 163 && keep.OriginalRealm != client.Player.Realm && ServerProperties.Properties.Allow_Claim_Own_Keeps_Only)
                            {
                                client.Out.SendMessage(LanguageMgr.GetTranslation(client, "Scripts.Player.Guild.KeepNotYourRealm"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }

                            if (client.Player.TargetObject == null || client.Player.TargetObject != null && client.Player.TargetObject is GuardLord == false)
                            {
                                client.Out.SendMessage(LanguageMgr.GetTranslation(client, "You need first to Targeting the Lord for Claiming this Keep!"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }
                    ...
                    case "release":
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (area is GameKeepArea && ((GameKeepArea)area).Keep.Guild == client.Player.Guild)
                                    {
                                        if (((GameKeepArea)area).Keep.CheckForRelease(client.Player))
                                        {
                                            ((GameKeepArea)area).Keep.Release();
                                        }
                                    }
                                }
                            }
                    ...
                    case "upgrade":
                        {
                            ...
                            if (keep.Region == 163 && keep.OriginalRealm != client.Player.Realm)
                            {
                                client.Out.SendMessage(LanguageMgr.GetTranslation(client, "Scripts.Player.Guild.KeepNotYourRealm"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }
                            ...
                            if (args.Length != 3)
                            {
                                client.Out.SendMessage(LanguageMgr.GetTranslation(client, "The level you wish ? /gc upgrade level"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }

                            try
                            {
                                keepUpgradelevel = Convert.ToByte(args[2]);
                                if (keepUpgradelevel > 5 || keepUpgradelevel < 1)
                                    return;
                            }
                            catch
                            {
                            ...
                            if (client.Player.Guild.ClaimedKeeps.Count > 1)
                            {
                                foreach (AbstractGameKeep Levelkeep in client.Player.Guild.ClaimedKeeps)
                                    Levelkeep.StartChangeLevel(targetlevel);
                            }
                            else
                            */
                            {
                                guildbps = client.Player.Guild.BountyPoints;

                                costforLevel = (long)keepUpgradelevel * 1000;

                                /*
                                log.ErrorFormat("targetlevel = {0}", keepUpgradelevel);
                                log.ErrorFormat("Guild bps = {0}", guildbps);
                                log.ErrorFormat("Guild costs = {0}", costforLevel);
                                */


                                if (costforLevel > client.Player.Guild.BountyPoints || client.Player.Guild.BountyPoints <= 0)
                                {



                                    client.Out.SendMessage(LanguageMgr.GetTranslation(client, "Your Guild need: " + costforLevel + " Bounty Points for this upgrade. Sorry Your Guild has only " + client.Player.Guild.BountyPoints + " Bounty Points. That is not enough for this upgrade!"), eChatType.CT_Guild, eChatLoc.CL_SystemWindow);
                                    return;

                                }
                                //this will cost
                                client.Out.SendCustomDialog("This upgrade will cost " + costforLevel + " Guild Bounty Points, do you sure to level up this Keep to level " + keepUpgradelevel + "?", new CustomDialogResponse(CheckGuildKeepLevelRequest));
                                return;
                            }
                            //client.Player.Guild.UpdateGuildWindow();
                            //return;
                    ...
                    case "dues":
                        {
                            ...
                            if (client.Player.Guild.GuildLevel < Properties.GUILD_DUE_LEVEL)
                            {
                                client.Out.SendMessage(LanguageMgr.GetTranslation(client, "Scripts.Player.Guild.GuildLevelReq"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }
        ...
        private static void CheckGuildKeepLevelRequest(GamePlayer player, byte response)
        {

            if (keepUpgradelevel <= 0)
                return;
            ...
            {
                {
                    ...
                    {
                        if (area is GameKeepArea && ((GameKeepArea)area).Keep.Guild == player.Guild)
                            ((GameKeepArea)area).Keep.ChangeLevel(keepUpgradelevel);
                    }
                }
                player.Guild.UpdateGuildWindow();
                player.Guild.RemoveBountyPoints(costforLevel);
                return;
            }
        }
        ...
        protected void AllianceInvite(GamePlayer player, byte reponse)
        {
            ...
            IList<DBAlliance> Alliance = GameServer.Database.SelectObjects<DBAlliance>("`AllianceName` = @AllianceName", new QueryParameter("@AllianceName", inviter.Guild.Name));

            if (Alliance != null)
            {
                foreach (DBAlliance db in Alliance)
                {
                    if (db.AllianceName == inviter.Guild.Name)
                    {
                        log.ErrorFormat("Alliance: {0} is arleady in DB!", inviter.Guild.Name);
                        return;
                    }
            ...
            try
            {
                if (inviter.Guild.alliance == null)
                {
                    ...
                    {
                        AllianceName = inviter.Guild.Name,
                        LeaderGuildID = inviter.GuildID,
                        DBguildleader = null,
                        Motd = String.Empty
                    };
            ...
            {
                log.ErrorFormat("Alliance konnte nicht geadded werden!");
            }
        }
