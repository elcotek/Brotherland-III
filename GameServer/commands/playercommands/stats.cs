            ...
            if (args.Length > 1)
            {
                ...
                if (args[1].ToLower() == "player")
                {
                    ...
                    if (args.Length > 2)
                    {
                        ...
                        clients = WorldMgr.GetClientByPlayerName(playerName, true, false);

                        if (clients == null)
                        {
                            DisplayMessage(client, "Player are not online or in your Realm.");
                            return;
                        }

                        if (clients != null && client.Player.Realm != clients.Player.Realm)
                        {
                            DisplayMessage(client, "Player are not online or in your Realm.");
                            return;
                        }
