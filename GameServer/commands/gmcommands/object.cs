...
namespace DOL.GS.Commands
{
    [CmdAttribute("&object", //command to handle
		          ...
		          "'/object level <value>' to set the level to the value",
				  "'/object modelinc' Increment the object model by 1",
                  "'/object modeldec' Decrement the object model by 1",
                  "'/object emblem <newEmblem>' to set the emblem to newEmblem, Emblem = 2250 allow to kill objects",
				  ...
				  "'/object health <value>' to sets the health of the object",
	...
	public class ObjectCommandHandler : AbstractCommandHandler, ICommandHandler
	{
		...
		public void OnCommand(GameClient client, string[] args)
		{
			...
			switch (args[1])
			{
				case "info":
					{
						...
						info.Add(" Level: " + targetObject.Level);
						info.Add(" Emblem: " + targetObject.Emblem);
						if (targetObject.Emblem == 2250)
                        {
							info.Add(" Killable: true");
						}
						else
						{
							info.Add(" Killable: false");
						}
						info.Add(" Health: " + targetObject.Health);
						info.Add(" MaxHealth: " + targetObject.MaxHealth);
				...
				case "level":
					{
						...
						try
						{
							level = Convert.ToUInt16(args[2]);
							targetObject.Level = (byte)level;
							targetObject.Health = targetObject.MaxHealth;
							targetObject.SaveIntoDatabase();
							DisplayMessage(client, "Object level changed to: " + targetObject.Level);
						}
						catch (Exception)
						{
							DisplayMessage(client, "Type /object for command overview");
				...
				case "health":
					{
						...
						try
						{
							...
							health = Convert.ToInt32(args[2]);  //Convert.ToUInt16(args[2]);
							
							targetObject.Health = health;
							targetObject.MaxHealth = health;
							targetObject.SaveIntoDatabase();
							
							DisplayMessage(client, "Object health changed to: " + targetObject.Health);
						}
						catch (Exception)
						{
							DisplayMessage(client, "Type /object for command overview");
				...
				case "respawn":
					{
                        if (int.TryParse(args[2], out int respawn))
                        {
