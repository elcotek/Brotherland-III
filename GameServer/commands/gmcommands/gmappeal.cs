                ...
                case "assist":
                    {
                        ...
                        GameClient targetClient = WorldMgr.GuessClientByPlayerNameAndRealm(targetName, 0, false, out int result);
                        switch (result)
                        {
                ...
                case "closeoffline":
                    {
                        ...
                        GameClient targetClient = WorldMgr.GuessClientByPlayerNameAndRealm(targetNameTwo, 0, false, out int resultTwo);
                        switch (resultTwo)
                        {
                            case 2: // name not unique
                                AppealMgr.MessageToClient(client, LanguageMgr.GetTranslation(client, "Scripts.Players.Appeal.NameNotUnique"));
