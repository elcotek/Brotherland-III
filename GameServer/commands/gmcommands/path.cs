	...
	public class PathCommandHandler : AbstractCommandHandler, ICommandHandler
	{
		...
		private void CreateTempPathObject(GameClient client, PathPoint pp, String name)
		{
            ...
            GameStaticItem obj = new GameStaticItem
            {
                ...
                X = pp.X,
                Y = pp.Y,
                Z = pp.Z + 1, // raise a bit off of ground level
        ...
        private void PathSave(GameClient client, string[] args)
        {
			...
			//make sure we dont override paths in db
			DBPath dbpath = GameServer.Database.SelectObjects<DBPath>("`PathID` = @PathID", new QueryParameter("@PathID", pathname)).FirstOrDefault();
			if (dbpath != null)
			{
				
				if (client.Account.PrivLevel != (uint)ePrivLevel.Admin)
				{
					DisplayMessage(client, "Route are not saved! - there is already a route with same name in DB, try another save name!");
					return;
				}
				
				if (dbpath != null && dbpath.PathID == pathname)
                {
					MovementMgr.RemovePathIDFromCache(dbpath.PathID);//remove old Route from Cache
					DisplayMessage(client, "Path are saved! - There was already a path with same name in DB. Update to the new one.");
                }
				MovementMgr.UpdatePathInCache(dbpath.PathID);//update/add to new Route in Cache
        ...
        private void PathAssignTaxiRoute(GameClient client, string[] args)
        {
            ...
            if (!ticketFound)
            {
				...
				DBPath dbpath = GameServer.Database.SelectObjects<DBPath>("`PathID` = @PathID", new QueryParameter("@PathID", pathname)).FirstOrDefault();
			if (dbpath != null)
			//IList<DBPathPoint> pathPoints = GameServer.Database.SelectObjects<DBPathPoint>("`PathID` = @PathID", new QueryParameter("@PathID", pathname));
			// foreach (DBPathPoint dba in pathPoints)
			{
				...
				if (dbpath != null && dbpath.PathID == pathname)
				{
					if (client.Account.PrivLevel != (uint)ePrivLevel.Admin)
					{
						DisplayMessage(client, "Taxi route not set! - there is already a route with same ticket in DB, try another ticket Id_nb name!");
						return;
					}
					if (dbpath != null && dbpath.PathID == pathname)
					{
						MovementMgr.RemovePathIDFromCache(dbpath.PathID);//remove old Route from Cache
						DisplayMessage(client, "Path are set! - There was already a path with same Ticket in the DB. Update to the new one.");
					}
					MovementMgr.UpdatePathInCache(dbpath.PathID); //update/add to new Route in Cache
				}
			}
            ...
            DisplayMessage(client, "Taxi route saved as '{0}'!", pathname);
        }
