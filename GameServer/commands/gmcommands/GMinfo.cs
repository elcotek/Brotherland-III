...
namespace DOL.GS.Commands
{
	...
	public class GMInfoCommandHandler : AbstractCommandHandler, ICommandHandler
	{
		...
		{
			long hour = WorldMgr.GetCurrentGameTime() / 1000 / 60 / 60;
			long minute = WorldMgr.GetCurrentGameTime() / 1000 / 60 % 60;
			long seconde = WorldMgr.GetCurrentGameTime() / 1000 % 60;
				
			string name = "(NoName)";
            var info = new List<string>
            {
                "        Current Region : " + client.Player.CurrentRegionID,
                " "
            };
			...
			if (client.Player.TargetObject != null)
			{
				...
				*/
				...
				if (client.Player.TargetObject is GamePlayer)
				{
					...
					info.Add("  - Guild : " + target.GuildName);
					info.Add(" ");
                    info.Add("  - Account Name : " + target.DBCharacter.AccountName);
