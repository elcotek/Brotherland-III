...
namespace DOL.GS.Commands
{
    [Cmd("&mob", //command to handle
         ...
         "'/mob styleid <string>' set the style ID for this mob",
         "'/mob styleallowed <true/false>' style allowed for the targt npc?.",
         "'/mob fish <true/false>' if the target npc is a fish ?.",
         "'/mob seeall <true/false>' if the target npc can see all in 360 radius or only in front?.",
         "'/mob spawn <'name'> <Region id>' respawn an mob like name in the current region",
         "'/mob yellrange <range> set the yelling range",
         "'/mob attackrange <range> set the melee attack range",
         "'/mob petcount <count>' set the max numer of allowed monster pets",
         "'/mob reloaddaynight' Reloads all day/night mobs into the cache"
        )]
    ...
    public class MobCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        ...
        public void OnCommand(GameClient client, string[] args)
        {
            ...
            try
            {
                ...
                switch (args[1])
                {
                    ...
                    case "petcount": petcount(client, targetMob, args); break;
                    ...
                    case "reloaddaynight": reloaddaynight(client, targetMob, args); break;
                    case "findname": findname(client, args); break;
                    case "trigger": trigger(client, targetMob, args); break;
                    case "styleid": styleid(client, targetMob, args); break;
                    case "styleallowed": Style(client, targetMob, args); break;
                    case "fish": IsFish(client, targetMob, args); break;
                    case "seeall": IsSeeAll(client, targetMob, args); break;
                    case "spawn": spawn(client, args); break;
                    case "yellrange": yellrange(client, targetMob, args); break;
                    case "attackrange": meleeattackrange(client, targetMob, args); break;
        ...
        {
            if (ushort.TryParse(args[3], out ushort mobOID))
            {
        ...
        private void name(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            if (String.IsNullOrEmpty(mobName) == false)
            {
        ...
        private void guild(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            if (String.IsNullOrEmpty(guildName) == false)
            {
            ...
            {
                if (String.IsNullOrEmpty(targetMob.GuildName) == false)
                {
        ...
        private void race(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            if (!short.TryParse(args[2], out short raceID))
            {
        ...
        private void flags(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            uint.TryParse(args[2], out uint flag);
        ...
        private void info(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            int _constitution = targetMob.GetModified(eProperty.Constitution);

            HITPOINTS = (20 + ((targetMob.Level * 20 * _constitution / 250) * 2) /* * (50 + CL) / 50]
          * (100 + TANK) / 100] + HPITEM + HPRA */);
            info.Add(" + HITPOINTS:  " + HITPOINTS);
            ...
            if (targetMob.Brain != null && targetMob.Brain is IControlledBrain)
            {
                try
                {
                    if ((targetMob.Brain as IControlledBrain).GetPlayerOwner() != null)
                    {
                    ...
                    if ((targetMob.Brain as IControlledBrain).GetNPCOwner() != null)
                    {
                        info.Add(" + -- Owner: " + (targetMob.Brain as IControlledBrain).GetNPCOwner() != null ? (targetMob.Brain as IControlledBrain).GetNPCOwner().Name : (targetMob.Brain as IControlledBrain).GetNPCOwner().Name);
                    }
                }
                catch
                {
            ...
            if (aggroBrain != null)
            {
                ...
                if (targetMob.Faction != null)
                {
                    if (targetMob.Faction.GetAggroToFaction(client.Player) != 0)
            ...
            info.Add(" + Mob_StyleID:  " + targetMob.MobStyleID);
            info.Add(" + Mob Style Allowed:  " + targetMob.IsStyleAllowed);
            info.Add(" + Mob is Fish:  " + targetMob.NPCIsFish);
            info.Add(" + Mob see all in 360 Radius:  " + targetMob.NpcIsSeeingAll);
            info.Add(" + Mob yellrange:  " + targetMob.YellRange);
            info.Add(" + Mob Allowed Petcount:  " + targetMob.ControlledCount);

            if (targetMob.MeleeAttackRange > 0)
                info.Add(" + Mob Melee Attackrange:  " + targetMob.MeleeAttackRange);
            else
                info.Add(" + Mob Melee Attackrange: 126");
        ...
        private void level(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            if (args.Length > 2 && byte.TryParse(args[2], out byte level))
            {
        ...
        private void petcount(GameClient client, GameNPC targetMob, string[] args)
        {

            if (args.Length > 2 && int.TryParse(args[2], out int pcount))
            {
                targetMob.ControlledCount = pcount;
                targetMob.SaveIntoDatabase();
                client.Out.SendMessage("Mob allowed pet Count changed to: " + targetMob.ControlledCount, eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
            else
            {
                DisplaySyntax(client, args[1]);
        ...
        private void dropcount<T>(GameClient client, GameNPC targetMob, string[] args) where T : MobXLootTemplate
        {
            ...
            else
            {
                ...
                if (!int.TryParse(args[2], out int dropCount))
                {
                    DisplaySyntax(client, args[1]);
                    return;
                }
        ...
        private void Addotd(GameClient client, GameNPC targetMob, string[] args)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        MobName = mobName,
                        ItemTemplateID = itemtemplate.Id_nb,
                        MinLevel = minlevel
                    };
            ...
            where LootTemplateType : LootTemplate
        {
            ...
            //var mobXloot = GameServer.Database.SelectObjects<MobDropTemplateType>("`MobName` = @MobName", new QueryParameter("@MobName", mobName));
            var mobXloot = GameServer.Database.SelectObjects<MobDropTemplateType>("`MobName` = @MobName", new QueryParameter("@MobName", mobName));
        ...
        private void Refreshloot(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            client.Out.SendMessage("Loot refreshed on:  " + targetMob.Name, eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
        ...
        private void Copy(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            mob.NpcIsSeeingAll = targetMob.NpcIsSeeingAll;
            mob.NPCIsFish = targetMob.NPCIsFish;
            mob.YellRange = targetMob.YellRange;
            mob.MeleeAttackRange = targetMob.MeleeAttackRange;
            ...
            mob.MobStyleID = targetMob.MobStyleID;
            ...
            if ((mob.Flags & GameNPC.eFlags.PEACE) != 0)
            {
                ...
                client.Out.SendMessage("This mobs PEACE flag is set!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
        }
        ...
        void NpctCreate(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            if (Int32.TryParse(args[3], out int id) == false)
            {
                DisplaySyntax(client, args[1], args[2]);
                return;
            }
        ...
        private void Style(GameClient client, GameNPC targetMob, string[] args)
        {
            targetMob.IsStyleAllowed ^= true;
            targetMob.UpdateNPCEquipmentAppearance();
            targetMob.SaveIntoDatabase();
            client.Out.SendMessage("Mob StyleAllowed flag is set to " + targetMob.IsStyleAllowed, eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }

        private void IsFish(GameClient client, GameNPC targetMob, string[] args)
        {
            targetMob.NPCIsFish ^= true;
            targetMob.UpdateNPCEquipmentAppearance();
            targetMob.SaveIntoDatabase();
            client.Out.SendMessage("Mob is fish flag is set to " + targetMob.NPCIsFish, eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
        private void IsSeeAll(GameClient client, GameNPC targetMob, string[] args)
        {
            targetMob.NpcIsSeeingAll ^= true;
            targetMob.UpdateNPCEquipmentAppearance();
            targetMob.SaveIntoDatabase();
            client.Out.SendMessage("Mob is fish flag is set to " + targetMob.NpcIsSeeingAll, eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
        ...
        private void bodytype(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            if (args.Length > 2 && ushort.TryParse(args[2], out ushort type))
            {
        ...
        private void yellrange(GameClient client, GameNPC targetMob, string[] args)
        {
            try
            {
                int yellrange = Convert.ToInt32(args[2]);
                targetMob.YellRange = yellrange;
                targetMob.SaveIntoDatabase();
                client.Out.SendMessage("Mob max yellrange changed to: " + targetMob.YellRange, eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
            catch (Exception)
            {
                DisplaySyntax(client, args[1]);
        ...
        private void meleeattackrange(GameClient client, GameNPC targetMob, string[] args)
        {
            try
            {
                int MeleeAttackrange = Convert.ToInt32(args[2]);
                targetMob.MeleeAttackRange = MeleeAttackrange;
                targetMob.SaveIntoDatabase();
                client.Out.SendMessage("Mob max melee attackrange changed to: " + targetMob.MeleeAttackRange, eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
            catch (Exception)
            {
                DisplaySyntax(client, args[1]);
        ...
        private void styleid(GameClient client, GameNPC targetMob, string[] args)
        {
            string MobstyleID;
            try
            {
                ...
                if (MobstyleID == String.Empty)
                {
                    DisplaySyntax(client, args[1]);
                    return;
                }

                targetMob.MobStyleID = MobstyleID;
                targetMob.SaveIntoDatabase();
                client.Out.SendMessage("StyleID set to " + MobstyleID, eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
            catch (Exception)
            {
                DisplaySyntax(client, args[1]);
        ...
        private void reloaddaynight(GameClient client, GameNPC targetMob, string[] args)
        {
            GameNPC.ReloadDayNightMobs();
            client.Player.Out.SendMessage("All Day/Night Mob lists reloaded!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
        private void reload(GameClient client, GameNPC targetMob, string[] args)
        {
            ...
            else if (targetMob != null)
            {
                ...
                {
                    if (targetMob.Brain is StandardMobBrain)
                    {
                        (targetMob.Brain as StandardMobBrain).ClearAggroList();
                    }

                    FactionMgr.Init();
                    targetMob.ReloadAmbientText();
        ...
        private void spawn(GameClient client, string[] args)
        {
            if (args.Length < 2)
                return;
            ...
            Log.WarnFormat("commando ausgeführt {0} 3 = {1}", args[2], args[3]);

            var mob = GameServer.Database.SelectObjects<Mob>("`Name` = @Name AND `Region` = @Region", new[] { new QueryParameter("@Name", args[2].ToString()), new QueryParameter("@Region", args[3].ToString()) });


            foreach (Mob db in mob)
            {
                if (db != null && db.Name == args[2].ToString() && args[3].ToString() == db.Region.ToString())
                {
                    if (db.Name.Length > 0)
                    {
                        for (eRealm i = eRealm._First; i <= eRealm._Last; i++)
                        {
                            mobs.Add(WorldMgr.GetNPCsByName(db.Name, i));
                        }
                        foreach (GameNPC[] ma in mobs)
                        {
                            foreach (GameNPC n in ma)
                            {
                                if (n.LoadedFromScript == false && db.Name == n.Name && db.Region == n.CurrentRegionID)
                                {
                                    //n.RemoveFromWorld();
                                    n.RespawnInterval = 10;
                                    n.LoadFromDatabase(db);
                                    n.AddToWorld();
                                    client.Player.Out.SendMessage(n.Name + " reloaded!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                    ...
                                    //    GameNPC npc = new GameNPC();
                                    /// npc.LoadFromDatabase(db);
                                    //  npc.Brain = db.Brain;
                                    // npc.AddToWorld();

                                    //Log.Warn("mob gefunden und geladen");
                                }



                                client.Player.Out.SendMessage(n.Name, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            }
                        }
                    }
                    else
        ...
        {
            if (args.Length < 4 || !int.TryParse(args[3], out int i))
            {
                DisplaySyntax(client);
                return;
            }
