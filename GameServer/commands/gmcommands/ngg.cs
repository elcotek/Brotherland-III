//===============================================
// '/ngg' command fixed to work with latest revision - 2023 By Elcotek
...
namespace DOL.GS.Commands
{
    [Cmd("&ngg", //command to handle
       ...
       "---------------------------------",
       "[slotNumber] :",
       "- for weapons : 'right hand'=10, 'left hand'=11, 'two handed'=12, 'distance'=13",
       "- for armors : 'head'=21, 'hands'=22, 'boots'=23, 'torso'=25, 'cloak'=26, 'legs'=27, 'arms'=28",
    ...
    public class NGGCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        ...
        */
        ...
        public static void OnScriptCompiled(DOLEvent e, object sender, EventArgs args)
        {
            ...
            for (int a = 0; a < 6; a++)
            {
                ...
                for (int b = 0; b < 6; b++)
                {
                    ...
                    temp = GameServer.Database.SelectObjects<ItemTemplate>("`Item_Type` = " + ot[a] + " AND @Item_Type", new QueryParameter("@Item_Type", +slots[b]));
            ...
            x = 0;
            //temp = GameServer.Database.SelectObjects<ItemTemplate>("Item_Type = " + slots[6]);
            //temp = GameServer.Database.SelectObjects<ItemTemplate>("Item_Type = " + slots[6]);
            temp = GameServer.Database.SelectObjects<ItemTemplate>("`Item_Type` = @Item_Type", new QueryParameter("@Item_Type", slots[6]));
            ...
            {
                x = 0;
                //temp = GameServer.Database.SelectObjects<ItemTemplate>( "Item_Type = " + slots[a]);
                temp = GameServer.Database.SelectObjects<ItemTemplate>("`Item_Type` = @Item_Type", new QueryParameter("@Item_Type", slots[a]));
            ...
            if (args[1] != "epic")
            {
                switch (args[1])
                {
                    ...
                    case "save":
                        {
                            Save(npc);
                            client.Out.SendMessage("NPC Saved!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        private void Save(GameNPC target)
        {
            ...
            do
            {
                tn = System.Guid.NewGuid().ToString();
