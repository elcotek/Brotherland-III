...
namespace DOL.GS.Commands
{
    ...
    public class PlayerCommandHandler : AbstractCommandHandler, ICommandHandler
    {
                                ...
                                default:
                                    {
                                        ...
                                        if (ushort.TryParse(args[modelIndex], out ushort modelID) == false)
                                        {
                ...
                case "friend":
                    {
                        ...
                        GameClient fclient = WorldMgr.GuessClientByPlayerNameAndRealm(name, 0, true, out int result);
                ...
                case "saddlebags":
                    {
                        ...
                        if (byte.TryParse(args[2], out byte activeBags))
                        {
                            if (activeBags <= 0x0F)
                            {
                ...
                case "allchars":
                    {
                        ...
                        GameClient targetClient = targetPlayer?.Client;
        ...
        private void Show_Inventory(GamePlayer player, GameClient client, string limitType)
        {
            var text = new List<string>
            {
                "  - Name Lastname : " + player.Name + " " + player.LastName,
                "  - Realm Level Class : " + GlobalConstants.RealmToName(player.Realm) + " " + player.Level + " " +
                     player.CharacterClass.Name,
                " ",
                Money.GetShortString(player.GetCurrentMoney()),
                " "
            };
        ...
        private void Show_Info(GamePlayer player, GameClient client)
        {
            ...
            {
                " ",
                "PLAYER INFORMATION (Client # " + player.Client.SessionID + ", " + player.GetType().FullName + ")",
                "  - Name Lastname : " + player.Name + " " + player.LastName,
                "  - Realm Level Class : " + GlobalConstants.RealmToName(player.Realm) + " " + player.Level + " " + player.CharacterClass.Name + " (" + player.CharacterClass.ID + ")",
                "  - Guild : " + player.GuildName + " " + (player.GuildRank != null ? "Rank: " + player.GuildRank.RankLevel.ToString() : ""),
                "  - XPs/RPs/BPs : " + player.Experience + " xp, " + player.RealmPoints + " rp, " + player.BountyPoints + " bp"
            };
            ...
            if (player.Champion)
            {
                ...
                if (player.ActiveSaddleBags != 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (String.IsNullOrEmpty(activeBags) == false)
                                activeBags += ", ";
