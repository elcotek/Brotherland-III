	...
	public class PacketLib1105 : PacketLib1104
	{
		...
		public override void SendTrainerWindow()
		{
			...
			if (raList != null && raList.Count > 0)
			{
                ...
                foreach (var (ra, playerRA) in from RealmAbility ra in raList
                                               let playerRA = (RealmAbility)m_gameClient.Player.GetAbility(ra.KeyName)
                                               select (ra, playerRA))
                {
                    if (playerRA != null)
                    {
                ...
                using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
				{
					...
					int i = 0;
                    for (int i1 = 0; i1 < offeredRA.Count; i1++)
					{
                        RealmAbility ra = offeredRA[i1];
                        pak.WriteByte((byte)i++);
            ...
            using (GSTCPPacketOut paksub = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
            {
                ...
                IList spcls = m_gameClient.Player.GetSpecList().ToArray();
                ...
                {
                    SpellLine line = (SpellLine)spls[i];
                    if (line.IsBaseLine)
                    {
                        continue;
                    }
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
			{
                ...
                for (int i1 = 0; i1 < ras.Count; i1++)
				{
                    RealmAbility ra = ras[i1];
