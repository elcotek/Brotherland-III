...
namespace DOL.GS.PacketHandler
{
    ...
    public class PacketLib1124 : AbstractPacketLib, IPacketLib
    {
        ...
        {
            get { return false; }//ServerProperties.Properties.USE_NEW_TOOLTIP_FORCEDUPDATE; }
        }
        ...
        protected string BuildTaskString()
        {
            ...
            AbstractTask task = m_gameClient.Player.QuestTask;
        ...
        public virtual void SendCharStatsUpdate()
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.StatsUpdate)))
            {
                ...
                for (int i = 0; i < updateStats.Length; i++)
                {
                    ...
                    if (updateStats[i] == m_gameClient.Player.CharacterClass.ManaStat)
                    {
                        if (m_gameClient.Player.IsArcherClass() == false)
                        {
        ...
        public virtual void SendDupNameCheckReply(string name, bool result)
        {
        ...
        */
        ...
        public virtual void SendKeepComponentUpdate(AbstractGameKeep keep, bool LevelUp)
        {
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentUpdate)))
            {
                ...
                foreach (GameKeepComponent component in keep.SentKeepComponents)
                {
                ...
                foreach (SpellLine line in spellLines)
                {
                    ...
                    if (player.CharacterClass.ClassType == eClassType.ListCaster || player.IsAdvancedSpellLine(line))
                    {
                        ...
                        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
                        {
                            ...
                            pak.WriteShort((ushort)spells[index].InternalID); //new 1.112
                            pak.WriteShort(0); // icon, not used when spell line
                            pak.WritePascalString(line.Name); // richtig??
                            ...
                            foreach (Spell spell in spells)
                            {
                                if (spell.Level <= line.Level && spell.SpellType != "StyleHandler")
                                {
                                    ...
                                    pak.WriteShortLowEndian((byte)spell.Level);
        ...
        public virtual void SendNonHybridSpellLines()
        {
            ...
            List<Tuple<SpellLine, List<Skill>>> spellsXLines = m_gameClient.Player.GetSpellLines();
        ...
        */
        ...
        public virtual void SendPlayerCreate(GamePlayer playerToCreate)
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.PlayerCreate172)))
            {
                ...
                if (playerToCreate.IsCharcterClass(eCharacterClass.Vampiir)) flags |= 0x40; //Vamp fly
        ...
        */
        ...
        public virtual void SendPlayerTitleUpdate(GamePlayer player)
        {
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VisualEffect)))
            {
                ...
                else
                {
                    ...
                    string val = title.GetValue(player); // GameServer.ServerRules.GetPlayerTitle(m_gameClient.Player, player);
        ...
        {
            SendQuestWindow(questObject, player, quest, true);
        }
        ...
        {
            SendQuestWindow(questObject, player, quest, false);
        }
        ...
        protected virtual void SendQuestWindow(GameObject questObject, GamePlayer player, DataQuest quest, bool offer)
        {
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.Dialog)))
            {
                ...
                pak.WriteShort((ushort)questObject.ObjectID);
                pak.WriteByte(0x00); // unknown
                pak.WriteByte(0x00); // unknown
                pak.WriteByte(0x00); // unknown
                pak.WriteByte(0x00); // unknown
        ...
        */
        ...
        public virtual void SendUpdatePlayer()
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
            {
                ...
                pak.WriteByte(0x00); //unk
                pak.WritePascalString(player.CurrentTitle.ToString()); //gettGetTitle(player, player.Level)); // player level
                ...
                pak.WritePascalString(player.RealmTitle.ToString()); // Realm title
                ...
                pak.WriteByte(0x0);
                pak.WritePascalString(player.CraftTitle.ToString()); //crafter title: legendary alchemist
                pak.WriteByte(0x0);
                pak.WritePascalString(player.MLTitle.ToString()); //ML title

                // new in 1.75
                pak.WriteByte(0x0);
                if (player.CurrentTitle != PlayerTitleMgr.ClearTitle)
                    pak.WritePascalString(player.CurrentTitle.ToString()); // new in 1.74 - Custom title
                else
                ...
                if (player.Champion)
                    ...
                    pak.WriteByte(0x0);
                pak.WritePascalString(player.CLTitle.ToString()); // Champion Title
                SendTCP(pak);
            }
        }
                    ...
                    for (int skillIndex = 0; skillIndex < tree.Count; skillIndex++)
                    {
                        ...
                        for (int itemIndex = 0; itemIndex < tree[skillIndex].Item2.Count; itemIndex++)
                        {
                            ...
                            if (sk != null)
                            {
                                ...
                                pak.WriteShortLowEndian(sk.ID); // Icon should be style icon + 3352 ???
                                pak.WritePascalString(sk.Name);
