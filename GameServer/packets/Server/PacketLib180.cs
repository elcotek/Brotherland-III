                ...
                lock (styleList.SyncRoot)
                {
                    lock (specs.SyncRoot)
                    {
                        lock (m_gameClient.Player.lockSpellLinesList)
                        {
                            ...
                            {
                                Specialization spec = (Specialization)specs[i];
                            ...
                            {
                                Style style = (Style)styleList[i];
                                CheckLengthHybridSkillsPacket(ref pak, ref maxSkills, ref firstSkills);

                                m_styleId[(int)style.ID] = count++;
                               // log.DebugFormat("Style {0} - {1} - {2}", style.ID, style.Name, style.Spec);
