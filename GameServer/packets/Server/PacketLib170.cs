...
namespace DOL.GS.PacketHandler
{
    ...
    public class PacketLib170 : PacketLib169
    {
        ...
        public override void SendKeepComponentRemove(GameKeepComponent keepComponent)
        {
        ...
        public override void SendKeepClaim(AbstractGameKeep keep, byte flag)
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepClaim)))
            {
                ...
                {
                    pak.WriteByte((byte)ServerProperties.Properties.MAX_KEEP_TOWER_LEVEL);
                }
                pak.WriteByte((byte)keep.Level);
                SendTCP(pak);
            }
        ...
        public override void SendKeepComponentInteract(GameKeepComponent component)
        {
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentInteractResponse)))
            {
                ...
                {
                    pak.WriteByte(component.Keep.EffectiveLevel((byte)ServerProperties.Properties.MAX_KEEP_TOWER_LEVEL));
                }
        ...
        public override void SendHookPointStore(GameKeepHookPoint hookPoint)
        {
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentHookpointStore)))
            {
                ...
                {
                    inventory = hookPoint.ID > 0x20 ? HookPointInventory.BlueHPInventory : HookPointInventory.RedHPInventory;//guard
                }

                pak.WriteByte((byte)inventory.GetAllItems().Count);//count
                pak.WriteShort(0);
                int i = 0;
                ArrayList list = inventory.GetAllItems();
                for (int i1 = 0; i1 < list.Count; i1++)
                {
                    HookPointItem item = (HookPointItem)list[i1];
        ...
        public override void SendWarmapUpdate(ICollection<AbstractGameKeep> list)
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.WarMapClaimedKeeps)))
            {
                ...
                foreach (AbstractGameKeep keep in list)
                {
                    ...
                    //by Elcotek if a Lord in combat so are the keep under fire!
                    foreach (GameKeepGuard guard in keep.Guards.Values)
                    {

                        if (guard != null && guard is GuardLord && (guard as GuardLord).HealthPercent < 100)
                        {
                            flag |= (byte)eRealmWarmapKeepFlags.UnderSiege;
                            //log.Error("Lord in combat!");
                        }
                    ...
                    if (keep != null)
                    {
                        ...
                        if (keep is GameKeep || keep is GameKeepTower)
                        {
                            foreach (GameKeepComponent keepComponent in keep.SentKeepComponents)
                            {
                                ...
                                if (keepComponent is GameKeepComponent && keepComponent != null && keepComponent.HealthPercent <= 0)
                                {
