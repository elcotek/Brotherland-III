    ...
    public class PacketLib173 : PacketLib172
    {
                        ...
                        for (int j = 0; j < characters.Length && written == false; j++)
                        {
                            if (characters[j].AccountSlot == i)
                            {
                                ...
                                if (zon != null)
                                {
                                    ...
                                    {
                                        AbstractArea area = (AbstractArea)areas[i1];
        ...
        public override void SendQuestUpdate(AbstractQuest quest)
        {
            ...
            if (m_gameClient != null && m_gameClient.Player != null && m_gameClient.Player.QuestList != null)
            {
                ...
                {
                    for (int i = 0; i < m_gameClient.Player.QuestList.Count; i++)
                    {
                        AbstractQuest q = m_gameClient.Player.QuestList[i];
                        if (q == quest)
                        {
        ...
        public override void SendQuestListUpdate()
        {
            ...
            lock (m_gameClient.Player.QuestList)
            {
                for (int i = 0; i < m_gameClient.Player.QuestList.Count; i++)
                {
                    AbstractQuest quest = m_gameClient.Player.QuestList[i];
                    if (quest.Step != -1)
                    {
