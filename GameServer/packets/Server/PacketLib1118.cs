...
{
    [PacketLib(1117, GameClient.eClientVersion.Version1118)]
    ...
    foreach (Skill t in skills)
    {
        ...
        if (t is RealmAbility)
        {
            if (m_gameClient.CanSendTooltip(27, t.InternalID))
                SendDelveInfo(DOL.GS.PacketHandler.Client.v168.DetailDisplayHandler.DelveSpell(m_gameClient, t.InternalID));
        }
        ...
        {
            if (t is Song || (t is Spell && ((Spell)t).InstrumentRequirement != 0))
            {
