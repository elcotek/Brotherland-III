    ...
    public class PacketLib175 : PacketLib174
    {
        ...
        public override void SendPlayerTitles()
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.DetailWindow)))
            {
                ...
                byte line = 1;
                IList<string> list = m_gameClient.Player.FormatStatistics();
                ...
                {
                    IPlayerTitle title = (IPlayerTitle)titles[i];
                    pak.WriteByte(line++);
        ...
        public override void SendUpdatePlayer()
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
            {
                ...
                pak.WritePascalString(player.CharacterClass.GetTitle(player.Level, player));
        ...
        public override void SendCharStatsUpdate()
        {
            ...
            int[] mythCaps = new int[updateStats.Length];
            ...
            for (int i = 0; i < updateStats.Length; i++)
            {
                ...
                switch ((eProperty)updateStats[i])
                {
                    case eProperty.Strength:
                        extraCaps = m_gameClient.Player.ItemBonus[(int)eProperty.MythicalStrCapBonus];
                        break;
                    case eProperty.Dexterity:
                        extraCaps = m_gameClient.Player.ItemBonus[(int)eProperty.MythicalDexCapBonus];

                        break;
                    case eProperty.Constitution:
                        extraCaps = m_gameClient.Player.ItemBonus[(int)eProperty.MythicalConCapBonus];

                        break;
                    case eProperty.Quickness:
                        extraCaps = m_gameClient.Player.ItemBonus[(int)eProperty.MythicalQuiCapBonus];

                        break;
                    case eProperty.Intelligence:
                        extraCaps = m_gameClient.Player.ItemBonus[(int)eProperty.MythicalIntCapBonus];

                        break;
                    case eProperty.Piety:
                        extraCaps = m_gameClient.Player.ItemBonus[(int)eProperty.MythicalPieCapBonus];

                        break;
                    case eProperty.Charisma:
                        extraCaps = m_gameClient.Player.ItemBonus[(int)eProperty.MythicalChaCapBonus];

                        break;
                    case eProperty.Empathy:
                        extraCaps = m_gameClient.Player.ItemBonus[(int)eProperty.MythicalEmpCapBonus];

                        break;
                    default: break;

                }
                mythCaps[i] = extraCaps;
                //log.ErrorFormat("mythCaps[i] {0}", mythCaps[i]);
            }
            ...
            for (int i = 0; i < updateStats.Length; i++)
            {
                ...
                mythcap = Math.Min(8, mythCaps[i]);

               //log.ErrorFormat("cap {0}. itemCap {1} + bonusCap {2} - mythExtraCap {3} ", cap, itemCap, bonusCap, mythExtraCap);
               itemCaps[i] = Math.Min(cap + mythcap, itemCap + mythcap + bonusCap);
            }
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.StatsUpdate)))
            {
                ...
                for (int i = 0; i < updateStats.Length; i++)
                {
                    ...
                    if (updateStats[i] == m_gameClient.Player.CharacterClass.ManaStat)
                    {
                        ...
                        if (m_gameClient.Player.CharacterClass.ClassType != eClassType.PureTank && m_gameClient.Player.IsArcherClass() == false)
                        {
        ...
        public override void SendPlayerCreate(GamePlayer playerToCreate)
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.PlayerCreate172)))
            {
                ...
                if (playerToCreate.IsCharcterClass(eCharacterClass.Vampiir))
                {
                    flags |= 0x40; //Vamp fly
                }
