...
namespace DOL.GS.PacketHandler
{
    ...
    public class PacketLib1109 : PacketLib1108
    {
        ...
        {
            if (m_gameClient == null || m_gameClient.Player == null) // Überprüfung auf null
            {
                return;
            }
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TradeWindow)))
            {
                ...
                {
                    if (m_gameClient.Player.TradeWindow.TradeItems == null) // Überprüfung auf null
                    {
                        return;
                    }
        ...
        protected override void WriteTemplateData(GSTCPPacketOut pak, ItemTemplate template, int count)
        {
            ...
            if (count > 1)
            {
                pak.WritePascalString(String.Format("{0} {1}", count.ToString(), template.Name));
            }
            else
            {
