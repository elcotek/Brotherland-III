...
namespace DOL.GS.PacketHandler
{
    ...
    public class PacketLib1112 : PacketLib1111
    {
		...
		public override void SendUpdatePlayerSkills()
		{
            ...
            int total = 0;//usableSkills.Count; // cache List count.
			...
			while (!sent)
			{
				...
				using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
				{
					...
					while(index < total)
					{
						...
						if (index >= null) // byte.MaxValue) || ((pak.Length + 8 + usableSkills[index].Item1.Name.Length) > 1400))
						{
							break;
						}
						
						// Enter Packet Values !! Format Level - Type - SpecialField - Bonus - Icon - Name
                        Skill skill = null;// usableSkills[index].Item1;
						...
						else if (skill is Spell)
						{
							...
							pak.WriteShort(spell.Icon > 0 ? spell.Icon : spell.Icon); // icon
							...
							switch (style.OpeningRequirementType)
							{
								case Style.eOpening.Offensive:
                                    ...
                                    //if (style.AttackResultRequirement == Style.eAttackResultRequirement.Style)
									{
                                        ...
                                        int styleindex = 0;// Math.Max(0, usableSkills.FindIndex(it => (it.Item1 is Style) && it.Item1.ID == style.OpeningRequirementValue));										
                                        int speccount = 0; // Math.Max(0, usableSkills.FindIndex(it => (it.Item1 is Specialization) == false));										
		...
		public override void SendNonHybridSpellLines()
		{
			...
			//List<Tuple<SpellLine, List<Skill>>> spellsXLines = player.GetAllDisabledSkills(true);
            string spellsXLines = null;
			int lineIndex = 0;
			foreach (var spXsl in spellsXLines)
			{
				...
				using(var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
                {
                    ...
                    pak.WriteByte((byte)(0)); //number of entry
                    ...
                    pak.WriteShort((ushort)0); //new 1.112
                    pak.WriteShort(0); // icon, not used when spell line
                    pak.WritePascalString(null);

                    int Item2 = 0;
                    // Add All Spells...
                    //foreach (Skill sk in spXsl)
                    {
                        //if (sk is Spell)
                        {
                            //Spell sp = (Spell)sk;
                            pak.WriteShortLowEndian((byte)0);
                            pak.WriteShort((ushort)0); //new 1.112
                            pak.WriteShort(0);
                            pak.WritePascalString(null);
                        }
			...
			if (item.Object_Type != (int)eObjectType.AlchemyTincture)
			{
				if (item.SpellID > 0/* && item.Charges > 0*/)
				{
					...
					if (chargeEffectsLine != null)
					{
                        ...
                        foreach (var spl in from Spell spl in spells
                                            where spl.ID == item.SpellID
                                            select spl)
                        {
                            flag |= 0x08;
                            icon1 = spl.Icon;
