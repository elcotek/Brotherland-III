...
namespace DOL.GS.PacketHandler
{
    ...
    public class PacketLib1115 : PacketLib1114
    {
        ...
        public override void SendVersionAndCryptKey()
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CryptKey)))
            {
        ...
        public virtual void SendKeepInfo(IKeep keep)
        {
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepInfo)))
            {
               ...
               // pak.WriteShort(Convert.ToUInt16(keep.KeepID)); //points
               // pak.WriteShort((uint)keep.KeepID);
                pak.WriteShort(0);
            ...
            using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.WarMapClaimedKeeps)))
            {
                ...
                foreach (AbstractGameKeep keep in list)
                {
                    ...
                    foreach (GameKeepGuard guard in keep.Guards.Values)
                    {
                        ...
                        if (guard != null && guard is GuardLord && guard.InCombat)
                        {
