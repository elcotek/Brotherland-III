    ...
    public class WorldInitRequestHandler : IPacketHandler
    {
        ...
        protected class WorldInitAction : RegionAction
        {
            ...
            protected override void OnTick()
            {
                ...
                {
                    log.ErrorFormat("Failed to add player to the region! {0}", player.Name);
                ...
                //Man darf nur in einem realm zur zeit sein ansonsten wird man disconnected!
                if (player.Client.Account.PrivLevel == (uint)ePrivLevel.Player)
                {
                    ...
                    foreach (GameClient client in WorldMgr.GetAllPlayingClients())
                    {
                        ...
                        if (client != null && (string.IsNullOrEmpty(player.Client.Account.AccountID) == false) && player.Client.Account.IsOtherRealmAllowed == false && player.Client.Account.AccountID == client.Account.AccountID && player.Realm != client.Player.Realm)
                        {
                        ...
                        if (client != null && (string.IsNullOrEmpty(player.Client.Account.AccountID) == false) && player.Client.Account.IsOtherRealmAllowed == false && player.Client.Account.AccountID == client.Account.AccountID)
                        {

                            totalacc++;


                            if (totalacc >= Properties.TOTAL_ACCOUNTS_ALLOWED_SAMEIP && totalacc > 0)
                            {
                                if (player.Client != null)
                                {
                                    log.Warn("Accounta login: too many from same Account ID - " + client.Account.Name + "");
