	...
	public class CharacterCreateRequestHandler : IPacketHandler
	{
				...
				if (pdata.CustomMode != 3 && client.Version >= GameClient.eClientVersion.Version189)
				{
					...
					if (flagChangedStats)
					{
						...
						if (charClass != null)
						{
							...
							//log.ErrorFormat("CustomPointsValid? {0}", valid.ToString());
							// Hacking attemp ?
				...
				foreach (DOLCharacters character in allChars.ToArray())
				{
					if (character.AccountSlot == charSlot && client.ClientState == GameClient.eClientState.CharScreen)
					{
						...
						else
						{
							...
							try
							{
								IList<InventoryItem> objs = GameServer.Database.SelectObjects<InventoryItem>("`OwnerID` = @OwnerID", new QueryParameter("@OwnerID", character.ObjectId));
							...
							try
							{
								IList<DBQuest> objs = GameServer.Database.SelectObjects<DBQuest>("`Character_ID` = @Character_ID", new QueryParameter("@Character_ID", character.ObjectId));
								GameServer.Database.DeleteObject(objs);
							}
							catch (Exception e)
							{
							...
							try
							{
								IList<DBCharacterXMasterLevel> objs = GameServer.Database.SelectObjects<DBCharacterXMasterLevel>("`Character_ID` = @Character_ID", new QueryParameter("@Character_ID", character.ObjectId));
								GameServer.Database.DeleteObject(objs);
							}
							catch (Exception e)
							{
		...
		public static bool IsCharacterValid(DOLCharacters ch)
		{
			...
			try
			{
				...
				*/
				...
				int pointsUsed;
				var stats = new Dictionary<eStat, int>{{eStat.STR, ch.Strength},{eStat.CON, ch.Constitution},{eStat.DEX, ch.Dexterity},{eStat.QUI, ch.Quickness}, {eStat.INT, ch.Intelligence},{eStat.PIE, ch.Piety},{eStat.EMP, ch.Empathy},{eStat.CHR, ch.Charisma},};
		...
		private void SetBasicCraftingForNewCharacter(DOLCharacters ch)
		{
			...
			foreach (int craftingSkillId in Enum.GetValues(typeof(eCraftingSkill)))
			{
				...
				{
					serializedAllCraftingSkills += craftingSkillId + "|1;";
