...
{
    [PacketHandler(PacketHandlerType.TCP, eClientPackets.UseSkill, eClientStatus.PlayerInGame)]
	public class UseSkillHandler : IPacketHandler
	{
		...
		protected class UseSkillAction : RegionAction
		{
			...
			protected override void OnTick()
			{
				...
				{
					IList specs = player.GetSpecList().ToArray();
				...
				if (sk != null)
				{
					...
					if (reuseTime > 60000)
					{
                        ((GamePlayer)player).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)player).Client.Account.Language, "UseSkillHandler.SendCastMessage.YouMustWait", reuseTime / 60000, reuseTime % 60000 / 1000), eChatType.CT_System, eChatLoc.CL_SystemWindow);
					...
					else if (reuseTime > 0)
					{
                        ...
                        ((GamePlayer)player).Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)player).Client.Account.Language, "UseSkillHandler.SendCastMessage.YouMustWaitMore", reuseTime / 1000 + 1), eChatType.CT_System, eChatLoc.CL_SystemWindow);
