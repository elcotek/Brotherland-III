                ...
                case 2:
                    {
                        ...
                        if (client.Account.PrivLevel == (int)ePrivLevel.Player)
                        {
                            ...
                            if (client.Player != null)
                            {
                                ...
                                long currentTimerSec = 0;
                                long UPDATETICK = client.Player.TempProperties.getProperty<long>(SpamTime);
                                long changeTime = client.Player.CurrentRegion.Time - UPDATETICK;

                                if (changeTime > 19 * 1000 || UPDATETICK == 0)
                                {
                                    client.Player.TempProperties.setProperty(SpamTime, client.Player.CurrentRegion.Time);

                                }
                                else
                                {
                                    //counter 20 seconds.
                                    currentTimerSec = ((UPDATETICK + 19 * 1000) - (client.Player.CurrentRegion.Time)) / 1000;

                                    (client.Player as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((client.Player as GamePlayer).Client.Account.Language, "Keeps.FrontierTeleortertStone", currentTimerSec), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                    return;
                                }
                            }
                        }
                        ...
                        if (x != 0)
                        {
                            client.Player.MoveTo((ushort)163, x, y, z, (ushort)heading);
