...
namespace DOL.GS.PacketHandler.Client.v168
{
    ...
    public class SiegeWeaponActionHandler : IPacketHandler
    {
        public void HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            {
                client.Out.SendMessage("You can't control a siege weapon while sitting!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            if (!client.Player.IsAlive || client.Player.IsMezzed || client.Player.IsStunned || client.Player.IsCasting)
            {
                client.Out.SendMessage("You can't control a siege weapon yet!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            switch (action)
            {
                ...
                case 8:
                    {
                        if (client.Player.SiegeWeapon != null && client.Player.SiegeWeapon.IsMoving)
                        {
                            client.Player.SiegeWeapon.StopMove();
                        }
                        else if (client.Player.SiegeWeapon != null)
                        {
                            client.Player.SiegeWeapon.ReleaseControl();
                            (client.Player as GamePlayer).DismountSteed(true);
                        }
                    }
                    break;//release
