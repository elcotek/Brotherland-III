    ...
    public class BloodDrinkingEffect : TimedEffect
    {
        ...
        protected void OnAttack(DOLEvent e, object sender, EventArgs arguments)
        {
            ...
            if (healAbsorbed > 0)
            {
                if (living.Health < living.MaxHealth)
                {
                    ...
                    {
                        (living as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((living as GamePlayer).Client.Account.Language, "Blooddrinking.ability.IshealingYouFor", healAbsorbed), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                    }
                    foreach (GamePlayer p in EffectOwner.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                    {
                        //heal effect
                ...
                if (living is GamePlayer)
                {
                    (living as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((living as GamePlayer).Client.Account.Language, "Blooddrinking.ability.YouAreAlreadyFullyHealed"), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                }
        ...
        public override IList<string> DelveInfo(GamePlayer player)
        {
            ...
            { //"Cause the Shadowblade to be healed for 20% of all damage he does for 30 seconds"
                ...
                list.Add(LanguageMgr.GetTranslation((player as GamePlayer).Client.Account.Language, "Blooddrinking.ability.Info"));
                return list;
            }
        }
    }
