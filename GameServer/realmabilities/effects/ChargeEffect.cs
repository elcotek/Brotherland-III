...
namespace DOL.GS.Effects
{
	...
	public class ChargeEffect : StaticEffect, IGameEffect
	{
        ...
        public override void Start(GameLiving living)
        {
            m_living = living;
            //Send messages
            if (m_living is GamePlayer && (m_living as GamePlayer).IsOnHorse)
            {
                ((GamePlayer)m_living).Out.SendMessage("you can't this use on a horse!", eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
            return;
            }
               if (m_living is GamePlayer && (m_living as GamePlayer).IsRiding)
                {
                    ((GamePlayer)m_living).Out.SendMessage("you can't this use on a horse!", eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                    return;
            }
            ...
            lock (living.EffectList)
            {
                ...
                {
                    SpeedOfSoundEffect effect1 = living.EffectList.GetOfType<SpeedOfSoundEffect>();
                    if (effect1 != null )
                        effect1.Cancel(false);
                    {

                        if (effect is GameSpellEffect == false) continue;
                        if ((effect as GameSpellEffect).Spell.SpellType == "SpeedEnhancement" || (effect as GameSpellEffect).Spell.SpellType == "SpeedOfTheRealm")
