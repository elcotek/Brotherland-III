	...
	public class DivineInterventionEffect : TimedEffect, IGameEffect
	{
		...
		public override void Start(GameLiving living)
		{
			...
			foreach (GamePlayer gp in m_group.GetPlayersInTheGroup())
			{
                ...
                if (gp.IsCharcterClass(eCharacterClass.Necromancer))
                {
		...
		protected void PlayerJoinedGroup(DOLEvent e, object sender, EventArgs args)
		{
            ...
            if (pjargs.Member is GamePlayer)
            {
                ...
                if (((GamePlayer)pjargs.Member).IsCharcterClass(eCharacterClass.Necromancer))
                {
		...
		protected void PlayerDisbandedGroup(DOLEvent e, object sender, EventArgs args)
		{
            ...
            if (pdargs.Member is GamePlayer)
            {
                ...
                if (((GamePlayer)pdargs.Member).IsCharcterClass(eCharacterClass.Necromancer))
                {
		...
		public override IList<string> DelveInfo(GamePlayer player)
        {
			
			{
				...
				delveInfoList.Add(" ");
				delveInfoList.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "DelveInfo.Target", "Group"));
                delveInfoList.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "DelveInfo.Duration") + " 15:00 min");
                delveInfoList.Add(" ");
				delveInfoList.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "DelveInfo.Value", PoolValue));
