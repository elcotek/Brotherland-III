	...
	public class VolcanicPillarAbility : TimedRealmAbility
	{
		...
		protected virtual int EndCast(RegionTimer timer)
		{
			...
			foreach (GamePlayer aeplayer in caster.TargetObject.GetPlayersInRadius(500))
			{
                ...
                if (aeplayer is GamePlayer)
                {
                    (aeplayer as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((aeplayer as GamePlayer).Client.Account.Language, "ArrowSpellHandler.HitYouForDamage", caster.Name, dmgValue), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
