...
namespace DOL.GS.RealmAbilities
{
    ...
    public class PurgeAbility : TimedRealmAbility
    {
        ...
        protected static bool RemoveNegativeEffects(GameLiving living, PurgeAbility purge)
        {
            ...
            if (player.IsCharcterClass(eCharacterClass.Necromancer))
            {
                ...
                lock (necroPet.EffectList)
                {
                    ...
                    foreach (IGameEffect effect in necroPet.EffectList)
                    {
                        ...
                        //Static effects like TauntEffect
            ...
            lock (living.EffectList)
            {
                foreach (IGameEffect effect in living.EffectList)
                {
                    ...
                    if (gsp.SpellHandler.HasPositiveEffect)//only enemy spells are affected
                        continue;
                    if (gsp.SpellHandler is Port.PhaseshiftHandler)//ignore phaseshift
                       continue;
        ...
        {
            list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "PurgeAbility.AddEffectsInfo.Info1"));
            list.Add("");
            list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "PurgeAbility.AddEffectsInfo.Info2"));
            list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "PurgeAbility.AddEffectsInfo.Info3"));
