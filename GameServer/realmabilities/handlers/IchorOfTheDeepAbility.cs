...
namespace DOL.GS.RealmAbilities
{
	public class IchorOfTheDeepAbility : TimedRealmAbility
	{
		...
		public override void Execute(GameLiving living)
		{
            ...
            if (caster.IsMoving)
            {
                caster.Out.SendMessage("You need to standing still for this ability!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                caster.DisableSkill(this, 3 * 1000);
                return;
            }
		...
		protected virtual int EndCast(RegionTimer timer)
		{
			...
			foreach (GameNPC mob in living.GetNPCsInRadius(500))
			{
				...
				if (mob.HasAbility(Abilities.CCImmunity) || mob.HasAbility(Abilities.RootImmunity) || mob.HasAbility(Abilities.DamageImmunity))
                  ...
                  GameSpellEffect aoemez = SpellHandler.FindEffectOnTarget(mob, "AOEMesmerize");
                if (aoemez != null)
                    aoemez.Cancel(false);
                ...
                if (caster is GamePlayer)
                {
                    ...
                    (caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((caster as GamePlayer).Client.Account.Language, "ArrowSpellHandler.YouHitTheForDamage", mob.Name, dmgValue), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
                }
            ...
            foreach (GamePlayer aeplayer in living.GetPlayersInRadius(500))
            {
                if (!GameServer.ServerRules.IsAllowedToAttack(caster, aeplayer, true))
                    continue;
                GameSpellEffect aoemez = SpellHandler.FindEffectOnTarget(aeplayer, "AOEMesmerize");
                if (aoemez != null)
                    aoemez.Cancel(false);
                ...
                if (caster is GamePlayer)
                {
                    ...
                    (caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((caster as GamePlayer).Client.Account.Language, "ArrowSpellHandler.YouHitTheForDamage", aeplayer.Name, dmgValue), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
                }
		...
		protected static void SendUpdates(GameLiving owner)
		{
            ...
            if (owner.IsMezzed || owner.IsStunned || owner == null || owner.IsAlive == false)
				return;
