			...
			lock (living.EffectList)
			{
				foreach (IGameEffect effect in living.EffectList)
				{
                    ...
                    if (effect is GameSpellEffect)
                    {
						...
						if (SpellHandler.FindEffectAndImmunityOnTarget(living, "speeddecrease") == null && oEffect.Spell.SpellType.ToLower().IndexOf("speeddecrease") != -1 && oEffect.Spell.Value != 99)
						{
                        ...
                        if (SpellHandler.FindEffectAndImmunityOnTarget(living, "DamageSpeedDecrease") == null && oEffect.Spell.SpellType.ToLower().IndexOf("DamageSpeedDecrease") != -1 && oEffect.Spell.Value != 99)
                        {
                            GamePlayer player = living as GamePlayer;
                            if (player != null) player.Out.SendMessage("You may not use this ability while snared!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            return true;
