...
namespace DOL.AI.Brain
{
    ...
    public class DracoBrain : StandardMobBrain, IOldAggressiveBrain
    {
        /// <summary>
        ...
        public override void AddToAggroList(GameLiving living, int aggroamount, bool LosCheck)
        {
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                { // player is in group, add whole group to aggro list
                    lock ((m_aggroTable as ICollection).SyncRoot)
                    {
                        foreach (GamePlayer p in Body.GetPlayersInRadius(2500))
                        {
        ...
        public override void Notify(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (sender == Body)
            {
                ...
                if (e == GameObjectEvent.TakeDamage)
                {
                    ...
                    if (eArgs.DamageSource is GamePlayer)
                    {
                        if (dragon.IsWithinRadius(eArgs.DamageSource, dragon.AttackRange))
                        {
                            dragon.CheckMeleeDebuff(eArgs.DamageSource as GamePlayer);
                        }
                        else
                        {
                            dragon.CheckGlare(eArgs.DamageSource as GamePlayer);
                        }
                    }
                ...
                if (e == GameLivingEvent.EnemyHealed)
                {
                    ...
                    if (eArgs != null && eArgs.HealSource is GameLiving)
                    {
                        ...
                        if (eArgs != null && eArgs.HealSource is GamePlayer)
                        {
                            GamePlayer player = eArgs.HealSource as GamePlayer;

                            if (dragon.IsWithinRadius(player, dragon.AttackRange))
                            {
                                dragon.CheckRangedDebuff(player as GamePlayer);
                            }
                            else
                            {
                                dragon.CheckGlare(player as GamePlayer);
                            }
                        }
        ...
        protected override void BringCloseFriends(AttackData attackData)
        {
            ...
            {
                if (npc.Brain is DracoBrain)
                {
                    brain = (DracoBrain)npc.Brain;
                }
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            DracoBrain brain = (DracoBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
...
&& Body.GetDistanceTo(Body.ControlledBrain.Body) <= spell.Range && spell.Target.ToLower() != "self")
        ...
        private bool CheckHealth()
        {
            ...
            if (healthNow < 30 && Body.Name == "Draco")
            {
        ...
        private bool PickThrowTarget()
        {
            ...
            if (inRangeLiving.Count > 8)
            {
                return dragon.CheckThrow((GameLiving)(inRangeLiving[Util.Random(2, inRangeLiving.Count) - 1]));
            }
