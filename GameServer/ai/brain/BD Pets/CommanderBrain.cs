    ...
    public class CommanderBrain : ControlledNpcBrain
    {
        ...
        private void SubpetFollow()
        {
            if (Body.ControlledNpcList != null)
            {
                ...
                {
                    foreach (BDPetBrain icb in Body.ControlledNpcList)
                        if (icb != null && (icb as BDPetBrain).Body.IsAlive)
                        {
                            if (icb is BDHealerBrain)
                            {
                                (icb as BDHealerBrain).Body.StopCurrentSpellcast();


                                if ((icb as BDHealerBrain).Body.SpellTimer != null && Body.SpellTimer.IsAlive)
                                {
                                    (icb as BDHealerBrain).Body.SpellTimer.Stop();
                                }
                            }
