...
{
    public class TheProtector : GameNPC
    {

        public override bool AddToWorld()
        {
            this.SetOwnBrain(new ProtectorBrain());
            Name = "Protector of the Day";
            Model = 908;
                    ...
                    if ((player is GamePlayer && IsWorthReward) && (player.Group != null))
                    {
                        foreach (GamePlayer players in player.Group.GetPlayersInTheGroup())
                        {
                            ...
                            switch (players.Realm)
                            {
                                case eRealm.Albion:
                                    {
                                        if (players.CurrentRegion.IsNightTime)
                                        {
                                            players.ReceiveItem(this, "belt_of_the_moon_book", 0);
                                            players.BountyPointsAndMassage(20);
                                            players.UpdatePlayerStatus();

                                        }
                                        else if (!players.CurrentRegion.IsNightTime)
                                        {
                                            players.ReceiveItem(this, "scholars_Notes_book", 0);
                                            players.BountyPointsAndMassage(20);
                                            players.UpdatePlayerStatus();
                            ...
                            case eRealm.Albion:
                                {
                                    ...
                                    {
                                        player.ReceiveItem(this, "belt_of_the_moon_book", 0);
                                        player.BountyPoints += 20;
                                        player.UpdatePlayerStatus();
                                        player.Out.SendUpdatePoints();
                                    }
                                    else if (!player.CurrentRegion.IsNightTime)
                                    {
                                        player.ReceiveItem(this, "scholars_Notes_book", 0);
                                        player.BountyPoints += 20;
                                        player.UpdatePlayerStatus();
                                        player.Out.SendUpdatePoints();
                                    }
        ...
        {
            return base.AttackDamage(weapon) * ServerProperties.Properties.SET_DIFFICULTY_ON_LOWLEVEL_ARTIFACT_ENCOUNTERS;
        }
...
{
    public class ProtectorBrain : StandardMobBrain, IOldAggressiveBrain
    {
        ...
        {
            //int RandName = Util.Random(1, 2);
            if (!Body.CurrentRegion.IsNightTime && Body.Model != 908)
            {
                Body.Name = "Protector of the Day";
                Body.Model = 908;
                Body.BroadcastUpdate();
            }
            if (Body.CurrentRegion.IsNightTime && Body.Model != 907)
            {
                Body.Name = "Protector of the Night";
                Body.Model = 907;
                Body.BroadcastUpdate();
            }
            //Satyr:
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.ProtectorBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        protected override void BringCloseFriends(AttackData attackData)
        {
            ...
            {
                if (npc.Brain is ProtectorBrain)
                {
                    brain = (ProtectorBrain)npc.Brain;
                }
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            ProtectorBrain brain = (ProtectorBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
        ...
        public override void NpcYell(GameNPC attackedNPC, GameLiving attacker)
        {
            ...
            if (attackedNPC == null || attacker == null || attackedNPC.Brain is ProtectorBrain == false)
                return;
            ...
            if (attacker != null && attackedNPC.InCombat)
            {
                ...
                foreach (GameLiving adds in attackedNPC.GetNPCsInRadius((ushort)attackedNPC.YellRange))
                {
                    ...
                    {
                        if (attackedNPC.Brain is ProtectorBrain && chanceToYellBoss && add != null && add.IsAlive && add.ObjectState == GameObject.eObjectState.Active && add.IsAvailable && add != attackedNPC && attackedNPC.BodyType == 36 && add.BodyType == 37)
                        {
                            npcFriend = add;
