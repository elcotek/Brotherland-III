...
{
    public class Sagittarius : GameNPC
    {

        public override bool AddToWorld()
        {

            this.SetOwnBrain(new SagittariusBrain());
            Name = "Sagittarius";
                ...
                if (player != null)
                {
                        ...
                        if (battleGroup != null && GS.ServerProperties.Properties.BATTLEGROUP_LOOT)
                        {
                            ...
                            foreach (GamePlayer players in battleGroup.Members.Keys)
                            {
                                if (players.IsWithinRadius(killer, 8000) == false)
                                    return;
                                players.BountyPointsAndMassage(65);
                            }
                        }
                        else
                        {
                    ...
                    if ((player is GamePlayer && IsWorthReward) && (player.Group != null))
                    {
                        foreach (GamePlayer players in player.Group.GetPlayersInTheGroup())
                        {
                            ...
                            //players.Out.SendMessage("You have gained Masterlevel 9.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                            //  players.ReceiveItem(this, "ml9token");
                            players.BountyPointsAndMassage(65);
                    ...
                    {
                        //player.Out.SendMessage("You have gained Masterlevel 9.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                        //   player.ReceiveItem(this, "ml9token");
                        player.BountyPointsAndMassage(65);
...
{
    public class SagittariusBrain : APlayerVicinityBrain, IOldAggressiveBrain
    {
        public const int desease = 5460;
        public const int pbaoeheal = 1667;
        public const int mez = 1686;
        public const int snare = 900021;
        public const int AEDot = 3474;
        public const int randombuffshear = 32025;
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.SagittariusBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            if (Body.TargetObject != null)
            {
                if (!CheckSpells(eCheckSpellType.Offensive))
                {
                    ...
                    foreach (GamePlayer player in Body.GetPlayersInRadius((ushort)AggroRange))
                    {
                        ...
                        if (Body.InCombat)
                        {
                            int RandSpell = Util.Random(1, 10);
                            ...
                            if (RandSpell == 4)
                            {
                                ...
                                Body.CastSpell(SkillBase.GetSpellByID(AEDot), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                            }
                            if (RandSpell == 6 && Body.TargetObject != null && Body.TargetObject is GamePlayer && (Body.TargetObject as GamePlayer).HealthPercent < 60)
                            {
                                //PBAOE Heal (Clerics "Banish Evil" Spell)
                                Body.CastSpell(SkillBase.GetSpellByID(desease), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                                //End Tap (Vampiirs "Exhausting Embrace" Spell)
                               // Body.CastSpell(SkillBase.GetSpellByID(endtap), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                            }
                            if (RandSpell == 7)
                            {
                                ...
                                Body.CastSpell(SkillBase.GetSpellByID(randombuffshear), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            SagittariusBrain brain = (SagittariusBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
