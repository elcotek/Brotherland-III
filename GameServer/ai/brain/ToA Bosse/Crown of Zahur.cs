...
{
    public class Zahur : GameNPC
    {

        public override bool AddToWorld()
        {

            this.SetOwnBrain(new ZahurBrain());
            Name = "Zahur";
                ...
                if (player != null)
                {
                        ...
                        if (battleGroup != null && GS.ServerProperties.Properties.BATTLEGROUP_LOOT)
                        {
                            ...
                            foreach (GamePlayer players in battleGroup.Members.Keys)
                            {
                                if (players.IsWithinRadius(killer, 8000) == false)
                                    continue;
                                players.ReceiveItem(this, "crown_of_zahur_book", 0);
                    ...
                    else
                    {
                        if (player.IsWithinRadius(killer, 8000) == false)
                            return;
                        player.ReceiveItem(this, "crown_of_zahur_book", 0);
...
{
    public class ZahurBrain : StandardMobBrain, IOldAggressiveBrain
    {
        ...
        public override void Think()
        {
            ...
            if (Body.MaxSpeedBase > 0 && Body.CurrentSpellHandler == null && !Body.IsMoving
                ...
                && Body.PathID != null && Body.PathID != String.Empty && String.IsNullOrEmpty(Body.PathID) == false)
            {
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.ZahurBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    ...
                    foreach (GamePlayer player in Body.GetPlayersInRadius((ushort)AggroRange))
                    {
                        ...
                        if (Body.InCombat)
                        {
                            ...
                            if (RandSpell == 2)
                            {
                                ...
                                //PBAOE Mez (Clerics "Theophany" Spell) 1686
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            ZahurBrain brain = (ZahurBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
        ...
        public override void NpcYell(GameNPC attackedNPC, GameLiving attacker)
        {
            ...
            if (attackedNPC == null || attacker == null || attackedNPC.Brain is ZahurBrain == false)
                return;
            ...
            if (attacker != null && attackedNPC.InCombat)
            {
                ...
                {
                    if (((GameNPC)adds) is GameKeepGuard || ((GameNPC)adds).Brain is StandardMobBrain == false || adds.Realm != attackedNPC.Realm || ((GameNPC)adds).Brain is GuardBrain)
                        continue;
                    ...
                    {
                        if (attackedNPC.Brain is ZahurBrain && chanceToYellBoss && add != null && add.IsAlive && add.ObjectState == GameObject.eObjectState.Active && add.IsAvailable && add != attackedNPC && attackedNPC.BodyType == 36 && add.BodyType == 37)
                        {
                            npcFriend = add;
