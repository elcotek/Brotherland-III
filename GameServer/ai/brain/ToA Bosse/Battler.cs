...
{
    public class Battler : GameNPC
    {
        public int Chance_sup = 0;
        public GameLiving Target = null; //1.3.0
        public bool HasParried = false;
        public bool HasBlocked = false;
        public bool HasEvaded = false;
        public Style Slam = null;
        public Style FrontStyle = null;
        public Style BackStyle = null;
        public Style SideStyle = null;
        public Style AnytimeStyle = null;
        public Style ParryStyle = null;
        public Style EvadeStyle = null;
        public Style BlockStyle = null;
        private bool useSlam = false;
        ...
        public override bool AddToWorld()
        {
            ...
            this.SetOwnBrain(new BattlerBrain());
            Name = "Battler";
            ...
            /************INVENTORY ITEM TEMPLATE. SO MOB DOESNT LOOK RETARDED USING STYLES*************/
            GameNpcInventoryTemplate template = new GameNpcInventoryTemplate();
            template.AddNPCEquipment(eInventorySlot.LeftHandWeapon, 2218); //Volcanus Small Shield 
            template.AddNPCEquipment(eInventorySlot.RightHandWeapon, 865); //Spiked Whip    
            Inventory = template.CloseTemplate();
            this.SwitchWeapon(GameLiving.eActiveWeaponSlot.Standard);
        ...
        {
            Slam = GetStyle("Slam");
            AnytimeStyle = GetStyle("Amethyst Slash");
            ParryStyle = GetStyle("Amethyst Slash");
            BlockStyle = GetStyle("Bash");
            SideStyle = GetStyle("Side Slicer");
            FrontStyle = GetStyle("Splitter");
        }
        protected override Style GetStyleToUse()
        {
            ...
            {
                float angle = (this.TargetObject as GameLiving).GetAngle(this);

                if ((angle >= 315 || angle < 45) && FrontStyle != null) //front 
                {
                    if (Util.Chance(85))
                        return Slam;
                    else
                ...
                if (((angle >= 45 && angle < 150) || (angle >= 210 && angle < 315)) && SideStyle != null) //side 
                    return SideStyle;
                if ((angle >= 150 && angle < 210) && BackStyle != null) //back 
                    return BackStyle;
                else //anytime 
                    return AnytimeStyle;
            }
        ...
        {
            eAttackResult result = eAttackResult.HitUnstyled;
            if ((IsMezzed) || (IsStunned))
            {
                result = eAttackResult.HitUnstyled;
                return result;
            }

            int Parry = (Level / 5) + Chance_sup; //11
            int Block = (int)(Level / 2.5) + Chance_sup; //22
            int Evade = (Level / 10) + 30 + Chance_sup; // 25.5

            if (Parry > 25) Parry = 25;
            if (Block > 50) Block = 50;
            if (Evade > 50) Evade = 50;

            if (this.IsObjectInFront(ad.Attacker, 180))
            {
                if (Parry > Util.Random(0, 99) && (CanParry))
                {
                    result = eAttackResult.Parried;
                    HasParried = true;
                }
                else if (Block > Util.Random(0, 99) && CanBlock)
                {
                    result = eAttackResult.Blocked;
                    HasBlocked = true;
                }
                else if (Evade > (Util.Random(0, 99)) && (CanEvade))
                {
                    result = eAttackResult.Evaded;
                    HasEvaded = true;
                }
                else
                {
            ...
            missrate = 0.25 + 0.05 * ad.Attacker.GetConLevel(this);

            if (Util.Chance(Convert.ToInt16(missrate)))
            {
                result = eAttackResult.Missed;
            }
                ...
                if (player != null)
                {
                        ...
                        if (battleGroup != null && GS.ServerProperties.Properties.BATTLEGROUP_LOOT)
                        {
                            ...
                            foreach (GamePlayer players in battleGroup.Members.Keys)
                            {
                                if (players.IsWithinRadius(killer, 8000) == false)
                                    continue;
                                players.ReceiveItem(this, "bane_of_battler_book", 0);
                    ...
                    else
                    {
                        if (player.IsWithinRadius(killer, 8000) == false)
                            return;
                        player.ReceiveItem(this, "bane_of_battler_book", 0);
        ...
        GameLiving m_BattlerTarget;
        public GameLiving BattlerTarget
        {
            get
            {
                return m_BattlerTarget;
            }
            set
            {
                m_BattlerTarget = value;
            }
        }
...
{
    public class BattlerBrain : StandardMobBrain, IOldAggressiveBrain
    {
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.BattlerBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            if (Body.TargetObject != null)
            {
                if (!CheckSpells(eCheckSpellType.Offensive))
                {
                    ...
                    foreach (GamePlayer player in Body.GetPlayersInRadius((ushort)AggroRange))
                    {
                        byte grow = (int)4;
                        byte lvl = (int)1;
                        if (Body.Level <= 74 && player != null && player.IsAlive == false)
                        {
                            Body.Level = Body.Level += lvl;
                            Body.Size = Body.Size += grow;
                        }
                        /*
                        if (!Body.InCombat && Body.Level != 65)
                        {
                            Body.Level = 65;
                            Body.Size = 50;
                        }
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            BattlerBrain brain = (BattlerBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
        ...
        public override void NpcYell(GameNPC attackedNPC, GameLiving attacker)
        {
            ...
            if (attackedNPC == null || attacker == null || attackedNPC.Brain is BattlerBrain == false)
                return;
            ...
            if (attacker != null && attackedNPC.InCombat)
            {
                ...
                foreach (GameLiving adds in attackedNPC.GetNPCsInRadius((ushort)attackedNPC.YellRange))
                {
                    ...
                    {
                        if (attackedNPC.Brain is BattlerBrain && chanceToYellBoss && add != null && add.IsAlive && add.ObjectState == GameObject.eObjectState.Active && add.IsAvailable && add != attackedNPC && attackedNPC.BodyType == 36 && add.BodyType == 37)
                        {
                            npcFriend = add;
