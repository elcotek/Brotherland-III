...
namespace DOL.GS
{
    public class Absorbacrom : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            this.SetOwnBrain(new AbsorbacromBrain());
            Name = "Absorbacrom";
        ...
        GameLiving m_AbsorbacromTarget;
        public GameLiving AbsorbacromTarget
        {
            get
            {
                return m_AbsorbacromTarget;
            }
            set
            {
                m_AbsorbacromTarget = value;
            }
        }
...
{
    public class AbsorbacromBrain : StandardMobBrain, IOldAggressiveBrain
    {

        public const int DieingSpell = 8626;
        const string addSpawn1 = "AddDpawned1_spawned";
        const string addSpawn2 = "AddDpawned2_spawned";
        ...
        /// </summary>
        public AbsorbacromBrain()
            : base()
        {
            base.m_aggroLevel = 65;
            base.AggroRange = 1500;
            base.m_aggroMaxRange = 2000;
        }
        ...
        public override void Think()
        {
            ...
            if (HasAggro == false && !Body.InCombat && Body.Constitution != 400)
            {

                Body.Health = Body.MaxHealth;
                Body.Constitution = 400;
                Body.Size = 130;
                Body.TempProperties.removeProperty(addSpawn1);
                Body.TempProperties.removeProperty(addSpawn2);
            }
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.AbsorbacromBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            if (Body.TargetObject != null)
            {
                if (!CheckSpells(eCheckSpellType.Offensive))
                {
                    ...
                    foreach (GamePlayer player in Body.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                    {
                        ...
                        {
                            Body.Constitution = Body.Constitution += 200;
                    ...
                    if (Body.InCombat)
                    {
                        ...
                        if (Body.Size >= 50)
                        {
                            Body.Constitution = Body.Constitution += con;
                            Body.Size = Body.Size -= grow;
                            Body.BroadcastUpdate();

                        }
                        /*
                        if (Body.Constitution <= 8000)
                        {
                            Body.Constitution = Body.Constitution += con;
                            Body.BroadcastUpdate();
                        ...
                        */
                        if (Body.HealthPercent <= 5 && Body.IsAlive)
                        {
                            Body.CastSpell(SkillBase.GetSpellByID(DieingSpell), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                        }

                        if (Body.TempProperties.getProperty<bool>(addSpawn1) == false && Body.Size <= 90)
                        {
                            //Adds unten
                            ADDSpawnAdd(79999, 53, 41625, 67825, 29481, 3308, 245, 0, -1);
                            ADDSpawnAdd(79999, 53, 42369, 67071, 29481, 3563, 245, 0, -1);
                            ADDSpawnAdd(79999, 53, 42202, 67746, 29481, 3556, 245, 0, -1);
                            ADDSpawnAdd(79999, 53, 42216, 67396, 29481, 3577, 245, 0, -1);
                            ADDSpawnAdd(79999, 53, 41787, 67822, 29481, 3585, 245, 0, -1);
                            Body.TempProperties.setProperty(addSpawn1, true);


                        }
                        if (Body.TempProperties.getProperty<bool>(addSpawn2) == false && Body.HealthPercent <= 20)
                        {
                            //Adds oben
                            ADDSpawnAdd(79999, 53, 42777, 68701, 29609, 2260, 245, 0, -1);
                            ADDSpawnAdd(79999, 53, 43104, 68245, 29609, 306, 245, 0, -1);
                            ADDSpawnAdd(79999, 53, 43103, 68647, 29609, 1610, 245, 0, -1);
                            ADDSpawnAdd(79999, 53, 42819, 67975, 29597, 1499, 245, 0, -1);
                            ADDSpawnAdd(79999, 53, 68416, 29597, 29597, 1499, 245, 0, -1);
                            Body.TempProperties.setProperty(addSpawn2, true);
                        }
                    }
                }
            }
        ...
        protected GameNPC ADDSpawnAdd(int templateID, int level, int x, int y, int Z, ushort heading, ushort region, int roamingrange, int respawnInterval)
        {
            ...
            try
            {
                ...
                if (m_addMobTemplate != null)
                {
                    ...
                    add.AddToWorld();
                    new DespawnTimers(add, add, 60 * 1000);
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            AbsorbacromBrain brain = (AbsorbacromBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
        ...
        public override void NpcYell(GameNPC attackedNPC, GameLiving attacker)
        {
            ...
            GameNPC npcFriend;
            GamePlayer attackOwner;
            attackedNPC = Body;

            //only npcs allow to add
            if (attackedNPC == null || attacker == null || attackedNPC.Brain is AbsorbacromBrain == false)
                return;
            ...
            if (attacker != null && attackedNPC.InCombat)
            {
                ...
                foreach (GameLiving adds in attackedNPC.GetNPCsInRadius((ushort)attackedNPC.YellRange))
                {
                    ...
                    {
                        if (attackedNPC.Brain is AbsorbacromBrain && chanceToYellBoss && add != null && add.IsAlive && add.ObjectState == GameObject.eObjectState.Active && add.IsAvailable && add != attackedNPC && attackedNPC.BodyType == 36 && add.BodyType == 37)
                        {
                            npcFriend = add;
