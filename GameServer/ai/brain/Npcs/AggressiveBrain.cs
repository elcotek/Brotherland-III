...
namespace DOL.AI.Brain
{
    ...
    public class AggressiveBrain : APlayerVicinityBrain
    {
        ...
        private readonly object m_syncBehaviour = new object();
        ...
        protected virtual void OnEnemyHealed(GameObject source, int amount)
        {
            ...
            if (source is GameLiving && amount > 1)
                Aggression.Raise(source as GameLiving, amount / 3);
        ...
        protected virtual void OnLowHealth(GameLiving living)
        {
            ...
            if (living == Body)
            {
                foreach (GamePlayer player in Body.GetPlayersInRadius(500))
                    ...
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.IsLowOnHealth", Body.Name), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                //player.Out.SendMessage(String.Format("{0} is low on health!",  Body.Name), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
            }
        }
        ...
        {
            private readonly IDictionary<GameLiving, long> m_aggression = new Dictionary<GameLiving, long>();
            ...
            public void Raise(GameLiving living, long amount)
            {
                ...
                lock (m_syncObject)
                {
                    ...
                    else
                    {
                        ...
                        //Pets and Player
                        if (living != null && living is GameNPC && ((GameNPC)living).Brain is ControlledNpcBrain)
                        {
                            brain = ((GameNPC)living).Brain as ControlledNpcBrain;
                        }
                        if (brain != null)
                        {
                            mobAttacker = brain.GetLivingOwner();

                            if (mobAttacker != null && mobAttacker is GamePlayer)
                            {
                                player = mobAttacker as GamePlayer;
                            }


                        }
                        else if (living != null && living is GamePlayer)
                        {
                        ...
                        if (player != null)
                        {
                            ...
                            if (group != null || player.MemberIsInBG(player) && GS.ServerProperties.Properties.BATTLEGROUP_LOOT)
                            {
...

                                {
                                    ...
                                    foreach (GamePlayer players in battleGroup.Members.Keys)
                                        if (!m_aggression.ContainsKey(players))
                                            m_aggression.Add(players, Minimum);
                                }
                                else
                                    foreach (GamePlayer players in group.GetPlayersInTheGroup())
                                        if (!m_aggression.ContainsKey(players))
                                            m_aggression.Add(players, Minimum);
                            }
