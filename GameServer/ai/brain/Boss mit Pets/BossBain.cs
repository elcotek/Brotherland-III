...
namespace DOL.GS
{
    public class BossNPC : GameNPC
    {
        public BossNPC()
            : base()
        {
            m_ownBrain = new BossBrain();
...
namespace DOL.AI.Brain
{
    ...
    public class BossBrain : StandardMobBrain, IOldAggressiveBrain
    {
        ...
        public virtual GameNPC GetNPCOwner()
        {
            ...
            while (owner != null)
            {
                ...
                if (i > 50)
                {
                    log.Error("Keinen Petowner gefunden !");
                    break;
                }
        ...
        public override void Think()
        {
            ...
            {
                if (Body.Formation != GameNPC.eFormationType.Line && Body.BodyType == 1)
                {
                    Body.Formation = GameNPC.eFormationType.Line;
                }
                else if (Body.Formation != GameNPC.eFormationType.Protect && Body.BodyType == 2)
                {
                    Body.Formation = GameNPC.eFormationType.Protect;
                }
                else if (Body.Formation != GameNPC.eFormationType.Triangle && Body.BodyType == 3)
                {
            ...
            if (!Body.AttackState && CanRandomWalk && Util.Chance(GS.ServerProperties.Properties.GAMENPC_RANDOMWALK_CHANCE))
            {
        ...
        */
        ...
        public override void AddToAggroList(GameLiving living, int aggroamount, bool LosCheck)
        {
            ...
            if (GS.ServerProperties.Properties.ALWAYS_CHECK_LOS && LosCheck)
            {
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.BossBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        protected override void OnAttackedByEnemy(AttackData ad)
        {
                ...
                && Body.ObjectState == GameObject.eObjectState.Active)
            {
                if ((Body is GameKeepGuard || Body is GamePet || Body.Brain is ControlledNpcBrain
                      || Body.Brain is MobPetBrain || Body.Brain is CommanderBrain
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            BossBrain brain = (BossBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
        ...
        public override void NpcYell(GameNPC attackedNPC, GameLiving attacker)
        {
            ...
            bool chanceToYellBoss = Util.Chance(Properties.GAMENPC_YELL_CHANCE_BOSS);//chance percent to yell
            bool chanceToAddOnOwner = Util.Chance(Properties.GAMENPC_YELL_Switch_CHANCE);//chance percent to attack pet owners
            GameNPC npcFriend = null;
            GamePlayer attackOwner = null;
            attackedNPC = Body;

            //only npcs allow to add
            if (attackedNPC == null || attacker == null || attackedNPC.Brain is BossBrain == false)
                return;
            ...
            if (attacker != null && attackedNPC.InCombat)
            {
                ...
                foreach (GameLiving adds in attackedNPC.GetNPCsInRadius((ushort)attackedNPC.YellRange))
                {
                    ...
                    {
                        if (attackedNPC.Brain is BossBrain && chanceToYellBoss && add != null && add.IsAlive && add.ObjectState == GameObject.eObjectState.Active && add.IsAvailable && add != attackedNPC && attackedNPC.BodyType == 36 && add.BodyType == 37)
                        {
                            npcFriend = add;
...
&& Body.GetDistanceTo(Body.ControlledBrain.Body) <= spell.Range && spell.Target.ToLower() != "self")
                ...
                case "BossPetSummonMinion":
                    ...
                    if (Body.ControlledCount > 0)
                        petcount = Body.ControlledCount;


                    if (petcount > 0 && Body.ControlledNpcList != null && Body.ControlledNpcList.Length < petcount)
                    {
                        Body.InitControlledBrainArray(petcount);
                    }


                    if (Body.ControlledNpcList == null || petcount > Body.ControlledNpcList.Count())
                    {
                        if (petcount == 0)
                        {
