            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        //protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.ArtefactMobBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                ...
                if (eArgs != null)
                {
                    ...
                    if (eArgs.Target is GameNPC)
                    {
                        ...
                        if (controlled != null)
                        {
                            ...
                            AddToAggroList(controlled.Owner, (int)contrAggro, true);
                        }
                    }
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            ArtefactMobBrain brain = (ArtefactMobBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1, true);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
