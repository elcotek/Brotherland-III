...
namespace DOL.GS
{
    ...
    public class GuildEventHandler
    {
        ...
        public static void RealmPointsGain(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (player.Guild != null)
            {
                if (player.Guild.BonusType == Guild.eBonusType.RealmPoints)
                {
                    ...
                    long bonusRealmPoints = 0;


                    if (player.RealmLevel < 40)
                    {
                        bonusRealmPoints = (long)Math.Ceiling((double)rpsArgs.RealmPoints * ServerProperties.Properties.GUILD_BUFF_RP / 100);
                    }
                    if (player.RealmLevel >= 40)
                    {
                        bonusRealmPoints = (long)Math.Ceiling((double)rpsArgs.RealmPoints * ServerProperties.Properties.LOW_RP_RATE / 100);
                    }
        ...
        {
            foreach (Guild checkGuild in GuildMgr.AllGuilds)
            {
