	...
	public static class RegionTimersResynch
	{
		...
		public static void Init()
		{
			...
			foreach (GameTimer.TimeManager mgr in WorldMgr.GetRegionTimeManagers())
			{
				try
				{
					if (old_time.ContainsKey(mgr) == false)
					{
						old_time.Add(mgr, 0);
					}
				}
				...
				{
                log.Error("Time Manager was alredy in list!");
				}
			}
						...
						if (reg.TimeManager == mgr)
						{
							foreach (GameObject obj in reg.Objects)
							{
								...
								if (obj is GamePlayer)
								{
									...
									if (plr.Client.Account.PrivLevel > 1)
									{
										//log.ErrorFormat("{0} detected as frozen, restarting the zone.", reg.Description);
										// Warn Player
										plr.Client.Out.SendMessage("[" + reg.Description + "] detected as frozen, restarting the zone.", eChatType.CT_Send, eChatLoc.CL_ChatWindow);
									}
								}
