        ...
        public void Update()
        {
            ...
            if (alliances.Any())
            {
                ...
                if (saved && log.IsInfoEnabled)
                    log.Info("GuildAndAllianceUpdate - Finished saving Alliances without Leader successfully!");
                if (!saved && log.IsErrorEnabled)
                    log.Error("GuildAndAllianceUpdate - Could not save all Alliances without Leader, check logs or records...");
            }
