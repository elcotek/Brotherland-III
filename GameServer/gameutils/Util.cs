...
namespace DOL.GS
{
    ...
    public static class Util
    {
        ...
        {
            return RandomGen.Next((int)min, (int)max);
        }
        ...
        public static List<string> SplitCSV(this string str, bool rangeCheck = false)
        {
            ...
            foreach (var currentResultat in resultat)
            {
                if (currentResultat.Contains('-'))
                {
                    ...
                    if (int.TryParse(currentResultat.Split(secondarySeparator)[0], out int from) && int.TryParse(currentResultat.Split(secondarySeparator)[1], out int to))
                    {
                        if (from > to)
                        {
                            int tmp = to;
