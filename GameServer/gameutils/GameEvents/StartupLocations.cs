		...
		public static void CharacterCreation(DOLEvent ev, object sender, EventArgs args)
		{
			...
			try
			{
				...
				if (ServerProperties.Properties.USE_CUSTOM_START_LOCATIONS)
				{
                    ...
                    var startupLocations = GameServer.Database.SelectObjects<StartupLocation>("`ClassIDs` = @ClassIDs", new QueryParameter("@ClassIDs", "ALL"));
					...
					{
						// find realm-based SL
						startupLocations = GameServer.Database.SelectObjects<StartupLocation>("`ClassIDs` LIKE @ClassIDs", new QueryParameter("@ClassIDs", "%" + GlobalConstants.RealmToName((eRealm)ch.Realm).Substring(0, 3) + "%"));
						...
						{
							// find class-based SL
							startupLocations = GameServer.Database.SelectObjects<StartupLocation>("`ClassIDs` LIKE @ClassIDs", new QueryParameter("@ClassIDs", "%" + ch.Class + "%"));
