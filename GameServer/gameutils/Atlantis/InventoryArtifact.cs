...
namespace DOL.GS
{
    ...
    public class InventoryArtifact : GameInventoryItem
	{
		...
		private readonly int[] m_levelRequirements;
		...
		public override void Delve(List<String> delve, GamePlayer player)
		{
			...
			for (ArtifactBonus.ID bonusID = ArtifactBonus.ID.MinStat; bonusID <= ArtifactBonus.ID.MaxStat; ++bonusID)
				DelveMagicalBonus(delve, Template.GetBonusAmount(bonusID), Template.GetBonusType(bonusID), m_levelRequirements[(int)bonusID], player);

			for (ArtifactBonus.ID bonusID = ArtifactBonus.ID.MinStat; bonusID <= ArtifactBonus.ID.MaxStat; ++bonusID)
				DelveFocusBonus(delve, Template.GetBonusAmount(bonusID), Template.GetBonusType(bonusID), m_levelRequirements[(int)bonusID], player);

			delve.Add("");

			for (ArtifactBonus.ID bonusID = ArtifactBonus.ID.MinStat; bonusID <= ArtifactBonus.ID.MaxStat; ++bonusID)
				DelveBonus(delve, Template.GetBonusAmount(bonusID), Template.GetBonusType(bonusID), m_levelRequirements[(int)bonusID], player);
		...
		protected virtual void DelveMagicalBonus(List<String> delve, int bonusAmount, int bonusType, int levelRequirement, GamePlayer player)
		{
			...
			if (IsStatBonus(bonusType) || IsSkillBonus(bonusType))
			{
				delve.Add(String.Format("- {0}{1}: {2} pts",
				                        levelTag,
				                        SkillBase.GetPropertyName((eProperty)bonusType, player.Client.Account.Language),
		...
		protected virtual void DelveFocusBonus(List<String> delve, int bonusAmount, int bonusType, int levelRequirement, GamePlayer player)
		{
			String levelTag = (levelRequirement > 0)
				? String.Format("[L{0}]: ", levelRequirement)
				: "";
		...
		/// <param name="bonusType"></param>
		/// <param name="levelRequirement"></param>
		protected virtual void DelveBonus(List<String> delve, int bonusAmount, int bonusType, int levelRequirement, GamePlayer player)
		{
		...
		protected override void DelveWeaponStats(List<String> delve, GamePlayer player)
		{
            ...
            double clampedDPS = Math.Min(itemDPS, 1.2 + 0.3 + DPSRR5 *player.Level);
