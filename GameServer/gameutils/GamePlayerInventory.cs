...
namespace DOL.GS
{
    ...
    public class GamePlayerInventory : GameLivingInventory
    {
        ...
        public override bool LoadFromDatabase(string inventoryID)
        {
            lock (m_items)
            {
                try
                {
                    ...
                    {
                        // Sicherstellen, dass item nicht null ist
                        if (item == null)
                        {
                            Log.Error("Loaded item is null, skipping...");
                            continue; // Bei null weitermachen
                        }
                        ...
                        try
                        {
                            ...
                            if (m_items.ContainsKey(itemSlot))
                            {
                                if (Log.IsErrorEnabled)
                                {
                                    Log.ErrorFormat("Error loading {0} inventory! Duplicate item {1} found in slot {2}; Skipping!", inventoryID, item.Name, itemSlot);
                                }

                                continue;
                            }

                            // Überprüfen ob es sich um ein Artefakt handelt
                            ...
                            else
                            {
                                ...
                                // Überprüfen ob playerItem nicht null ist
                                if (playerItem == null)
                                {
                                    Log.ErrorFormat("Failed to create GameInventoryItem for item '{0}', skipping!", item.Name);
                                    continue;
                                }
                            ...
                            {
                                // Warnung für Waffen mit Schadenstyp 0
                                if (GlobalConstants.IsWeapon(item.Object_Type)
                                    ...
                                    && item.Object_Type != (int)eObjectType.GenericWeapon)
                                {
                                    Log.Error(m_player.Name + ": weapon with damage type 0 is loaded \"" + item.Name + "\" (" + item.ObjectId + ")");
                                }
                            }
                        }
                        catch (Exception ex)
                    ...
                    // Benachrichtigen, dass das Item gerade ausgerüstet wurde
                    foreach (eInventorySlot slot in EQUIP_SLOTS)
                    {
                        ...
                        if (m_items.TryGetValue(slot, out InventoryItem item) && item != null) // Sicherstellen, dass item nicht null ist
                        {
        ...
        protected bool RemoveItem(InventoryItem item, bool deleteObject)
        {
            ...
            {
                if (Log.IsErrorEnabled)
                    Log.ErrorFormat("PlayerInventory {0} player ID: {1} ->  tried to remove item with wrong owner ID: {2}", m_player.Name, m_player.InternalID, item.OwnerID);
        ...
        public override bool MoveItem(eInventorySlot fromSlot, eInventorySlot toSlot, int itemCount)
        {
            ...
            lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
            {
                ...
                if (valid && toItem != null && toItem.AllowedClasses != "0" && string.IsNullOrEmpty(toItem.AllowedClasses) == false)
                {
                ...
                if (valid)
                {
                    ...
                    switch (toSlot)
                    {
                        ...
                        case eInventorySlot.TwoHandWeapon:
                            {
                                if (fromItem.Level >= 50 && fromItem.LevelRequirement >= 50 && fromItem.LevelRequirement > m_player.Level)
                                {
                                    m_player.Out.SendMessage(string.Format(LanguageMgr.GetTranslation(m_player.Client.Account.Language, "GamePlayer.OnItemEquipped.LevelRequired", fromItem.LevelRequirement)), eChatType.CT_Skill, eChatLoc.CL_SystemWindow);

                                    // m_player.Out.SendMessage("You can't equip the " + fromItem.GetName(0, true) + " , while in combat.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                    valid = false;
                                }
                                break;
                            }
                    ...
                    //Item equip in combat not allowed
                    switch (toSlot)
                    {
                        ...
                        case eInventorySlot.Waist:
                            {
                                if (m_player.InCombat)
                                {
                                    m_player.Out.SendMessage(
                                    "You can't equip the " + fromItem.GetName(0, true) + " , while in combat.",
                    ...
                    switch (toSlot)
                    {
                        ...
                        case eInventorySlot.Mythical:
                            ...
                            if (fromItem.Item_Type == (int)eInventorySlot.Mythical && fromItem.Type_Damage > m_player.ChampionLevel)
                            {
                                valid = false;
                                m_player.Out.SendMessage(
                        ...
                        case eInventorySlot.RightHandWeapon:
                            ...
                            //Artifact Weapon RightHand
                            if (m_player.Inventory.GetItem(eInventorySlot.RightHandWeapon) != null && fromItem.PackageID == "Artifact"
                            && fromItem.Name == m_player.Inventory.GetItem(eInventorySlot.RightHandWeapon).Name)
                            {
                                valid = false;
                                m_player.Out.SendMessage("You can not use same type of Aritifacts!", eChatType.CT_System, eChatLoc.CL_SystemWindow);

                            }
                            if (m_player.Inventory.GetItem(eInventorySlot.LeftHandWeapon) != null && fromItem.PackageID == "Artifact"
                           && fromItem.Name == m_player.Inventory.GetItem(eInventorySlot.LeftHandWeapon).Name)
                            {
                                valid = false;
                                m_player.Out.SendMessage("You can not use same type of Aritifacts!", eChatType.CT_System, eChatLoc.CL_SystemWindow);

                            }
                            if (m_player.Inventory.GetItem(eInventorySlot.TwoHandWeapon) != null && fromItem.PackageID == "Artifact"
                         && fromItem.Name == m_player.Inventory.GetItem(eInventorySlot.TwoHandWeapon).Name)
                            {
                                valid = false;
                                m_player.Out.SendMessage("You can not use same type of Aritifacts!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        ...
                        case eInventorySlot.LeftHandWeapon:
                          ...
                          && fromItem.Name == m_player.Inventory.GetItem(eInventorySlot.TwoHandWeapon).Name)
                            ...
                            if (fromItem.PackageID == "Lefthanditem" == false && fromItem.Item_Type != (int)toSlot ||
                                (fromItem.PackageID == "Lefthanditem" == false && fromItem.Object_Type != (int)eObjectType.Shield && !m_player.CanUseLefthandedWeapon))
                            ...
                            else if (fromItem.PackageID == "Lefthanditem" == false && !m_player.HasAbilityToUseItem(fromItem.Template))
                            {
                                valid = false;
                                m_player.Out.SendMessage("You have no skill in using this weapon type!", eChatType.CT_System,
                                                         eChatLoc.CL_SystemWindow);
                        ...
                        case eInventorySlot.RightBracer:
                            ...
                            //Artifact Bracer
                            if (m_player.Inventory.GetItem(eInventorySlot.LeftBracer) != null && fromItem.PackageID == "Artifact"
                                && fromItem.Name == m_player.Inventory.GetItem(eInventorySlot.LeftBracer).Name)
                            {
                                valid = false;
                                m_player.Out.SendMessage("You can not use same type of Aritifacts!", eChatType.CT_System, eChatLoc.CL_SystemWindow);

                            }
                            if (m_player.Inventory.GetItem(eInventorySlot.RightBracer) != null && fromItem.PackageID == "Artifact"
                           && fromItem.Name == m_player.Inventory.GetItem(eInventorySlot.RightBracer).Name)
                            {
                                valid = false;
                                m_player.Out.SendMessage("You can not use same type of Aritifacts!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            ...
                            if (fromItem.Item_Type != Slot.LEFTWRIST && fromItem.Item_Type != Slot.RIGHTWRIST)
                            {
                                valid = false;
                                m_player.Out.SendMessage(fromItem.GetName(0, true) + " can't go there!", eChatType.CT_System,
                                                         eChatLoc.CL_SystemWindow);
                        ...
                        case eInventorySlot.RightRing:
                            ...
                            //Artifact Ring
                            if (m_player.Inventory.GetItem(eInventorySlot.LeftRing) != null && fromItem.PackageID == "Artifact"
                                && fromItem.Name == m_player.Inventory.GetItem(eInventorySlot.LeftRing).Name)
                            {
                                valid = false;
                                m_player.Out.SendMessage("You can not use same type of Aritifacts!", eChatType.CT_System, eChatLoc.CL_SystemWindow);

                            }
                            if (m_player.Inventory.GetItem(eInventorySlot.RightRing) != null && fromItem.PackageID == "Artifact"
                           && fromItem.Name == m_player.Inventory.GetItem(eInventorySlot.RightRing).Name)
                            {
                                valid = false;
                                m_player.Out.SendMessage("You can not use same type of Aritifacts!", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                ...
                if (valid && toItem != null)
                {
                    switch (fromSlot)
                    {
                        ...
                        case eInventorySlot.Mythical:
                            ...
                            if (toItem.Item_Type == (int)eInventorySlot.Mythical && toItem.Type_Damage > m_player.ChampionLevel)
                            {
                                valid = false;
                                m_player.Out.SendMessage(
        ...
        protected override bool StackItems(eInventorySlot fromSlot, eInventorySlot toSlot, int itemCount)
        {
            ...
            m_items.TryGetValue(fromSlot, out InventoryItem fromItem);
            m_items.TryGetValue(toSlot, out InventoryItem toItem);
        ...
        public override int InventoryWeight
        {
            get
            {
                ...
                lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
                {
                    ...
                    {
                        if (m_items.TryGetValue(slot, out InventoryItem item))
                        {
                            weight += item.Weight;
                        }
                    }
            ...
            foreach (eInventorySlot updatedSlot in m_changedSlots)
            {
                ...
                if (!appearanceUpdated)
                {
                    foreach (eInventorySlot visibleSlot in VISIBLE_SLOTS)
                    {
                        ...
                        m_player.UpdatePlayerEquipment();
