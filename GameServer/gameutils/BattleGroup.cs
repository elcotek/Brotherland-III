...
namespace DOL.GS
{
    ...
    public class BattleGroup
	{
        ...
        {
            if (m_battlegroupMembers.Count > 0)
            {
                foreach (GamePlayer players in Members.Keys)
                    players.Out.SendMessage(msg, type, loc);
            }
        }
		/// <summary>
		...
		public virtual bool AddBattlePlayer(GamePlayer player,bool leader) 
		{
			...
			lock (m_battlegroupMembers)
			{
                ...
                player.IsInBattleGroup = true; //Xarik: Player is in BG
			}
			return true;
		}
		...
		public virtual bool RemoveBattlePlayer(GamePlayer player)
		{
			...
			lock (m_battlegroupMembers)
			{
                ...
                player.IsInBattleGroup = false; //Xarik: Player is no more in the BG
			}
			return true;
		}
	}
