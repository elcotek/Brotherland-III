...
namespace DOL.Mail
{
    ...
    public class GZipCompress
    {
        ...
        public static bool compressFile(string source, string dest)
        {
            try
            {
                ...
                {
                    FileStream fstreamCreate;
                    // We store the complete file into a buffer
                    byte[] buf = new byte[fstream.Length];
                    fstream.Read(buf, 0, buf.Length);
                    fstream.Close();
                    fstreamCreate = fstream;//richtig ??
                    // Stream to write the compressed file
                    using (fstreamCreate = new FileStream(dest, FileMode.Create))
                    {
                        // File compression (fstream is automatically closed)
                        GZipStream zipStream = new GZipStream(fstreamCreate, CompressionMode.Compress, false);
    ...
    public class MailMgr
    {
        ...
        private static readonly int m_mailFrequency = 5 * 60 * 1000;

        private static readonly Queue m_mailQueue = new Queue();
