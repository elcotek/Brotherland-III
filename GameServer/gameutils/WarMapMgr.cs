...
namespace DOL.GS
{
    ...
    {   
        ...
        static readonly Dictionary<long, Fight> m_fights = new Dictionary<long, Fight>();
        static readonly Dictionary<string, Dictionary<long, Group>> m_groups = new Dictionary<string, Dictionary<long, Group>>();
        static readonly Dictionary<byte, Dictionary<byte, Dictionary<byte, Dictionary<byte, int>>>> b_fights = new Dictionary<byte, Dictionary<byte, Dictionary<byte, Dictionary<byte, int>>>>();
        static readonly Dictionary<byte, Dictionary<byte, Dictionary<byte, Dictionary<byte, int>>>> b_groups = new Dictionary<byte, Dictionary<byte, Dictionary<byte, Dictionary<byte, int>>>>();
        static readonly List<List<byte>> w_fights = new List<List<byte>>();
        static readonly List<List<byte>> w_groups = new List<List<byte>>();
        ...
        public static bool AddFight(byte zoneid, int x, int y, byte realm1, byte realm2)
        {
            ...
            {
                Zone = zoneid,
                X = (byte)((x - zone.XOffset) >> 14),
                Y = (byte)((y - zone.YOffset) >> 14),
                Realm1 = realm1,
                Realm2 = realm2
            };
            lock (m_fights)
