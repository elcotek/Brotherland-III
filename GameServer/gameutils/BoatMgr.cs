...
namespace DOL.GS
{
    ...
    public sealed class BoatMgr
    {
        ...
        {
           //if (m_boats.Contains(boat)) return false;
           if (boat == null)
              return false;
        ...
        public static bool DoesBoatExist(string boatName)
        {
            lock (((ICollection)m_boats).SyncRoot)
            {
                if (m_boatids.Contains(boatName))
                    return true;
                return false;
            }
        }
        ...
        public static GameBoat CreateBoat(GamePlayer creator, GameBoat boat)
        {
            ...
            try
            {
                ...
                GameBoat curBoat = BoatMgr.GetBoatByOwner(creator.InternalID);
                if (curBoat != null)
                {
                    if (creator != null)
                ...
                {
                    BoatOwner = creator.InternalID,
                    BoatID = boat.BoatID,
                    BoatMaxSpeedBase = boat.MaxSpeedBase,
                    BoatModel = boat.Model,
                    BoatName = boat.Name
                };
                boat.BoatOwner = creator.InternalID;
