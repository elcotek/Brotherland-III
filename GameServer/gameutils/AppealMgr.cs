...
namespace DOL.GS.Appeal
{
    public static class AppealMgr
	{
		...
		private static readonly int m_CallbackFrequency = 5 * 60 * 1000; // How often appeal stat updates are sent out.
		...
		public static void ChangeStatus(string staffname, GamePlayer target, DBAppeal appeal, string status)
		{
			...
			target.Out.SendMessage("[Appeals]: " + LanguageMgr.GetTranslation(target.Client.Account.Language, "Scripts.Players.Appeal.StaffChangedStatus", staffname, status), eChatType.CT_Important, eChatLoc.CL_ChatWindow);
			return;
		}
