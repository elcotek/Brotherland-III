    ...
    public class LootGeneratorMobTemplate : LootGeneratorBase
	{
			...
			if (m_lootTemplates == null)
			{
				...
				lock (m_lootTemplates)
				{
					...
					if (dbLootTemplates != null)
					{
                        ...
                        {
                            if (!m_lootTemplates.TryGetValue(dbTemplate.TemplateName.ToLower(), out Dictionary<string, DropTemplateXItemTemplate> loot))
                            {
				...
				lock (m_mobXLootTemplates)
				{
					...
					if (dbMobXLootTemplates != null)
					{
						foreach (MobDropTemplate dbMobXTemplate in dbMobXLootTemplates)
						{
                            ...
                            if (!m_mobXLootTemplates.TryGetValue(dbMobXTemplate.MobName.ToLower(), out List<MobDropTemplate> mobxLootTemplates))
                            {
		...
		public override void Refresh(GameNPC mob)
		{
			...
			if (mxlts != null)
			{
				lock (m_mobXLootTemplates)
				{
					...
					foreach (MobDropTemplate mxlt in mxlts)
					{
                        if (!m_mobXLootTemplates.TryGetValue(mxlt.MobName.ToLower(), out List<MobDropTemplate> mobxLootTemplates))
                        {
                            mobxLootTemplates = new List<MobDropTemplate>();
		...
		public override LootList GenerateLoot(GameNPC mob, GameObject killer)
		{
			...
			try
			{
				...
				if (!m_mobXLootTemplates.TryGetValue(mob.Name.ToLower(), out List<MobDropTemplate> killedMobXLootTemplates))
				{
					// We can use DropTemplateXItemTemplate.Count to determine how many of a item can drop
					if (m_lootTemplates.TryGetValue(mob.Name.ToLower(), out Dictionary<string, DropTemplateXItemTemplate> lootTemplatesToDrop))
					{
