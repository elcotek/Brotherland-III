    ...
    public class MarketSearch
	{
        ...
        public virtual List<InventoryItem> FindItemsInList(IList<InventoryItem> inventoryItems, SearchData search)
		{
			...
			foreach (InventoryItem item in inventoryItems)
			{
                ...
                //wenn der Hausbesitzer nicht der Owner der items ist werden die items nicht gelistet!
                if (house != null && item != null && house.OwnerID != (item.OwnerID) && item.OwnerLot == house.HouseNumber)
                {
                    continue;
                }

                //search items for all Realms or per realm seperated.
                if (ServerProperties.Properties.MARKET_LIST_SEPERATE)
                {
                    //Aussenhändler vorhanden ?
                    if (HouseMgr.GetConsignmentByHouseNumber((int)item.OwnerLot) == null)
                    {
                        continue;
                    }

                    if (item.OwnerLot >= 1 && item.OwnerLot <= 1383 && ((GamePlayer)m_searchPlayer).Realm != eRealm.Albion)
                    {
                        continue;
                    }

                    if (item.OwnerLot >= 1601 && item.OwnerLot <= 2574 && ((GamePlayer)m_searchPlayer).Realm != eRealm.Midgard)
                    {
                        continue;
                    }

                    if (item.OwnerLot >= 3200 && item.OwnerLot <= 4399 && ((GamePlayer)m_searchPlayer).Realm != eRealm.Hibernia)
                    {
                        continue;
                    }
                }
