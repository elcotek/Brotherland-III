    ...
    public class LootGeneratorTemplate : LootGeneratorBase
    {
                ...
                lock (m_mobXLootTemplates)
                {
                    ...
                    if (dbMobXLootTemplates != null)
                    {
                        foreach (MobXLootTemplate dbMobXTemplate in dbMobXLootTemplates)
                        {
                            ...
                            if (!m_mobXLootTemplates.TryGetValue(dbMobXTemplate.MobName.ToLower(), out List<MobXLootTemplate> mobxLootTemplates))
                            {
        ...
        public override void Refresh(GameNPC mob)
        {
            ...
            if (mxlts != null)
            {
                lock (m_mobXLootTemplates)
                {
                    ...
                    {
                        if (!m_mobXLootTemplates.TryGetValue(mxlt.MobName.ToLower(), out List<MobXLootTemplate> mobxLootTemplates))
                        {
                            mobxLootTemplates = new List<MobXLootTemplate>();
        ...
        public override LootList GenerateLoot(GameNPC mob, GameObject killer)
        {
            ...
            try
            {
                ...
                {
                    player = ((killer as GameNPC).Brain as IControlledBrain).GetPlayerOwner();
                }

                //no killer no loot
                if (killer is GameNPC && player == null)
                    return loot;
                if (player == null || player.ObjectState != GameObject.eObjectState.Active)
                    return loot;

                // allow the leader to decide the loot realm
                if (player != null && (player.Group != null || player.MemberIsInBG(player) && ServerProperties.Properties.BATTLEGROUP_LOOT))
                {
                    if (player.GetBattleGroupLeader(player) != null && GS.ServerProperties.Properties.BATTLEGROUP_LOOT)
                    {
                        //BattleGroup
                        player = player.GetBattleGroupLeader(player);
                        ...
                        if (m_lootTemplates.ContainsKey(mob.Name.ToLower()))
                        {
                            ...
                            if (lootTemplatesToDrop != null)
                            {
                                foreach (LootTemplate lootTemplate in lootTemplatesToDrop.Values)
                                {
                                    ...
                                    if (!killer.IsWithinRadius(mob, 3500)) continue;

                                    if (Properties.Player_MaxLevel < 50)
                                    {
                                        if (Properties.Player_MaxLevel < drop.Level + 2)
                                        {
                                            continue;
                                        }
                                    }
                                    double con = GamePlayer.GetConLevel(killer.Level, mob.Level);
                                    if (con < -3.0)
                                        con = -3.0;
                                    ...
                                    if (GameObject.GetConLevel(player.Level, drop.Level) <= -3 && GameLiving.GetConLevel(killer.Level, mob.Level) <= -2 && drop.Item_Type != 40 && drop.Item_Type != 0) continue;
                                    if (GameObject.GetConLevel(player.Level, drop.Level) <= -3 && drop.Object_Type != 0 && drop.Item_Type != 41 && drop.Item_Type != 40 && drop.Object_Type != 40) continue;


                                    //Grüne mobs sollten keie blauen items dropen
                                    //if (GameObject.GetConLevel(player.Level, mob.Level) <= -1 && drop.Item_Type != 40) continue;



                                    /*

                                    if ((drop.Level - 1) > mob.Level && drop.Item_Type != 40 && drop.Item_Type != 0) continue;
                                    if ((killer.Level > 0 && killer.Level < 8) && drop.Level + 1 < killer.Level && drop.Object_Type != 0 && drop.Item_Type != 41 && drop.Item_Type != 40 && drop.Object_Type != 40) continue;
                                    if ((killer.Level > 7 && killer.Level < 25) && drop.Level + 3 < killer.Level && drop.Object_Type != 0 && drop.Item_Type != 41 && drop.Item_Type != 40 && drop.Object_Type != 40) continue;
                                    if ((killer.Level > 24 && killer.Level < 35) && drop.Level + 5 < killer.Level && drop.Object_Type != 0 && drop.Item_Type != 41 && drop.Item_Type != 40 && drop.Object_Type != 40) continue;
                                    if ((killer.Level > 34 && killer.Level < 45) && drop.Level + 10 < killer.Level && drop.Object_Type != 0 && drop.Item_Type != 41 && drop.Item_Type != 40 && drop.Object_Type != 40) continue;
                                    if ((killer.Level > 44 && killer.Level < 51) && drop.Level + 15 < killer.Level && drop.Object_Type != 0 && drop.Item_Type != 41 && drop.Item_Type != 40 && drop.Object_Type != 40) continue;
                                    */
                        ...
                        foreach (MobXLootTemplate mobXLootTemplate in killedMobXLootTemplates)
                        {
                            ...
                            if (lootTemplatesToDrop != null)
                            {
                                foreach (LootTemplate lootTemplate in lootTemplatesToDrop)
                                {
                                    ...
                                    //log.DebugFormat("ConLevel0: {0}", GameLiving.GetConLevel(drop.Level, mob.Level));
                                    //Grüne mobs sollten keie blauen items dropen
                                    //if (GameObject.GetConLevel(player.Level, mob.Level) <= -1 && drop.Item_Type != 40) continue;
                                    //log.DebugFormat("ConLevel1: {0}", GameLiving.GetConLevel(drop.Level, mob.Level));
                                   // log.ErrorFormat("drop4: {0}", drop.Name);
                                    ...
                                    */
                                    ...
                                    if (drop != null && (drop.Realm == (int)player.Realm || drop.Realm == 0 || player.CanUseCrossRealmItems))
                                    {
                                        //log.ErrorFormat("lootTemplate Chance: {0}", lootTemplate.Chance);
                                        loot.AddRandom(lootTemplate.Chance, drop, 1);
                                    }
                                }
                            }
