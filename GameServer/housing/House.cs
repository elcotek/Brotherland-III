...
namespace DOL.GS.Housing
{
    public class House : Point3D, IGameLocation
    {
        ...
        public const string House_Owner_ID = "HOUSEOWNERIDFROMVAULT";
        ...
        {
            get { return _databaseItem.HasConsignment; }
            set { _databaseItem.HasConsignment = value; }
        }
        ...
        /// </summary>
        public GameLocation OutdoorJumpPoint(GamePlayer player)
        {
            ...
            if (player.CurrentRegion.IsHousing && player.TargetObject == null)
            {
                player.Z = Z;
            }
        ...
        /// <summary>
        /// Update Players House Emblem to Current one
        /// </summary>
        /// <param name="player"></param>
        public virtual void UpdateHouseEmblem(GamePlayer player)
        {

            if (player.CurrentHouse.HouseNumber > 0 && player.Guild != null && player != null && player.CurrentHouse.Emblem != player.Guild.Emblem)
            {
                IList<DBHouse> regionHousing = GameServer.Database.SelectObjects<DBHouse>("`HouseNumber` = @HouseNumber", new QueryParameter("@HouseNumber", player.CurrentHouse.HouseNumber));

                foreach (DBHouse house in regionHousing)
                {
                    if (house != null && house.HouseNumber == player.CurrentHouse.HouseNumber && player.Guild.Emblem > 0 && house.OwnerID == player.DBCharacter.ObjectId)
                    {
                        try
                        {


                            //log.Debug("emblem eingetragen");
                            house.Emblem = player.Guild.Emblem;
                            house.LastTimeRowUpdated = DateTime.Now;
                            house.AllowAdd = true;
                        ...
                        {
                            log.Error("House emblem konnet nicht aktualisiert werden!!");
                        }
                    }
                }
            }
        ...
        public void Enter(GamePlayer player)
        {
            ...
            //ChatUtil.SendSystemMessage(player, "House.Enter.EnteringHouse", player.Name);
            player.Out.SendEnterHouse(this);
            player.Out.SendFurniture(this);

            player.InHouse = true;
            player.CurrentHouse = this;
            UpdateHouseEmblem(player);
        ...
        {
            player.MoveTo(OutdoorJumpPoint(player));
        ...
        {
            //Manson can have 6 Vaults but have 5 on the server
            if (Model == 4 || Model == 8 || Model == 12)
            {
                var usedVaults = new[] { false, false, false, false, false };
                ...
                {
                    if (housePointItem.Index >= 0 && housePointItem.Index <= 4)
                    {
                        usedVaults[housePointItem.Index] = true;
                    }
                }

                for (int freeVault = 0; freeVault <= 4; ++freeVault)
                {
        ...
        public bool RemovePorch()
        {
            ...
            // set porch to false, no mor porch!
            Porch = false;
            Consignment = false;
            // broadcast updates
            SendUpdate();
        ...
        {
            //log.ErrorFormat("Remove house!! nummer:{0} ", HouseNumber);
            if (ConsignmentMerchant != null)
            {
                //log.Error("Remove house!!");
                ...
                //By Elcotek: Delete the Consignment Merchant from the Mob Database
                //var houseCMMob = GameServer.Database.SelectObject<Mob>("HouseNumber = '" + HouseNumber + "'");
                var houseCMMob = GameServer.Database.SelectObjects<HouseConsignmentMerchant>("`HouseNumber` = @HouseNumber", new QueryParameter("@HouseNumber", HouseNumber)).FirstOrDefault();

                if (houseCMMob != null)
                {
                    GameServer.Database.DeleteObject(houseCMMob);
                }
        ...
        {
            _permissionLevels.TryGetValue(permissionLevel, out DBHousePermissions permissions);
        ...
        public bool CanUseVault(GamePlayer player, GameHouseVault vault, VaultPermissions vaultPerms)
        {
            ...
            //Get the OwnerID form Current viewed vault
            player.TempProperties.setProperty(House_Owner_ID, OwnerID);
