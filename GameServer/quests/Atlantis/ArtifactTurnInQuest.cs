...
namespace DOL.GS.Quests.Atlantis
{
    ...
    public class ArtifactTurnInQuest : ArtifactQuest
	{
		...
		private string m_scholarName = String.Empty;
		...
		private string m_artifactID = String.Empty;
		/// <summary>
		/// The artifact ID.
		/// </summary>
		...
		private bool GetNextOptions(Dictionary<String, ItemTemplate> versions)
		{
			...
			while (m_curTypes.Count <= 1 && virtualStep < MAXNUMOFSTEPS)
			{
				...
				foreach (string str in versions.Keys)
				{
					...
					if (String.IsNullOrEmpty(type) == false && !m_curTypes.Contains(type))
					{
						m_curTypes.Add(type);
					}
				}
