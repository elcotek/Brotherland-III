...
namespace DOL.GS.Quests.Albion
{
	...
	public class BeginningOfWar : BaseFrederickQuest
	{
		...
		private static readonly GameLocation locationDunwynClone = new GameLocation(null, 1, 567604, 509619, 2813, 3292);

		private static GameNPC dunwyn = null;
		private GameNPC dunwynClone = null;

		private static GameNPC princessObera = null;
		private static readonly GameNPC[] fairySorceress = new GameNPC[4];
...
//				dunwyn.AddNPCEquipment((byte) eEquipmentItems.RIGHT_HAND, 19, 0, 0, 0);
			...
			for (int i = 0; i < fairySorceress.Length; i++)
			{
				if (fairySorceress[i] == null)
				{
                    ...
                    {
                        Model = 603, // //819;
                        Name = "ire fairy sorceress",
                        GuildName = "Part of " + questTitle + " Quest",
                        Realm = eRealm.None,
                        CurrentRegionID = 1,
                        Size = 35,
                        Level = 3,
                        X = princessObera.X + Util.Random(-150, 150),
                        Y = princessObera.Y + Util.Random(-150, 150),
                        Z = princessObera.Z
                    };
		...
		protected void CreateDunwynClone()
		{
			if (dunwynClone == null)
			{
                dunwynClone = new GameNPC
                {
                    Name = dunwyn.Name,
                    Model = dunwyn.Model,
                    GuildName = dunwyn.GuildName,
                    Realm = dunwyn.Realm,
                    CurrentRegionID = 1,
                    Size = dunwyn.Size,
                    Level = 15, // to make the figthing against fairy sorceress a bit more dramatic :)

                    X = 567604 + Util.Random(-150, 150),
                    Y = 509619 + Util.Random(-150, 150),
                    Z = 2813,
                    Heading = 3292
                };
