	...
	public class CityOfCamelot : BaseFrederickQuest
	{
		...
		public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
		{
			...
			*/
			...
			if (scrollUrqhart == null)
			{
                ...
                {
                    Name = "Scroll for Vault Keeper Urqhart"
                };
                if (log.IsWarnEnabled)
			...
			if (recruitsRoundShield == null)
			{
                ...
                {
                    Name = "Recruit's Round Shield",
                    Level = 4,

                    Weight = 31,
                    Model = 59, // studded Boots                

                    Object_Type = 0x2A, // (int)eObjectType.Shield;
                    Item_Type = (int)eEquipmentItems.LEFT_HAND,
                    Id_nb = "recruits_round_shield",
                    Price = Money.GetMoney(0, 0, 0, 4, 0),
                    IsPickable = true,
                    IsDropable = true,
                    Color = 36,
                    Hand = 2,
		...
		protected virtual int CreateAssistant(RegionTimer timer)
		{
			...
			else
			{
                ...
                {
                    Model = 951,
                    Name = m_questPlayer.Name + "'s Assistant",
                    GuildName = "Part of " + questTitle + " Quest",
                    Realm = m_questPlayer.Realm,
                    CurrentRegionID = m_questPlayer.CurrentRegionID,
                    Size = 25,
                    Level = 5,
                    X = m_questPlayer.X + 50,
                    Y = m_questPlayer.Y + 50,
                    Z = m_questPlayer.Z,
                    Heading = m_questPlayer.Heading
                };
