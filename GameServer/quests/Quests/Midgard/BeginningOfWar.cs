...
namespace DOL.GS.Quests.Midgard
{
    ...
    public class BeginningOfWar : BaseDalikorQuest
    {
        ...
        private static readonly GameLocation locationBriediClone = new GameLocation(null, 100, 794455, 721224, 4989, 3292);

        private static GameNPC briedi = null;
        private GameNPC briediClone = null;

        private static GameNPC princessAiyr = null;
        private static readonly GameNPC[] askefruerSorceress = new GameNPC[4];
        ...
        public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (recruitsHelm == null)
            {
                ...
                {
                    Name = "Recruit's Studded Helm (Mid)"
                };
                if (log.IsWarnEnabled)
        ...
        protected void CreateBriediClone()
        {
            if (briediClone == null)
            {
                briediClone = new GameNPC
                {
                    Name = briedi.Name,
                    Model = briedi.Model,
                    GuildName = briedi.GuildName,
                    Realm = briedi.Realm,
                    CurrentRegionID = locationBriediClone.RegionID,
                    Size = briedi.Size,
                    Level = 15, // to make the figthing against fairy sorceress a bit more dramatic :)

                    X = locationBriediClone.X + Util.Random(-150, 150),
                    Y = locationBriediClone.X + Util.Random(-150, 150),
                    Z = locationBriediClone.Y,
                    Heading = locationBriediClone.Heading
                };
        ...
        protected static void PlayerUseSlot(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (item != null && item.Id_nb == listBriedi.Id_nb)
            {
                if (quest.Step >= 4 && quest.Step <= 8)
                {
                    ...
                    {
                        "tattered shirt from escaped thrall",
                        "smiera-gatto's claw",
                        "coastal wolf blood"
                    };
