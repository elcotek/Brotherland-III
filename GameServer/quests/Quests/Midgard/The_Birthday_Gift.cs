	...
	GameNPC[] npcs;
			...
			if (npcs.Length == 0)
			{
				if (!WorldMgr.GetRegion(100).IsDisabled)
				{
                    ...
                    {
                        Model = 212,
                        Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "Mid.TheBirthdayGift.NPCBarkeepNognar")
                    };
			...
			if (npcs.Length == 0)
			{
				if (!WorldMgr.GetRegion(101).IsDisabled)
				{
                    ...
                    {
                        Model = 213,
                        Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "Mid.TheBirthdayGift.NPCBarkeepPrugar")
                    };
                    if (log.IsWarnEnabled)
		...
		rattlingskeletonpendant = GameServer.Database.FindObjectByKey<ItemTemplate>("rattlingskeletonpendant");
			if (rattlingskeletonpendant == null)
			{
                ...
                {
                    Name = "Rattling Skeleton Pendant"
                };
                if (log.IsWarnEnabled)
