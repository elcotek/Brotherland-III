...
namespace DOL.GS.Quests.Midgard
{
    ...
    public class TraitorInMularn : BaseDalikorQuest
    {
        ...
        private static readonly GameLocation hindaEnd = new GameLocation("Lady Hinda", 100, 100, 56484, 55671, 4682, 29);
        private static readonly GameLocation hindaStart = new GameLocation("Lady Hinda", 100, 811022, 727295, 4677, 908);
        ...
        public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (npcs.Length == 0)
            {
                ...
                ladyHinda = new GameNPC
                {
                    ...
                    Realm = eRealm.None,
                    CurrentRegionID = hindaStart.RegionID,
                    Size = 50,
                    Level = 30,
                    X = hindaStart.X,
                    Y = hindaStart.Y,
                    Z = hindaStart.Z,
                    Heading = hindaStart.Heading
                };
            ...
            if (recruitsBoots == null)
            {
                ...
                {
                    Name = "Recruit's Studded Boots (Mid)"
                };
                if (log.IsWarnEnabled)
                ...
                if (item != null && item.Id_nb == necklaceOfDoppelganger.Id_nb)
                {
                    if (player.IsWithinRadius(hindaEnd, 2500))
                    {
                        ...
                        if (!ladyHinda.IsAlive || ladyHinda.ObjectState != GameObject.eObjectState.Active)
                        {
                            ...
                            ladyHinda.AddToWorld();
                            ladyHinda.PathTo(hindaEnd.X, hindaEnd.Y, hindaEnd.Z, ladyHinda.MaxSpeed);
                        }
                        quest.Step = 3;
                    }
                }
