...
namespace DOL.GS.Quests
{
    ...
    public class MoneyTask : AbstractTask
    {
        ...
        private static readonly int[] MoneyReward = new int[45] { 25, 34, 46, 63, 84, 114, 154, 208, 282, 379, 441, 511, 592, 688, 798, 925, 1074, 1246, 1444, 1830, 1920, 2560, 3006, 4560, 5230, 6570, 7455, 8578, 9473, 10689, 11979, 12880, 13890, 14999, 16296, 17576, 19450, 21345, 23687, 25896, 27899, 30593, 43670, 50856, 63230 };
        ...
        public static bool BuildTask(GamePlayer player, GameLiving source)
        {
            ...
            else
            {
                ...
                player.QuestTask = new MoneyTask(player);
                player.QuestTask.TimeOut = DateTime.Now.AddHours(2);
                player.QuestTask.ItemName = TaskItems.Name;
                player.QuestTask.RecieverName = NPC.Name;
                ((MoneyTask)player.QuestTask).RecieverZone = NPC.CurrentZone.Description;
