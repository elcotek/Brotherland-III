...
namespace DOL.GS.Quests
{
    ...
    public class DataQuest : AbstractQuest
    {
        ...
        /// An item given to a player when starting with a search.
        /// </summary>
        protected int m_searchStartNPCTemplateID = 0;
        ...
        private byte NPClevel;
        private byte SearchFailSSpawn;


        //NPCTEMPLATE
        protected void CreateSearchAreaNPCTemplate(string areaStr)
        {
            string[] parse = areaStr.Split(';');

            //NPC TemplateID
            if (Convert.ToInt32(parse[1]) > 0)
            {
                m_searchStartNPCTemplateID = Convert.ToInt32(parse[1]);

                //log.ErrorFormat("Npctemplate gefunden ID:{0}", m_searchStartNPCTemplateID);


            }
            //NPC Level
            if (Convert.ToInt32(parse[2]) > 0)
            {
                NPClevel = Convert.ToByte(parse[2]);

                //log.ErrorFormat("Npc level ID:{0}", m_searchStartNPCTemplateID); 


            }
            //SearchFailSSpawn 1/0
            if (Convert.ToInt32(parse[3]) > 0)
            {
                SearchFailSSpawn = Convert.ToByte(parse[3]);
        ...
        protected void CreateSearchArea(string areaStr)
        {
            try
            {
                ...
                //NpcX = Convert.ToInt32(parse[4]);
                //NpcY = Convert.ToInt32(parse[5]);
                //RegionId = Convert.ToUInt16(parse[3]);
                ...
                if (ExecuteCustomQuestStep(QuestPlayer, Step, eStepCheckType.Step))
                {
                    ...
                    if (stepTemplates.Count == 1 && (StepType == eStepType.Kill || StepType == eStepType.KillFinish || StepType == eStepType.Search || StepType == eStepType.SearchFinish))
                    {
                            ...
                            if (template.Length > 1)
                        {
                            ...
                            int.TryParse(template[1], out int chance);
                            ...
                            if (chance > 0)
                            {
                                if (Util.Chance(chance) == false)
                                {
                                    ...
                                    //SendMessage(QuestPlayer, "Unfortunately you weren't lucky, nothing dropped, keep trying.", 0, eChatType.CT_System, eChatLoc.CL_SystemWindow);

                                    QuestPlayer.Out.SendMessage(LanguageMgr.GetTranslation(QuestPlayer.Client.Account.Language, "NothingDroppedKeepTrying"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                    // failed to drop, ignore step advance
                                    return false;
                                }
                                //This is a kill quest with a count of items that must we get before the step ends.
                                if (template.Length > 2)
                                {

                                    int.TryParse(template[2], out int count);
                                    itemCount = count;
                                    ...
                                    lock (QuestPlayer.Inventory)
                                    {
                                        ...
                                        {
                                            foreach (string Itemtemplate in stepTemplates)
                                            {
                                                ItemTemplate item = GameServer.Database.FindObjectByKey<ItemTemplate>(Itemtemplate);
                                                if (item == null)
                                                {
                                                    string errorMsg = string.Format("StepItemTemplate {0} not found in DB!", Itemtemplate);
                                                ...
                                                try
                                                {
                                                    ...
                                                    if (m_questPlayer.Inventory.GetFirstItemByID(item.Id_nb, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack) != null)
                                                {
                                                    itemsTest = m_questPlayer.Inventory.GetFirstItemByID(item.Id_nb, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack);
                                                }
                                                ...
                                                if (obj != null && obj is GameLiving)
                                                {
                                                    if (itemsTest == null)
                                                    {
                                                        m_questPlayer.Inventory.AddTemplate(GameInventoryItem.Create<ItemTemplate>(item), 1, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack);
                                                    }
                                                    else

                                                    if (itemsTest != null && itemsTest.Count < count)
                                                    {
                                                        m_questPlayer.Inventory.AddTemplate(GameInventoryItem.Create<ItemTemplate>(item), 1, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack);
                                                    }
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (m_questPlayer.Inventory.GetFirstItemByID(item.Id_nb, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack) != null)
                                                {
                                                    items = m_questPlayer.Inventory.GetFirstItemByID(item.Id_nb, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack);
                                                }
                                                    ...
                                                    if (items.Count < count && item.IsStackable)
                                                    {
                                                        //SendMessage(QuestPlayer, "You found " + items.Count + "/" + count + " of " + items.Name + " ", 0, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                                        QuestPlayer.Out.SendMessage(LanguageMgr.GetTranslation(QuestPlayer.Client.Account.Language, "YouFound", items.Count, count, items.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                                        
                                                        return false;
                                                    }
                                                    //SendMessage(QuestPlayer, "Congratulations! You found " + items.Count + "/" + count + " of " + items.Name + "  ", 0, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                                    QuestPlayer.Out.SendMessage(LanguageMgr.GetTranslation(QuestPlayer.Client.Account.Language, "CongratulationsYouFound", items.Count, count, items.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    ...
                    if (stepTemplates.Count > 0 && countComplete == false)
                    {
        ...
        public override void Notify(DOLEvent e, object sender, EventArgs args)
        {
            ...
            try
            {
                ...
                if (e == GamePlayerEvent.GiveItem)
                {
                    ...
                    if (itemCount > 0  && StepType == eStepType.Deliver)
                    {
                        if (a.Item.Count == itemCount)
                        {
                            //Player gibt Item zum NPC
                        ...
                        {
                            SendMessage(QuestPlayer, "Hand the full stack to " + a.Target.Name + "! Some seems are missing. You give: " + a.Item.Count + "/" + itemCount + " of " + a.Item.Name + "  ", 0, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        }
                    }
                    else
                        OnPlayerGiveItem(a.Source, a.Target, a.Item);
        ...
        protected virtual void CheckOfferQuest(GamePlayer player, GameObject obj)
        {
            ...
            if (CheckQuestQualification(player))
            {
                ...
                if (StartType == eStartType.RewardQuest)
                {
                    ...
                    GameObject offerObject = obj as GameObject;
                    if (offerObject != null)
                    {
                        TryTurnTo(obj, player);
                        ...
                        {
                            player.Out.SendQuestOfferWindow(offerObject, player, this);
                        }
                    }
            ...
            if (StartType == eStartType.Collection && item.Id_nb == DBDataQuest.CollectItemTemplate)
            {
                ...
                if (charQuest.Count < MaxQuestCount && player.Level <= MaxLevel && player.Level >= Level)
                {
                    ...
                    if (item.Count == 1)
                    {
                        ...
                        if (long.TryParse(DBDataQuest.RewardXP, out long rewardXP))
                        {
                            if (rewardXP > 0)
                    ...
                    case eStepType.InteractFinish:
                        {
                            if (StartType == eStartType.RewardQuest)
                            {
                                GameObject finishNPC = obj as GameObject;
                                if (finishNPC != null)
                                {
                                    TryTurnTo(obj, player);
                        ...
                        case eStepType.Collect:
                            {
                                ...
                                if (!string.IsNullOrEmpty(TargetText))
                                {
                                    if (obj.Realm == eRealm.None && obj is GameNPC)
                                    {
        ...
        /// <summary>                                                          #;templateID;Level;SearchFailSSpawn = 1/0 standard = 0
        /// # Load NPC TepmlateID and LEVEL FROM QUESTDB before SERARCH, like: #;550;35;0|SEARCH;8;Stop! You are in the right place, here you can /search ;101;36377;29844;500;10
        /// </summary>
        protected void ParseSearchAreasNPC()
        {
            if (m_dataQuest == null)
                return;
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(lastParse))
                {
                    ...
                    foreach (string str in parse1)
                    {
                        ...
                        if (str.ToUpper().StartsWith("#"))
                        {
                            CreateSearchAreaNPCTemplate(str);
                        }
                    ...
                    foreach (AbstractArea playerArea in player.CurrentAreas)
                    {
                        if (playerArea is QuestSearchArea && (playerArea as QuestSearchArea).DataQuest != null && (playerArea as QuestSearchArea).DataQuest.ID == ID)
                        {
                            if ((playerArea as QuestSearchArea).Step == Step)
                            {
                                ...
                                if (m_searchStartNPCTemplateID > 0 && SearchFailSSpawn == 0)
                                {
                                    int templateID = 0;

                                    templateID = m_searchStartNPCTemplateID;
                                    //log.Error("spawne mob ok ?");

                                    if (templateID == 0)
                                    {
                                        string errorMsg = string.Format("SearchStart NPC TemplateID {0} not found in DB!", m_searchStartItemTemplate);
                                        ChatUtil.SendDebugMessage(player, errorMsg);
                                        log.Error(errorMsg);
                                        return false;
                                    }
                                    ...
                                    NpcX = (playerArea as QuestSearchArea).X;
                                    NpcY = (playerArea as QuestSearchArea).Y;
                                    if (NPClevel <= 0)
                                    {
                                        NPClevel = player.Level;
                                    }
                                    if (player != null && RegionId > 0)
                                    {
                                        //NPC Spawn
                                        NpcSpawnAdd(templateID, NPClevel, NpcX, NpcY, player.Z, player.Heading /= 2, RegionId, 0, 0);
                                    }
                                    else
                                        log.Error("konnte NpcSpawnAdd nicht laden");
                                }
                                ////////////////////////////////////////////////////////////////////////////////////////////
                ...
                if (StepType == eStepType.Search)
                {
                    ...
                    if (AdvanceQuestStep() == false)
                    {
                        ...
                        if (m_searchStartNPCTemplateID > 0 && SearchFailSSpawn > 0)
                        {
                            ...
                            if (player != null && RegionId > 0 && NpcX != 0 && NpcY != 0)
                            {
                                //NPC Spawn
                                NpcSpawnAdd(templateID, NPClevel, NpcX, NpcY, player.Z, player.Heading /= 2, RegionId, 0, 0);
                            }
        ...
        private INpcTemplate m_addNpcTemplate;
        ...
        protected GameNPC NpcSpawnAdd(int templateID, int level, int x, int y, int Z, ushort heading, ushort region, int roamingrange, int respawnInterval)
        {
            GameNPC add = null;

            try
            {
                if (m_addNpcTemplate == null || m_addNpcTemplate.TemplateId != templateID)
                {
                    m_addNpcTemplate = NpcTemplateMgr.GetTemplate(templateID);
                }

                // Create add from template.


                if (m_addNpcTemplate != null)
                {
                    add = new GameNPC(m_addNpcTemplate)
                    {
                    ...
                    add.AddToWorld();
                    new DespawnTimers(add, add, 80 * 1000);
                }
            }
        ...
        protected class DespawnTimers : GameTimer
        {
            ...
            public DespawnTimers(GameObject timerOwner, GameNPC npc, int delay)
