...
namespace DOL.GS.ServerRules
{
    public abstract class AbstractServerRules : IServerRules
    {
        ...
        public abstract bool IsSameRealm(GameObject source, GameObject target, bool quiet);
        ...
        public virtual bool IsAllowedToAttack(GameLiving attacker, GameLiving defender, bool quiet)
        {
            ...
            //NPC in der gleichen Faction sollen sich nicht gegenseitig angreifen!
            if (defender.Realm == eRealm.None && attacker.Realm == eRealm.None && defender is GameNPC && attacker is GameNPC 
                && (attacker as GameNPC).Faction == (defender as GameNPC).Faction)
                return false;
            ...
            {
                if ((playerDefender as GamePlayer).Steed != null && (playerDefender as GamePlayer).Steed is GameTaxiBoat)
                {
                    if (!quiet)
                        MessageToLiving(attacker, defender.Name + " you can't attack enemys on boats!");
                    return false;
                }
                if ((playerAttacker as GamePlayer).Steed != null && (playerAttacker as GamePlayer).Steed is GameTaxiBoat)
                {
                    if (!quiet)
                        MessageToLiving(attacker, defender.Name + " you can't attack enemys from boats!");
                    return false;
                }
                if ((playerDefender as GamePlayer).Steed != null && (playerDefender as GamePlayer).Steed is GameBoat)
                {
                    if (!quiet)
                        MessageToLiving(attacker, defender.Name + " you can't attack enemys on boats!");
                    return false;
                }
                if ((playerAttacker as GamePlayer).Steed != null && (playerAttacker as GamePlayer).Steed is GameBoat)
                {
                    if (!quiet)
                        MessageToLiving(attacker, defender.Name + " you can't attack enemys from boats!");
                    return false;
            ...
            if (attacker is GameFont || attacker is GameStorm || attacker is GameMine)
            {
                Z2 = attacker.Z;
                Z1 = defender.Z;
                d = Z1 - Z2;
            ...
            if (defender is GamePlayer && attacker is GameFont && d > 50 || d < -50)
                return false;

            if (defender is GamePlayer && attacker is GameStorm && d > 50 || d < -50)
                return false;

            if (defender is GamePlayer && attacker is GameMine && d > 50 || d < -50)
                return false;

            if (defender is GameNPC && attacker is GameStorm && d > 50 || d < -50)
                return false;

            if (defender is GamePlayer && attacker is GameStorm && defender.IsStealthed)
                return false;
            ...
            if (defender is GameFont && attacker is GameNPC && attacker.Realm == 0)
                return false;

            if (defender is GameSiegeRam && attacker is GameNPC && attacker.Realm == 0)
                return false;

            if (defender is GameSiegeWeapon && attacker is GameNPC && attacker.Realm == 0)
                return false;
            //Pets sollen keine rammen zerstören können
            if (defender is GameSiegeWeapon && attacker is TheurgistPet && attacker.Realm != 0)
                return false;

            if (defender is GameSiegeWeapon && attacker is GamePet && attacker.Realm != 0)
                return false;

            if (defender is GameSiegeWeapon && attacker is GameNPC && (attacker as GameNPC).Brain is ControlledNpcBrain && attacker.Realm != 0)
                return false;



            if (defender is GameSiegeTrebuchet && attacker is GameNPC && attacker.Realm == 0)
                return false;

            if (defender is GameSiegeWeapon && attacker is GameNPC && attacker.Realm == 0)
                return false;

            if (defender is GameStorm && attacker is GameNPC && attacker.Realm == 0)
                return false;

            if (defender is GameMine && attacker is GameNPC && attacker.Realm == 0)
                return false;
        ...
        public virtual bool IsAllowedToCastSpell(GameLiving caster, GameLiving target, Spell spell, SpellLine spellLine)
        {
            ...
            if (target is GameKeepComponent || target is GameKeepDoor || target is GameSiegeCauldron || target is GameSiegeRam || target is GameSiegeTrebuchet || target is GameFont || target is GameSiegeWeapon
                || target is GameStorm || target is GameMine)
            {
                bool isAllowed = false;


                if (spell.Name == "Sabotage" || spell.Name == "SiegeWrecker" || spell.Name == "siegearrow")
                {
                    isAllowed = true;
                }
                        ...
                        if (spell.Radius == 0)
                        {
                            switch (spell.SpellType.ToLower())
                            {
                                ...
                                case "summontheurgistpet":
                                    {
                                        if (target is GameSiegeRam || target is GameKeepComponent)
                                        {
                                            isAllowed = false;
                                        }
                                        else
                ...
                if (!isAllowed && caster is GamePlayer)
                    ...
                    (caster as GamePlayer).Client.Out.SendMessage(LanguageMgr.GetTranslation(((GamePlayer)caster).Client.Account.Language, "Caster.CheckBeginCast.OnTarget", target.Name), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
        ...
        public virtual void OnNPCKilled(GameNPC killedNPC, GameObject killer)
        {
            ...
            GameSpellEffect charm = null;

            if (killedNPC != null)
            {
                charm = SpellHandler.FindEffectOnTarget(killedNPC, "Charm");
            }


            if (charm != null && killedNPC.CurrentRegion.Time - GameNPC.CHARMED_NOEXP_TIMEOUT < killedNPC.TempProperties.getProperty<long>(GameNPC.CHARMED_TICK_PROP))
            {
                message = "This monster has been charmed recently.";
            }

            
            if (!killedNPC.IsWorthReward)
            {
                if (killedNPC.Name == "VolleyTarget")
                    return;
                ...
                foreach (var de in killedNPC.XPGainers)
                {
                    ...
                    if (player != null)
                        ...
                        if (message != String.Empty)
                            (player as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((player as GamePlayer).Client.Account.Language, "AbstractServerRules.YouGainNoExperience3"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        else
                    
                            (player as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((player as GamePlayer).Client.Account.Language, "AbstractServerRules.YouGainNoExperience2"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            foreach (var de in killedNPC.XPGainers)
            {
                ...
                //Hard cap for keep and Tower Capture wort rp
                if (waslordKill && killedNPC is GuardLord && (killedNPC as GuardLord).Component.Keep != null && (killedNPC as GuardLord).Component.Keep is GameKeepTower && living is GamePlayer && (living as GamePlayer).CapturedTowers >= Properties.TOWER_CAPTURE_WORTRP_CAP)
                {
                    (living as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((living as GamePlayer).Client.Account.Language, "GamePlayer.GainRealmPoints.NoRP.CapturedTowers", Properties.TOWER_CAPTURE_WORTRP_CAP), eChatType.CT_Important, eChatLoc.CL_SystemWindow);

                    //log.Error("Keine rps für Tower");
                    realmPoints = 0;
                   
                }
                if (waslordKill && killedNPC is GuardLord && (killedNPC as GuardLord).Component.Keep != null && (killedNPC as GuardLord).Component.Keep is GameKeep && living is GamePlayer && (living as GamePlayer).CapturedKeeps >= Properties.KEEP_CAPTURE_WORTRP_CAP)
                {
                    (living as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((living as GamePlayer).Client.Account.Language, "GamePlayer.GainRealmPoints.NoRP.CapturedKeeps", Properties.KEEP_CAPTURE_WORTRP_CAP), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    //log.Error("Keine rps für Keep");
                    realmPoints = 0;
                ...
                if (Properties.LOW_XP_RATES)
                {
                    xpReward = npcExpValue;
                }

                else if (player != null && (player.Group == null || !plrGrpExp.ContainsKey(player.Group)))
                {
                ...
                if (player != null)
                {
                    expCap = (long)(GameServer.ServerRules.GetExperienceForLiving(player.Level) * ServerProperties.Properties.XP_CAP_PERCENT + (killedNPC.Level * 0.1) / 100);


                    if (player.Group != null && isGroupInRange)
                    {
                        if (Properties.LOW_XP_RATES_Group_Level_Difference)
                        {
                            foreach (GamePlayer players in player.Group.GetMembersInTheGroup())
                            {
                               double conLevel = GameObject.GetConLevel(players.Level, player.Level);
                                long malusPercent = 100;

                                
                                    //log.ErrorFormat("conlvel2 {0}", conLevel);

                                if (conLevel < 0)
                                {
                                    //log.ErrorFormat("conlvel {0}", conLevel);
                                    if (conLevel >= -2)
                                    {
                                        malusPercent = 35;
                                        malusPercent += (int)conLevel;
                                     //log.ErrorFormat("malus -1 = {0} ", malusPercent);
                                    }
                                    if (conLevel < -2)
                                    {
                                        malusPercent = 32;
                                        malusPercent += (int)conLevel;
                                       //log.ErrorFormat("malus -2 = {0} ", malusPercent);
                                    }
                                    if (conLevel <= -3)
                                    {
                                        malusPercent = 30;
                                        malusPercent += (int)conLevel;
                                       //log.ErrorFormat("malus -3 = {0} ", malusPercent);
                                    }
                                }
                                
                                //Kein malus
                                if (player.Level >= players.Level && player != players || conLevel >= -1.5)
                                {
                                    //log.ErrorFormat("kein malsus {0}", player.Name);
                                    //long _normalXP = (long)(GameServer.ServerRules.GetExperienceForLiving(player.Level) * (ServerProperties.Properties.XP_GROUP_CAP_PERCENT - malus) + (killedNPC.Level * 0.1) / 100);
                                    expCap = (long)(GameServer.ServerRules.GetExperienceForLiving(player.Level) * ServerProperties.Properties.XP_GROUP_CAP_PERCENT + (killedNPC.Level * 0.1) / 100);

                                   // expCap = _normalXP * malus / 100;
                                    //log.ErrorFormat("player 50 xp: {0}, player: {1}", expCap.ToString(), player.Name);
                                }




                                else if (player.Level < players.Level && player != players && players != null)
                                {
                                    //log.ErrorFormat("malsus {0},malusPercent: {1}", player.Name, malusPercent);
                                    long _xPCap = (long)(GameServer.ServerRules.GetExperienceForLiving(player.Level) * malusPercent + (killedNPC.Level * 0.1) / 100);

                                    //log.ErrorFormat("_xPCap: {0}", _xPCap);

                                    expCap = _xPCap * malusPercent / 100;

                                    //log.ErrorFormat("expCap: {0}", expCap);
                                    if (xpReward <= 0)
                                    {
                                        xpReward = 1;
                                    }
                ...
                int mobCountExtraBonus = StandardMobBrain.GroupExtraXPBonus; ;

                if (StandardMobBrain.WasBonus == false && mobCountExtraBonus > 0)
                {
                    mobCountExtraBonus = 0;
                }
                    ...
                    if (player != null)
                    {
                        ...
                        if (player.Group != null && plrGrpExp.ContainsKey(player.Group))
                        {
                            ...
                            foreach (GamePlayer players in player.Group.GetMembersInTheGroup())
                            {
                                ...
                                if (player.Level < players.Level && players != null)
                                {
                                    level = players.Level;
                                    ...
                                    level = player.Level;//the hightest in Group deside the Con level
                               // log.ErrorFormat("Level: {0}", level);
                            }

                           

                            if (GameObject.GetConLevel(level, killedNPC.Level) >= 0 && (int)plrGrpExp[player.Group] == 2)
                            {
                                 groupExp += (long)(0.125 * xpReward * (2 /*((int)plrGrpExp[player.Group]*/ + mobCountExtraBonus));
                                //log.ErrorFormat("groupExp:{0} xpReward: {1} mobCountExtraBonus: {2} ", groupExp, xpReward, mobCountExtraBonus);
                            }
                            if (GameObject.GetConLevel(level, killedNPC.Level) >= 1 && (int)plrGrpExp[player.Group] > 2 && (int)plrGrpExp[player.Group] <= 4)
                            {
                                groupExp += (long)(0.125 * xpReward * (2 /*((int)plrGrpExp[player.Group]*/ + mobCountExtraBonus));
                               // log.ErrorFormat("groupExp:{0} xpReward: {1} mobCountExtraBonus: {2} ", groupExp, xpReward, mobCountExtraBonus);
                            }
                            if (GameObject.GetConLevel(level, killedNPC.Level) >= 2 && (int)plrGrpExp[player.Group] > 5 && (int)plrGrpExp[player.Group] <= 7)
                            {
                                groupExp += (long)(0.125 * xpReward * (2 /*((int)plrGrpExp[player.Group]*/ + mobCountExtraBonus));
                               // log.ErrorFormat("groupExp:{0} xpReward: {1} mobCountExtraBonus: {2} ", groupExp, xpReward, mobCountExtraBonus);
                            }
                            if (GameObject.GetConLevel(level, killedNPC.Level) >= 3 && (int)plrGrpExp[player.Group] > 7)
                            {
                                groupExp += (long)(0.125 * xpReward * (2 /*((int)plrGrpExp[player.Group]*/ + mobCountExtraBonus));
                                // log.ErrorFormat("groupExp:{0} xpReward: {1} mobCountExtraBonus: {2} ", groupExp, xpReward, mobCountExtraBonus);
                            }
                        ...
                        //resets the add count
                        if (mobCountExtraBonus > 0 && StandardMobBrain.WasBonus == true)
                        {
                            StandardMobBrain.WasBonus = false;
                        }

                        if (groupExp > xpReward / ServerProperties.Properties.GROUP_XP_RATE + mobCountExtraBonus)
                            groupExp = xpReward / ServerProperties.Properties.GROUP_XP_RATE + mobCountExtraBonus;
        ...
        public virtual void OnPlayerKilled(GamePlayer killedPlayer, GameObject killer)
        {
            ...
            if (killedPlayer.DBCharacter.DeathTime + noExpSeconds > killedPlayer.PlayedTime)
            {
                ...
                foreach (var Gainer in killedPlayer.XPGainers)
                {
                    if (Gainer.Key != null && Gainer.Key is GamePlayer)
                    {
                        (Gainer.Key as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((Gainer.Key as GamePlayer).Client.Account.Language, "AbstractServerRules.killedRecentlyGainNoExperience", killedPlayer.Name), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        (Gainer.Key as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((Gainer.Key as GamePlayer).Client.Account.Language, "AbstractServerRules.killedRecentlyGainNoRealmPoints", killedPlayer.Name), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            ...
            { 
                ...
                if (dealNoXP)
                {
                    foreach (var gainer in killedPlayer.XPGainers)
                    {
                        ...
                        if (gainer.Key != null && gainer.Key is GamePlayer)
                            ...
                            player = gainer.Key as GamePlayer;

                        if (player != null)
                            (player as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((player as GamePlayer).Client.Account.Language, "AbstractServerRules.YouGainNoExperience"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                ...
                foreach (var de in killedPlayer.XPGainers)
                {
                    GamePlayer expGainPlayer = null;
                    GameLiving living = de.Key as GameLiving;

                    if (living != null && living is GamePlayer)
                    {
                        expGainPlayer = living as GamePlayer;
                    }
                    ...
                    if (living is GamePlayer)
                    {
                        ...
                        if (realmPoints > 0)
                        {
                            if (killedPlayer != null && living is GamePlayer && living.GetConLevel(killedPlayer) <= -3 && living is GamePlayer)
                            {//no RPS for Grey Players
                                (living as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((living as GamePlayer).Client.Account.Language, "AbstractServerRules.YouGainNoExperienceWhileResurection", killedPlayer), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                                return;
                                //((GamePlayer)de.Key).Out.SendMessage(killedPlayer.Name + " (player with resurection illnes / grey for you)has been killed and is worth no realm points!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            }
                            else
                            ...
                            //death player Participating RPS
                            if (killedPlayer != null && Properties.ALLOW_RPS_FOR_DEAD_PLAYERS)
                            {
                                if (killedPlayer.Guild != null && killedPlayer.Client.Account.PrivLevel == 1)
                                {
                                    int deathRPS = 0;
                                    deathRPS = realmPoints * 25 / 100;
                                    killedPlayer.Out.SendMessage(LanguageMgr.GetTranslation(killedPlayer.Client.Account.Language, "GamePlayer.Participating.Rps"), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                                   
                                    killedPlayer.GainRealmPoints(deathRPS);
                                    killedPlayer.Out.SendUpdatePoints();
                                }
                            }
                    ...
                    if (bountyPoints > 0)
                    {
                        if (killedPlayer != null && living.GetConLevel(killedPlayer) <= -3 && living is GamePlayer)
                        {//no BPS for Grey Players
                            //((GamePlayer)de.Key).Out.SendMessage(killedPlayer.Name + " (player with resurection illnes / grey for you)has been killed and is worth no bounty points!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            (living as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((living as GamePlayer).Client.Account.Language, "AbstractServerRules.YouGainNoExperienceWhileResurection2", killedPlayer.Name), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            return;
                        }
                        else
                        {
                            //death player Participating BPS
                            if (killedPlayer != null && Properties.ALLOW_RPS_FOR_DEAD_PLAYERS)
                            {
                                if (killedPlayer.Guild != null && killedPlayer.Client.Account.PrivLevel == 1)
                                {
                                    int deathBPS = 0;
                                    deathBPS = bountyPoints * 25 / 100;
                                    killedPlayer.Out.SendMessage(LanguageMgr.GetTranslation(killedPlayer.Client.Account.Language, "GamePlayer.Participating.Bps"), eChatType.CT_Important, eChatLoc.CL_SystemWindow);

                                    killedPlayer.GainBountyPoints(deathBPS);
                                    killedPlayer.Out.SendUpdatePoints();
                    ...
                    {
                        if (killedPlayer != null && living.GetConLevel(killedPlayer) <= -3 && living is GamePlayer)
                        {//no XPS for Grey Players
                         //((GamePlayer)de.Key).Out.SendMessage(killedPlayer.Name + " (player with resurection illnes / grey for you)has been killed and is worth no experience points!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            (living as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((living as GamePlayer).Client.Account.Language, "AbstractServerRules.YouGainNoExperienceWhileResurection3", killedPlayer.Name), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            return;
                        }
                        else
                        {
                    ...
                    if (living is GamePlayer)
                    {
                        ...
                        (player as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((player as GamePlayer).Client.Account.Language, "AbstractServerRules.AddMoney"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        player.AddMoney(money, "{0}");
                        (player as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((player as GamePlayer).Client.Account.Language, "AbstractServerRules.AddMoney2", gold), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    ...
                    if (living is GamePlayer)
                    {
                        ...
                        player.AddMoney(money, LanguageMgr.GetTranslation(player.Client.Account.Language, "GamePlayer.PickupObject.YouPickUp", Money.GetString(money)));
                        //(player as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((player as GamePlayer).Client.Account.Language, "GamePlayer.PickupObject.YouPickUp", gold), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        //player.AddMoney(money, "{0}");
        ...
        public virtual int GetBountyPointsForKeep(AbstractGameKeep keep)
        {
            ...
            if (keep is GameKeepTower)
            {
                value = Math.Max(5, 25 + (keep.BaseLevel - 50));
            }
            else
            {
                value = Math.Max(30, 50 + (keep.BaseLevel - 50));
            }
        ...
        public virtual void SendHousingMerchantWindow(GamePlayer player, DOL.GS.PacketHandler.eMerchantWindowType merchantType)
        {
            switch (merchantType)
            {
                ...
                default:
                    ...
                    log.ErrorFormat("Unknown merchant type {0}", merchantType.ToString());
                    break;
            }
        }
        ...
        public virtual void MessageToLiving(GameObject obj, string message)
        {
            MessageToLiving(obj, message, eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
        /// <summary>
        ...
        public virtual void MessageToLiving(GameObject obj, string message, eChatType type)
        {
            MessageToLiving(obj, message, type, eChatLoc.CL_SystemWindow);
        }
        /// <summary>
        ...
        public virtual void MessageToLiving(GameObject obj, string message, eChatType type, eChatLoc loc)
        {
            if (obj is GamePlayer)
                ((GamePlayer)obj).Out.SendMessage(message, type, loc);
        }
        #endregion
    }
}
