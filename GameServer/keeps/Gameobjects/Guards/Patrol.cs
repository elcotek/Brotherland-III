...
namespace DOL.GS.Keeps
{
    ...
    public class Patrol
    {
        ...
        public const int PATROL_SPEED = 225;
        ...
        /// </summary>
        public static Type[] GuardTypes = new Type[] { typeof(GuardFighter), typeof(GuardFighter), typeof(GuardHealer), typeof(GuardFighter), typeof(GuardHealer), typeof(GuardFighter) };
        ...
        public void InitialiseGuards()
        {
            ...
            int guardsOnPatrol = Component.Keep.Level;
            ...
            if (Component != null && Component.Keep != null && Component.Keep is GameKeep)
            {
                ...
                if (guardsOnPatrol < Component.Keep.Level)
                    guardsOnPatrol++;
            }

            if (PatrolGuards.Count < Component.Keep.Level)
            {
            ...
            //remove patrol if more as intended
            if (PatrolGuards.Count > Component.Keep.Level)
            {
                for (int i = 0; i < guardsOnPatrol; i++)
                {
        ...
        private void CreatePatrolGuard(int type)
        {
            ...
            guard.BodyType = 1576;
            guard.Flags ^= GameNPC.eFlags.TORCH;
        ...
        {
            int guardsToPatrol = Component.Keep.Level;

           
            //Fix:  Remove more as intended Gurads
            if (PatrolGuards != null)
            {
                for (int i = 0; i < PatrolGuards.Count; i++)
                {
                    ...
                    if (guard.ObjectState == GameObject.eObjectState.Active && PatrolGuards.Count > guardsToPatrol)
                    {
                        guard.Delete();
                      //  DeletePatrol();
                    }
            ...
            if (Component != null && Component.Keep != null && Component.Keep is GameKeep)
            {
                ...
                //if (Component.Keep.Level > 0 && guardsOnPatrol <= 5)
                   // guardsOnPatrol++;

                if (Component.Keep.Level > 0 && guardsToPatrol < Component.Keep.Level)
                    guardsToPatrol++;
            }
