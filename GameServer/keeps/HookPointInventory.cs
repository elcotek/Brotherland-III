...
namespace DOL.GS.Keeps
{
    public class HookPointInventory
    {
        ...
        public static void OnServerStarted(DOLEvent e, object sender, EventArgs args)
        {
            if (ServerProperties.Properties.LOAD_HOOKPOINTS)
            {
                ...
                //HookPointInventory.RedHPInventory.AddFirstFreeSlot(new HookPointItem("Caster Guard", 67, 2625, "DOL.GS.Keeps.GuardCaster", 0));
                ...
                //HookPointInventory.BlueHPInventory.AddFirstFreeSlot(new HookPointItem("Healer", 10, 0x0A40, "DOL.GS.Keeps.GuardHealer", 0));
                //HookPointInventory.BlueHPInventory.AddFirstFreeSlot(new HookPointItem("Hastener", 10, 0x0A40, "DOL.GS.Keeps.FrontierHastener", 0));
                //HookPointInventory.BlueHPInventory.AddFirstFreeSlot(new HookPointItem("BlackSmith", 1, 0x0A40, "DOL.GS.Blacksmith", 0));
                // HookPointInventory.BlueHPInventory.AddFirstFreeSlot(new HookPointItem("Dye Master", 1, 0x0A40, "DOL.GS.Keeps.GuardFighter", 0));
                // HookPointInventory.BlueHPInventory.AddFirstFreeSlot(new HookPointItem("Arrow Merchant", 1, 0x0A40, "DOL.GS.Keeps.GuardFighter", 0));
                // HookPointInventory.BlueHPInventory.AddFirstFreeSlot(new HookPointItem("Poison Merchant", 1, 0x0A40, "DOL.GS.Keeps.GuardHealer", 0));
        ...
        private readonly ArrayList hookpointItemList;
    ...
    public class HookPointItem
    {
        ...
        public void Invoke(GamePlayer player, int payType, GameKeepHookPoint hookpoint, GameKeepComponent component)
        {
            ...
            //log.ErrorFormat("Siege name: {0}", hookPointObj.Name);
            if (hookPointObj.Name == "cauldron of boiling oil")
            {
                hookPointObj.Z = player.Z; //+ 14 //= hookpoint.Z;
                //log.Error("cauldron of boiling oi gesetzt!");
            }
            else
                hookPointObj.Z = player.Z;
        ...
        public static void Invoke(GameKeepHookPoint hookpoint, string objectType)
        {
            ...
            if (hookPointObj is GameNPC || hookPointObj is GameSiegeWeapon)
            {
                ((GameNPC)hookPointObj).RespawnInterval = -1;//do not respawn
            }
            hookPointObj.AddToWorld();
            ...
            if (hookpoint.Component.IsRaized)
                hookPointObj.RemoveFromWorld();

            if (hookPointObj.Z != hookpoint.Z)
                hookPointObj.RemoveFromWorld();
        ...
        {
            if (String.IsNullOrEmpty(objTypeName) == false)
            {
