...
namespace DOL.GS.Keeps
{
    public class TemplateMgr
    {
        ...
        private static void SetGuardRespawn(GameKeepGuard guard)
        {
            ...
            {
                guard.RespawnInterval = Util.Random(5, 25) * 60 * ServerProperties.Properties.KEEP_GUARD_RESPAWN_INTERVAL;
            }
        }
        ...
        private static void SetGuardAggression(GameKeepGuard guard)
        {
            ...
            {
                (guard.Brain as KeepGuardBrain).SetAggression(90, 1000);
            }
            else if (guard is GuardStaticArcher)
            {
                (guard.Brain as KeepGuardBrain).SetAggression(90, 1200);
            }
        }
        ...
        /// <summary>
        /// Loads Guard Spells from NpcTmplate by guard Name
        /// </summary>
        /// <param name="guard"></param>
        private static void SetGuardSpells(GameKeepGuard guard)
        {
            INpcTemplate template;
            if (guard is GuardCaster || guard is GuardStaticCaster)
            {
                if (guard.Name == LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "SetGuardName.MasterWizard"))
                {
                    template = NpcTemplateMgr.GetTemplate(5005);

                    if (template != null && (template as NpcTemplate).Spells != null)

                        if (template.Spells != null && template.Spells.Count > 0)
                        {
                            try
                            {
                                foreach (Spell mobSpell in template.Spells)
                                {
                                    if ((guard as GameNPC).Spells.Contains(mobSpell) == false)

                                        (guard as GameNPC).Spells.Add(mobSpell);
                                }
                            }
                            catch (Exception ex)
                            {
                                log.Error("SetGuardSpells Error: ", ex);
                            }
                        }
                }
                if (guard.Name == LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "SetGuardName.MasterRunes"))
                {
                    template = NpcTemplateMgr.GetTemplate(5003);
                    if (template != null && (template as NpcTemplate).Spells != null)
                        ...
                        if (template.Spells != null && template.Spells.Count > 0)
                        {
                            try
                            {
                                foreach (Spell mobSpell in template.Spells)
                                {
                                    if (guard.Spells.Contains(mobSpell) == false)

                                        guard.Spells.Add(mobSpell);
                                }
                            }
                            catch (Exception ex)
                            {
                ...
                if (guard.Name == LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "SetGuardName.MasterEldritch"))
                {
                    template = NpcTemplateMgr.GetTemplate(5001);
                    if (template != null && (template as NpcTemplate).Spells != null)
                ...
                if (guard.Name == LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "SetGuardName.Wizard"))
                {
                    template = NpcTemplateMgr.GetTemplate(5004);
                    if (template != null && (template as NpcTemplate).Spells != null)
                ...
                if (guard.Name == LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "SetGuardName.Runemaster"))
                {
                    template = NpcTemplateMgr.GetTemplate(5002);
                    if (template != null && (template as NpcTemplate).Spells != null)
                ...
                if (guard.Name == LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "SetGuardName.Eldritch"))
                {
                    template = NpcTemplateMgr.GetTemplate(5000);
                    if (template != null && (template as NpcTemplate).Spells != null)
        ...
        public static void SetGuardSpeed(GameKeepGuard guard)
        {
            if (guard.IsPortalKeepGuard)
            {
                if (guard is GuardStaticArcher || guard is GuardStaticCaster)
                {
        ...
        {
            if (guard is GuardLord && guard.IsTowerGuard == true && guard.GuildName == "")
            {
                guard.Strength = (short)(5 + (guard.Level * DOL.GS.ServerProperties.Properties.KEEP_GUARD_STRENGTH));
                guard.Dexterity = (short)(guard.Level * 2);
                guard.Constitution = (short)(DOL.GS.ServerProperties.Properties.GAMENPC_BASE_CON + 50);
                guard.Quickness = Math.Max((byte)30, guard.Level);
            }
            if (guard is GuardLord && guard.IsTowerGuard == true && guard.GuildName != "")
            {
                guard.Strength = (short)(5 + (guard.Level *(DOL.GS.ServerProperties.Properties.KEEP_GUARD_STRENGTH +3)));
                guard.Dexterity = (short)(guard.Level * 2);
                guard.BlockChance = 5;
                guard.Constitution = (short)(DOL.GS.ServerProperties.Properties.GAMENPC_BASE_CON + 80);

                guard.BlockChance = 15;
                guard.Quickness = Math.Max((byte)30, guard.Level);
            }
            if (guard is GuardLord && guard.IsTowerGuard == false && guard.GuildName == "")
            {
                guard.Strength = (short)(5 + (guard.Level * (DOL.GS.ServerProperties.Properties.KEEP_GUARD_STRENGTH + 1)));
                guard.Dexterity = (short)(guard.Level * 3);
                guard.BlockChance = 10;
                guard.Constitution = (short)(DOL.GS.ServerProperties.Properties.GAMENPC_BASE_CON + 150);
                guard.Quickness = Math.Max((byte)30, guard.Level);
            }
            if (guard is GuardLord && guard.IsTowerGuard == false && guard.GuildName != "")
            {
                guard.Strength = (short)(5 + (guard.Level * (DOL.GS.ServerProperties.Properties.KEEP_GUARD_STRENGTH + 3)));
                guard.Dexterity = (short)(guard.Level * 4);
                guard.BlockChance = 20;
                guard.Constitution = (short)(DOL.GS.ServerProperties.Properties.GAMENPC_BASE_CON + 200);
                guard.Quickness = Math.Max((byte)30, guard.Level);
            }
            ...
            else
            {
                ...
                if (guard.GuildName == String.Empty)
                {
                    guard.Strength = (short)(5 + (guard.Level * DOL.GS.ServerProperties.Properties.KEEP_GUARD_STRENGTH));
                    guard.Dexterity = (short)(guard.Level);
                    guard.Intelligence = (short)(guard.Level);
                    guard.Constitution = (short)(DOL.GS.ServerProperties.Properties.GAMENPC_BASE_CON + 40);
                    guard.Quickness = Math.Max((byte)30, guard.Level);
                }
                if (String.IsNullOrEmpty(guard.GuildName) == false)
                {
                    guard.Strength = (short)(5 + (guard.Level * DOL.GS.ServerProperties.Properties.KEEP_GUARD_STRENGTH +1));
                    guard.Dexterity = (short)(guard.Level);
                    guard.Intelligence = (short)(guard.Level);
