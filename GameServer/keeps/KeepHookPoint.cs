...
namespace DOL.GS.Keeps
{
    ...
    public class GameKeepHookPoint : Point3D
    {
        ...
        private readonly HookpointTimer m_hookpointTimer;
    ...
    {
        private readonly GameKeepHookPoint m_hookpoint;
        ...
        protected override void OnTick()
        {
            ...
            if (m_hookpoint.Object != null && m_hookpoint.Object.ObjectState != GameObject.eObjectState.Deleted)
            {
                IList<DBKeepHookPointItem> items = GameServer.Database.SelectObjects<DBKeepHookPointItem>("`KeepID` = @KeepID AND `hookpointID` = @hookpointID", new[] { new QueryParameter("@KeepID", m_hookpoint.Component.Keep.KeepID), new QueryParameter("@hookpointID", m_hookpoint.ID) });

                foreach (DBKeepHookPointItem item in items)
                {
                    if (item != null && m_hookpoint.ID == item.HookPointID && m_hookpoint.Component.Keep.KeepID == item.KeepID)
                    {
                        



                        if (m_hookpoint.Object != null && m_hookpoint.Object.ObjectState == GameObject.eObjectState.Active)
                        {
                            m_hookpoint.Object.RemoveFromWorld();
                        }


                        if (m_hookpoint.Object != null && m_hookpoint.Object.ObjectState != GameObject.eObjectState.Deleted)
                        {
                            m_hookpoint.Object.ObjectState = GameObject.eObjectState.Deleted;
