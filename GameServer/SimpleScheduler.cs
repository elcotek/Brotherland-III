    ...
    public class SimpleScheduler
    {
        ...
        private void LaunchScheduledTask(ScheduledTask task)
        {
            ...
            if (log.IsWarnEnabled)
            {
                ...
                if (runningTime > 500)
                    log.WarnFormat("Simple Scheduler Task (interval:{1}) execution took {0}ms! ({2}.{3})", runningTime.ToString(), delay.ToString(), task.Method.GetMethodInfo().ReflectedType.ToString(), task.Method.GetMethodInfo().Name);
            }
        ...
        private void ResolvePendingTimers()
        {
            ...
            {
                if (PendingTimers.TryTake(out Tuple<long, ScheduledTask> task))
                {
                    if (!ScheduledTimers.TryGetValue(task.Item1, out List<ScheduledTask> scheduled))
                    {
