...
namespace DOL.GS
{
    ...
    public class GameHouseVault : GameVault, IHouseHookpointItem
	{
		...
		public GameHouseVault(ItemTemplate itemTemplate, int vaultIndex)
		{
			...
			base.Name = itemTemplate.Name;
            base.Model = (ushort) (itemTemplate.Model);
        ...
        public bool Attach(House house, uint hookpointID, ushort heading)
        {
			...
			IList<DBHouseHookpointItem> hpitem = GameServer.Database.SelectObjects<DBHouseHookpointItem>("`HouseNumber` = @HouseNumber AND `HookpointID` = @HookpointID", new[] { new QueryParameter("@HouseNumber", house.HouseNumber), new QueryParameter("@HookpointID", hookpointID) });
		...
		public override string GetOwner(GamePlayer player)
		{
         ...
         public override IList GetExamineMessages(GamePlayer player)
            ...
            {
                "[Right click to display the contents of house vault " + (m_vaultIndex + 1) + "]"
            };
            return list;
	  	                 }
		...
		public override bool Interact(GamePlayer player)
		{
            ...
            lock (m_vaultSync)
            {
                ...
                catch { log.Error("missing observer in GameHauseVault"); }
            }
                return true;
		}
