...
namespace DOL.GS
{
    public class GamePets : GameNPC
    {
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        public GamePets(INpcTemplate template)
            : base(template)
        {
            if (Inventory != null)
            {
        ...
        */
        ...
        protected override Style GetStyleToUse()
        {
            if (Styles != null && Styles.Count > 0 && Util.Chance(Properties.GAMENPC_CHANCES_TO_STYLE + Styles.Count))
            {
                ...
                else if (style != null && style.WeaponTypeRequirement != (int)eObjectType.Shield && style.WeaponTypeRequirement != (int)eObjectType.Spear)
                {
                    if (StyleProcessor.CanUseStyle(this, style, AttackWeapon))
                        return style;
                }
