...
namespace DOL.GS
{
    public class Ansgar : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            this.SetOwnBrain(new AnsgarBrain());
            Name = "Ansgar";
        ...
        public override void Die(GameObject killer)
        {
            if (killer != this)
                base.Die(killer);
            TempProperties.setProperty(NecormancerDie, null);
            ...
            if (player != null)
            {
                if ((player is GamePlayer && IsWorthReward) && (player.Group != null))
                {
                    ...
                    {
                        players.Out.SendMessage("Ansgar dies and his Portal was destroyed.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                        players.Out.SendMessage("Ansgar dies and his Portal was destroyed.", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);
                         
                        foreach (GameNPC npc in players.GetNPCsInRadius(WorldMgr.VISIBILITY_DISTANCE))
                        {


                            if (npc != null && npc.IsAlive && npc.Name == "Ansgar's Portal")
                            {
                                npc.Die(npc);
                            }
                        }
                ...
                {
                    player.Out.SendMessage("Ansgar dies and his Portal was destroyed.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage("Ansgar dies and his Portal was destroyed.", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);
...
{
    public class AnsgarBrain : StandardMobBrain, IOldAggressiveBrain
    {
        ...
        protected virtual void CanBeAttack(GameNPC body)
        {
            ...
            GameNPC NecroservantRespawn = null;
            ...
            foreach (GameNPC npc in Body.GetNPCsInRadius(WorldMgr.VISIBILITY_DISTANCE))
            {
                ...
                if (npc.Realm == eRealm.None && npc.InCombat == false && npc != null && npc.Name != "Ansgar" && (npc.Level < Body.Level - 2 || npc.Level > Body.Level - 2))
                {
                    _level = Body.Level;
                ...
                if (npc != null && npc != Body)
                {
                    ...
                    NecroservantRespawn = npc.TempProperties.getProperty<GameNPC>(NecromancerRespawned);


                    if (NecroservantRespawn != null)
                    {
                        npc.TempProperties.setProperty(NecromancerRespawned, null);
                        ...
                        switch (Body.Name)
                        {
                            ...
                            case "Ansgar":
                                {
                                    if (Body.Flags != (GameNPC.eFlags)24)
                                    {
                                        Body.Flags = (GameNPC.eFlags)24;
                                        Body.Level = 32;
                ...
                if (maxAdds > 0)
                {
                    ...
                    while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                    {
                        foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                        {
                            ...
                            {
                            AnsgarBrain brain = (AnsgarBrain)npc.Brain;
                                brain.AddToAggroList(PickTarget(attacker), 1);
                                brain.AttackMostWanted();
                                ++numAdds;
                            }
