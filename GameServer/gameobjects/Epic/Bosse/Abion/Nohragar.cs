...
//Nohra'gar Epic 5 Albion
namespace DOL.GS
{
    public class Nohragar : GameNPC
    {

        public override bool AddToWorld()
        {

            this.SetOwnBrain(new NohragaBrain());
            Name = "Nohra'gar";
            //GuildName = "Epic Encounter";
            Model = 755;
            Level = 28;
            Size = 50;
            //EquipmentTemplateID = "98cf4293-b66-4df9-8da4-3b221d97f54f";
            Strength = 100;
            Constitution = 250;
            Dexterity = 65;
            Quickness = 60;
        ...
        public override void Die(GameObject killer)
        {
            ...
            if ((player is GamePlayer && IsWorthReward) && (player.Group != null))
            {
                foreach (GamePlayer players in player.Group.GetPlayersInTheGroup())
                {
                    if (players != null)
                    {
                        ...
                        //npc.Die(npc);
                        players.Out.SendMessage("Nohra'gar dies. You has Nohra'gar's Portal Magic destroyed.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                        players.Out.SendMessage("Nohra'gar dies. You has Nohra'gar's Portal Magic destroyed.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            else
            {
                ...
                {
                    //npc.Die(npc);
                    player.Out.SendMessage("Nohra'gar dies and You has Nohra'gar's Portal Magic destroyed.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage("Nohra'gar dies and You has Nohra'gar's Portal Magic destroyed.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
...
{
    public class NohragaBrain : StandardMobBrain, IOldAggressiveBrain
    {
        const string SpamTime = "NOPORTSPAM";
        //public const int ThyponSpell = 5186;
        ...
        /*
        protected virtual bool CanBeAttack(GameNPC body)
        {
            ArrayList _plist = new ArrayList();
            GameNPC Necroservant = null;
            GameNPC BarriereNpc = null;
            byte _leveldd = 0;
            byte _level = 0;
            ...
            foreach (GamePlayer player in Body.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            {
                if (player != null)
                {
                    ...
                    if (_plist != null)
                        ...
                        _leveldd = player.Level;
                    _leveldd += 2;
                    _leveldd += (byte)_plist.Count;

                }


                if (Body.Realm == eRealm.None && Body.InCombat == false && player != null && (Body.Level < _leveldd || Body.Level > _leveldd))
                {
                    Body.Level = _leveldd;
                }
            }
            ...
            foreach (GameNPC npc in Body.GetNPCsInRadius(WorldMgr.VISIBILITY_DISTANCE))
            {
                ...
                if (npc.Realm == eRealm.None &&  npc.InCombat == false && npc != null && npc.Name != "Nohraga" && (npc.Level < Body.Level - 2 || npc.Level > Body.Level - 2))
                {
                    _level = Body.Level;

                    npc.Level = _level -= 2;
                }
                if (npc.IsAlive && npc.Name == "Barriere")
                {
                    if (npc.IsAlive)
                    {
                        BarriereNpc = npc;
                ...
                if (npc != null && npc != Body)
                {
                    if (npc.IsAlive && npc.Name == "Nohraga's Necroservant")
                    {
                        Necroservant = npc;
                        Body.TempProperties.setProperty(NecormancerAlive, true);
                    }
                    else
                        Necroservant = null;
            ...
            if (Necroservant != null)
            {
                if (Body.TempProperties.getProperty<bool>(IsPeace) == false)//peace
                {
                    switch (Body.Name)
                    {
                        ...
                        case "Nohraga":
                            {
                                Body.Flags ^= GameNPC.eFlags.CANTTARGET | GameNPC.eFlags.PEACE;
                                Body.Level = 32;
                                Body.BroadcastUpdate();
                                Body.TempProperties.setProperty(IsPeace, true);
                                return false;
                            }
                       }
                    return false;
            ...
            else if (Necroservant == null && Body.IsAlive) //(Body.IsAlive && Body.TempProperties.getProperty<bool>(PetAlive) == false)//in der nacht von 0 uhr bis 6
            {
                if (BarriereNpc != null)
                {
                    BarriereNpc.Die(BarriereNpc);
                }

                if (Body.TempProperties.getProperty<bool>(NecormancerAlive))
                {
                    switch (Body.Name)
                    {
                        case "Nohraga":
                            {
                                ...
                                if (mobs != null)
                                {
                                    ...
                                    Body.TempProperties.setProperty(IsPeace, false);
                                    Body.TempProperties.setProperty(NecormancerAlive, false);
                                }
                                return true;
                            }
                    }
        ...
        */
        ...
        protected virtual void CanPort(GameNPC body)
        {
            ...
            foreach (GamePlayer player in Body.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            {
                if (player != null)
                    ...
                    if (player != null && Body.X != 482709 && Body.IsMoving == false && Body.InCombat == false && Body.AttackState == false && (player.TargetObject != Body || player.TargetObject == null))
                    {
                        PortalSpell.SetDestination(51, 482709, 480902, 3110, 3084);
                        GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5611), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                    }


                if (player.Client.Account.PrivLevel >= (int)ePrivLevel.GM == false && player != null && player.IsAlive && player.TargetObject == Body && Body.IsMoving == false && Body.IsAlive && !Body.IsReturningHome)
                {
                    ...
                    {
                        m_aggroLevel = 65;
                        //Body.TempProperties.setProperty(IsAttacking, true);
                            ...
                            case 1:
                                {
                                    if (Body.X != 482931)
                                    {
                                        ...
                                        PortalSpell.SetDestination(51, 482931, 480688, 310, 3312);
                                        GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5611), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                        player.Out.SendMessage("Nohra'gar says, I saw you coming.And going...You want my knowledge to open what is closed. You can't have it. I am where you are going, where you were and where you are.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            ...
                            case 2:
                                {
                                    if (Body.X != 483665)
                                    {
                                        ...
                                        PortalSpell.SetDestination(Body.CurrentRegionID, 483665, 482779, 3192, 1890);
                                        GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5611), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                        player.Out.SendMessage("Nohra'gar says, I saw you coming.And going...You want my knowledge to open what is closed. You can't have it. I am where you are going, where you were and where you are.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            ...
                            case 3:
                                {
                                    if (Body.X != 484514)
                                    {
                                        ...
                                        PortalSpell.SetDestination(Body.CurrentRegionID, 484514, 480787, 3174, 903);
                                        GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5611), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                        player.Out.SendMessage("Nohra'gar says, I saw you coming.And going...You want my knowledge to open what is closed. You can't have it. I am where you are going, where you were and where you are.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            ...
                            case 4:
                                {
                                    if (Body.X != 483651)
                                    {
                                        ...
                                        PortalSpell.SetDestination(Body.CurrentRegionID, 483651, 480217, 3176, 70);
                                        GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5611), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                        player.Out.SendMessage("Nohra'gar says, I saw you coming.And going...You want my knowledge to open what is closed. You can't have it. I am where you are going, where you were and where you are.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            ...
                            case 5:
                                {
                                    if (Body.X != 483035)
                                    {
                                        ...
                                        PortalSpell.SetDestination(Body.CurrentRegionID, 483035, 481399, 3064, 2798);
                                        GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5611), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                        player.Out.SendMessage("Nohra'gar says, I saw you coming.And going...You want my knowledge to open what is closed. You can't have it. I am where you are going, where you were and where you are.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            ...
                            case 6:
                                {
                                    if (Body.X != 484058)
                                    {
                                        ...
                                        PortalSpell.SetDestination(Body.CurrentRegionID, 484058, 480584, 3078, 337);
                                        GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5611), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                        player.Out.SendMessage("Nohra'gar says, I saw you coming.And going...You want my knowledge to open what is closed. You can't have it. I am where you are going, where you were and where you are.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        {
            base.m_aggroLevel = 0;
            base.m_aggroMaxRange = 16000;
            base.AggroRange = 1750;
        }
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.NohragaBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            NohragaBrain brain = (NohragaBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
        ...
        public override void NpcYell(GameNPC attackedNPC, GameLiving attacker)
        {
            ...
            if (attackedNPC == null || attacker == null || attackedNPC.Brain is NohragaBrain == false)
                return;
            ...
            if (attacker != null && attackedNPC.InCombat)
            {
                ...
                foreach (GameLiving adds in attackedNPC.GetNPCsInRadius((ushort)attackedNPC.YellRange))
                {
                    ...
                    {
                        if (attackedNPC.Brain is NohragaBrain && chanceToYellBoss && add != null && add.IsAlive && add.ObjectState == GameObject.eObjectState.Active && add.IsAvailable && add != attackedNPC && attackedNPC.BodyType == 36 && add.BodyType == 37)
                        {
                            npcFriend = add;
