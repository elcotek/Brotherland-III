...
namespace DOL.GS
{
    public class AnsgarsNecroservant : GameNPC
    {
        const string NecormancerDie = "NECROMANCERDIE";
        const string NecromancerRespawned = "NECROMANCERRESPAWNED";

        public override bool AddToWorld()
        {

            this.SetOwnBrain(new AnsgarsNecroservantBrain());
            Name = "Ansgar's Necroservant";
            Model = 2177;
            Level = 32;
            Size = 50;
                ...
                if (player != null)
                {
                    if ((player is GamePlayer && IsWorthReward) && (player.Group != null))
                    {
                        foreach (GamePlayer players in player.Group.GetPlayersInTheGroup())
                        {
                            ...
                            players.Out.SendMessage("Ansgar's barrier disappears! Imagine another fight!", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);
                            players.Out.SendMessage("Ansgar says no one can defeat me, you will die just like any others before!", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);


                            foreach (GameNPC npc in players.GetNPCsInRadius(WorldMgr.VISIBILITY_DISTANCE))
                            {
                                if (npc != null && npc.IsAlive && npc.Name == "Barriere")
                                {
                                    npc.Die(npc);


                                }
                                if (npc != null && npc.IsAlive && npc.Name == "Ansgar" && npc.TempProperties.getProperty<GameLiving>(NecormancerDie) == null)
                                {
                                    Mob mobs = GameServer.Database.FindObjectByKey<Mob>(npc.InternalID);

                                    if (mobs != null)
                                    {
                                        npc.LoadFromDatabase(mobs);
                                        npc.BroadcastUpdate();
                                        npc.UpdateNPCEquipmentAppearance();
                                        npc.TempProperties.setProperty(NecormancerDie, npc);
                                    }
                                }
                            }
                        }
                    ...
                    else
                    {
                        foreach (GameNPC npc in player.GetNPCsInRadius(WorldMgr.VISIBILITY_DISTANCE))
                        {
                            ...
                            {
                                npc.Die(npc);
                                player.Out.SendMessage("Ansgar's barrier disappears! Imagine another fight!", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);
                                player.Out.SendMessage("Ansgar says no one can defeat me, you will die just like any others before!", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);
                            }
        ...
        public override void Notify(DOLEvent e, object sender, EventArgs args)
        {
            ...
            try
            {
                ...
                if (e == GameLivingEvent.AddToWorld)
                {
                   GameLiving SpawnedMob = sender as GameLiving;

                    if (SpawnedMob == null)
                        return;
                   
                    OnAnsgarspetAddToWorld(SpawnedMob);
                    return;
            ...
            {
                log.Error("Fehler bei:  " + e.Name, ex);
        ...
        /// <summary>
        /// The living is add to world.
        /// </summary>
        /// <param name="dying"></param>
        /// <param name="killer"></param>
        /// <param name="playerKillers"></param>
        protected virtual void OnAnsgarspetAddToWorld(GameLiving living)
        {
            if (living != null)
            {
               living.TempProperties.setProperty(NecromancerRespawned, living);
...
{
    public class AnsgarsNecroservantBrain : StandardMobBrain, IOldAggressiveBrain
    {
        ...
        /// </summary>
        public AnsgarsNecroservantBrain()
            : base()
        {
                ...
                if (living is GamePlayer && aggroamount > 0)
                {
                    ...
                    foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                    {
                        ...
                        if (protectAmount > 0)
                        {
                            ...
                            protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.AnsgarsNecroservantBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        protected override void AttackMostWanted()
        {
            ...
            // group.
            AnsgarsNecroservantBrain brain = null;

                GamePlayer attacker = (GamePlayer)attackData.Attacker;

                foreach (GameNPC npc in Body.GetNPCsInRadius(BAFCloseRange))
                {
                    if (npc.Brain is AnsgarsNecroservantBrain)
                    {
                        brain = (AnsgarsNecroservantBrain)npc.Brain;
                    }
                ...
                if (maxAdds > 0)
                {
                    ...
                    while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                    {
                        foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                        {
                            ...
                            {
                            AnsgarsNecroservantBrain brain = (AnsgarsNecroservantBrain)npc.Brain;
                                brain.AddToAggroList(PickTarget(attacker), 1);
                                brain.AttackMostWanted();
                                ++numAdds;
                            }
            ...
            public override void NpcYell(GameNPC attackedNPC, GameLiving attacker)
            {
                ...
                if (attackedNPC == null || attacker == null || attackedNPC.Brain is AnsgarsNecroservantBrain == false)
                    return;
                ...
                if (attacker != null && attackedNPC.InCombat)
                {
                    ...
                    foreach (GameLiving adds in attackedNPC.GetNPCsInRadius((ushort)attackedNPC.YellRange))
                    {
                        ...
                        {
                            if (attackedNPC.Brain is AnsgarsNecroservantBrain && chanceToYellBoss && add != null && add.IsAlive && add.ObjectState == GameObject.eObjectState.Active && add.IsAvailable && add != attackedNPC && attackedNPC.BodyType == 36 && add.BodyType == 37)
                            {
                                npcFriend = add;
