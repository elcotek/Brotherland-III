    ...
    public class GameTrainer : GameNPC
    {
        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            if (CanTrain(player))
            {
                ...
                if (player.GainXP == false)
                {
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "GameTrainer.CheckXPoFF.Text"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    return false;
                }
        ...
        {
            player.Out.SendMessage(String.Format(LanguageMgr.GetTranslation(player.Client, "GameTrainer.Interact.Respecialize", this.Name, player.Name)),
        ...
        public override bool ReceiveItem(GameLiving source, InventoryItem item)
        {
            ...
            if (player != null)
            {
                switch (item.Id_nb)
                {
                    ...
                    case "respec_single_dragon_merchant":
                        {
                            player.Inventory.RemoveCountFromStack(item, 1);
                            //InventoryLogging.LogInventoryAction(player, this, eInventoryActionType.Merchant, item.Template);
                            player.RespecAmountSingleSkill++;
                    ...
                    case "respec_full_dragon_merchant":
                        {
                            player.Inventory.RemoveCountFromStack(item, 1);
                            //InventoryLogging.LogInventoryAction(player, this, eInventoryActionType.Merchant, item.Template);
                            player.RespecAmountAllSkill++;
                    ...
                    case "respec_realm_dragon_merchant":
                        {
                            player.Inventory.RemoveCountFromStack(item, 1);
                            //InventoryLogging.LogInventoryAction(player, this, eInventoryActionType.Merchant, item.Template);
                            player.RespecAmountRealmSkill++;
        ...
        public bool PromotePlayer(GamePlayer player, int classid, string messageToPlayer, InventoryItem[] gifts)
        {
            ...
            if (player.SetCharacterClass(classid))
            {
                ...
                if (String.IsNullOrEmpty(messageToPlayer) == false)
