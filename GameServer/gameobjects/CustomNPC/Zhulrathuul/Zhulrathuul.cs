...
namespace DOL.AI.Brain
{
    ...
    /// </summary>
    public class Zhulrathuul : StandardMobBrain, IOldAggressiveBrain
    {
        /// <summary>
        ...
        public override void Think()
        {
            ...
            int oldPercent = Body.HealthPercent;//elcotek: mob health back ath returning home

            if (Body.IsReturningHome == true && Body.InCombat == false && Body.HealthPercent < 100 && Body is GameSiegeWeapon == false)
                Body.Health++;
        ...
        /*
                ...
                //If we have an aggrolevel above 0, we check for players and npcs in the area to attack
                if (Body.AttackState == false && AggroLevel > 0)
                {
                    CheckPlayerAggro();
                    CheckNPCAggro();
                }
                ...
                if (HasAggro)
                {
                    ...
                    int RandSpell = Util.Random(1, 4);

                    if (RandSpell == 1)
                    {    //lifeleech Heal for mob (spell 309322)
                        Body.CastSpell(SkillBase.GetSpellByID(Mesmerize), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                        AttackMostWanted();

                        Body.CastSpell(SkillBase.GetSpellByID(DirectDamage), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                        AttackMostWanted();
                        {
                            if (RandSpell == 2)
                            {   //lifeleech Heal for mob (spell 10594)
                                Body.CastSpell(SkillBase.GetSpellByID(Disarm), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                                AttackMostWanted();
                                ...
                                {
                                    if (RandSpell == 3)
                                    {    //lifeleech Heal for mob (spell 30138)
                                        Body.CastSpell(SkillBase.GetSpellByID(Nearsight), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                                        AttackMostWanted();

                                        Body.CastSpell(SkillBase.GetSpellByID(Mesmerize), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                                        AttackMostWanted();

                                        Body.CastSpell(SkillBase.GetSpellByID(DamageOverTime), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                                        AttackMostWanted();
                                        {
                                            if (RandSpell == 4)
                                            {    //lifeleech Heal for mob (spell 50001)
                                                Body.CastSpell(SkillBase.GetSpellByID(DamageOverTime), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                                                AttackMostWanted();
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            Zhulrathuul brain = (Zhulrathuul)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
