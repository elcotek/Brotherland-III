...
namespace DOL.GS
{
    ...
    public class Enchanter : GameNPC
    {
        ...
        public override bool ReceiveItem(GameLiving source, InventoryItem item)
        {
            ...
            if (item.Object_Type != 0 && (item.Level >= 10 && item.IsCrafted || item.Quality <= 91 && item.Template is ItemUnique == false && item.Level > 2))
            {
        ...
        protected void EnchanterDialogResponse(GamePlayer player, byte response)
        {
            ...
            if (item.Level > 10 && item.Level < 50)
                item.Bonus = BONUS_TABLE[(item.Level / 5) - 2];
            else if (item.Level > 2 && item.Level <= 10)
                item.Bonus = 5;
            else
                item.Bonus = 35;
