...
namespace DOL.GS
{
    ...
    public class HealingBarrelExchanger : GameNPC
    {


        private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        public HealingBarrelExchanger()
            : base()
        {
        ...
        {
            GuildName = "Healing Barrel Exchanger";
            Level = 50;
            base.AddToWorld();
        ...
        public override bool ReceiveItem(GameLiving source, InventoryItem item)
        {
            ...
            if (t != null && item != null && item.Name == "Elixir of Instant Healing")
            {
                if (t.BarrelForHealingElixirCount >= 34)
                {
                ...
                //34 * 3 = 102
                if (t.BarrelForHealingElixirCount + item.Count > 34 && t.BarrelForHealingElixirCount < 34)
                {
                ...
                t.BarrelForHealingElixirCount += item.Count;
                t.SaveIntoDatabase();

                if (t.BarrelForHealingElixirCount < 34)
                {

                    t.Out.SendMessage(Name + "Receive one " + t.BarrelForHealingElixirCount + " of 34 pices of " + item.Name + "! ", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    Emote(eEmote.Induct);
                }
                else
                {
                    t.Out.SendMessage("You receive the Barrel of Instant Healing from " + Name + " ", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    t.ReceiveItem(this, "barrel_of_instant_healing", 0);
        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            if (player.BarrelForHealingElixirCount >= 34)
            {
            ...
            msg = Name + " says, I'm the Barrel Exchanger.  Hand me 34 pices of (Elixir of Instant Healing) to me, and I Crafting for you one Barrel." +
                " " + Name + " Has Received " + player.BarrelForHealingElixirCount + " of 34 pices of (Elixir of Instant Healing) to create a Barrel for today.";

            player.Out.SendMessage(msg, eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            player.Out.SendMessage(Name + " Has Received " + player.BarrelForHealingElixirCount + " of 34 pices (Elixir of Instant Healing) to create a Barrel for today.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
