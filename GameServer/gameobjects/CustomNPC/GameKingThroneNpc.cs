...
namespace DOL.GS
{
    ...
    public class KingNPC : GameNPC
    {
        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            CharClass = player.CharacterClass.Name;
            if (player.Champion)
            {
                bool cllevel = false;
                ...
                {
                    player.Out.SendMessage("" + Name + " says, I can [respecialize] your champion skills if you so desire.", eChatType.CT_System, eChatLoc.CL_PopupWindow);
                    player.Out.SendMessage("" + Name + " says, My Quartermaster awaits, " + CharClass + "! Visit her now to recive the boon I granted you!", eChatType.CT_System, eChatLoc.CL_PopupWindow);
                    player.Out.SendMessage("" + Name + " says, you can train your Champion Levels, You can train your champion level with a base trainer if it doesn't belong to your basic class.!", eChatType.CT_System, eChatLoc.CL_PopupWindow);
                    player.Out.SendMessage("" + Name + " says, if you are a melee fighter, you have to go to a base magican trainer of your choice, since you belong to a class assigned to magic, you have to go to one of the base fighter trainers of your choice.", eChatType.CT_System, eChatLoc.CL_PopupWindow);
                }
                else
                {
                    player.Out.SendMessage("Come back to me if you have reached a Champion Level.  Your current Champ level is: " + player.ChampionLevel + " ", eChatType.CT_System, eChatLoc.CL_PopupWindow);
                }
