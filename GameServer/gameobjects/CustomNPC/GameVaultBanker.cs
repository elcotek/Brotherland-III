...
namespace DOL.GS
{
    ...
    /// </summary>
    [NPCGuildScript("Vault Banker")]
    public class GameVaultBanker : GameNPC, IGameInventoryObject
    {
        /// <summary>
        /// Constructor
        /// </summary>
        public GameVaultBanker()
            : base()
        {
        }
        ...
        public virtual ConcurrentDictionary<int, InventoryItem> GetClientInventory(GamePlayer player)
        {
            ...
            foreach (InventoryItem item in DBItems(player))
            {
                if (item != null)
                {
                    ...
                    {
                        log.ErrorFormat("HouseVAULT: Duplicate item {0}, owner {1}, position {2}", item.Name, player.OwnerID, (item.SlotPosition + slotOffset));
                    }
                }
            }
        ...
        public override bool Interact(GamePlayer player)
        {
           ...
           */
            ...
            if (house != null && player.ActiveInventoryObject != null)
            {
                ...
                {
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client, "House.GameVaultBanker.Description", Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_PopupWindow);
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client, "House.GameVaultBanker.HaveHouse", player.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_PopupWindow);
            ...
            player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client, "House.GameVaultBanker.Description", Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        public virtual bool MoveItem(GamePlayer player, ushort fromSlot, ushort toSlot)
        {
            ...
            GameVaultBanker gameVault = player.ActiveInventoryObject as GameVaultBanker;
            if (gameVault == null)
            {
