...
{
    public class Ammut : GameNPC
    {

        public override bool AddToWorld()
        {

            this.SetOwnBrain(new AmmutBrain());
            Name = "Ammut";
        ...
        public override void Die(GameObject killer)
        {
            if (killer != null && killer != null)
            {
                ...
                if (player != null)
                {
                        ...
                        if (battleGroup != null && GS.ServerProperties.Properties.BATTLEGROUP_LOOT)
                        {
                            ...
                            foreach (GamePlayer players in battleGroup.Members.Keys)
                            {
                                if (players.IsWithinRadius(killer, 8000) == false)
                                    continue;
                                players.Out.SendMessage("You have gained Masterlevel 5.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                    ...
                    {
                        player.Out.SendMessage("You have gained Masterlevel 5.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
...
{
    public class AmmutBrain : APlayerVicinityBrain, IOldAggressiveBrain
    {
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.AmmutBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            AmmutBrain brain = (AmmutBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
