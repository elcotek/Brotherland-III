...
{
    public class Medusa : GameNPC
    {

        public override bool AddToWorld()
        {

            this.SetOwnBrain(new MedusaBrain());
            Name = "Medusa";
                        ...
                        if (battleGroup != null && GS.ServerProperties.Properties.BATTLEGROUP_LOOT)
                        {
                            ...
                            foreach (GamePlayer players in battleGroup.Members.Keys)
                            {
                                if (players.IsWithinRadius(killer, 8000) == false)
                                    continue;
                                player.Out.SendMessage("You have gained Masterlevel 3.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.MedusaBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    ...
                    foreach (GamePlayer player in Body.GetPlayersInRadius((ushort)AggroRange))
                    {
                        ...
                        if (Body.InCombat)
                        {
                            ...
                            if (RandSpell == 2)
                            {
                                ...
                                Body.CastSpell(SkillBase.GetSpellByID(StoneMorph), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            MedusaBrain brain = (MedusaBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
