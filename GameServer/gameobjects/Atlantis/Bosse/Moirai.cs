...
namespace DOL.GS
{
    public class Moirai : GameNPC
    {
       public const string Minion_Lost_Healt = "MINIONATTACKEDANLOSTHHEALTH";
        public const string barrier_has_disappear = "BARRIERHASDISAPEAR";
        public const string Add_Spawned = "ADDSPAWNED";


        public override bool AddToWorld()
        {

            this.SetOwnBrain(new MoiraiBrain());
            Name = "Moirai";
            GuildName = "Epic Encounter";
            Model = 1183;
            Level = 65;
            Size = 75;
            //EquipmentTemplateID = "9596a1f7-676c-437a-af24-b66ea81b23d3";
            Strength = 100;
            Constitution = 250;
            Dexterity = 65;
            Quickness = 60;
        ...
        public override void Die(GameObject killer)
        {
            if (killer != this)
                base.Die(killer);
            //TempProperties.setProperty(Minion_Lost_Healt, false);
            //Body.TempProperties.setProperty(barrier_has_disappear, false);
            ...
            if (player != null)
            {
                if ((player is GamePlayer && IsWorthReward) && (player.Group != null))
                {

                    foreach (GamePlayer players in player.Group.GetPlayersInTheGroup())
                    {
                        players.Out.SendMessage("Moirai dies and her Portal was destroyed.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                        players.Out.SendMessage("Moirai dies and her Portal was destroyed.", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);
                                              
                    }
                }
                else
                {
                    player.Out.SendMessage("Moirai dies and her Portal was destroyed.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage("Moirai dies and her Portal was destroyed.", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);
        ...
        public override bool AddControlledNpc(IControlledBrain controlledNpc)
        {
            ...
            PetCount++;
            // log.DebugFormat("PetCount == {0}, m_controlledBrain.lenght = {1}", PetCount, m_controlledBrain.Length);
...
//public const int ThyponSpell = 5186;

namespace DOL.AI.Brain
{

    public class MoiraiBrain : StandardMobBrain, IOldAggressiveBrain
    {
        public const int heal = 3028;

        GameNPC barriere = null;
        GameNPC moirai = null;
        int i = 0;
        protected static List<GameLiving> Barrierelist = new List<GameLiving>();
        /// <summary>
        /// Defines a logger for this class.
        /// </summary>
        private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);

        private bool m_addspawn;

        /// <summary>
        /// The Ork Spawn
        /// </summary>
        private bool AddSpawn
        {
            get { return m_addspawn; }
            set { m_addspawn = value; }
        }
        ...
        protected virtual void CanAttack()
        {
            ...
            if (Body.IsAlive && barriere != null && barriere.IsAlive) // Model 928
            {

                //Peace remove from Morai
                ...
                barriere.RemoveFromWorld();
                //add Barrierre to list for later respawning
                if (Barrierelist.Contains(barriere) == false)
                {
                ...
                {
                    player.Out.SendMessage("Moirai's barrier disappears! Imagine another fight!", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage("Moirai says, no one can defeat me, you will die just like any others before!", eChatType.CT_Broadcast, eChatLoc.CL_SystemWindow);

                }
            }
        }

        protected virtual void CanBeAttack(GameNPC body)
        {
            ...
            {
                if (Body != null && Body.IsAlive && Body.Name == "Moirai")
                {

                    moirai = Body;
                }

                if (npc.IsAlive && npc.Name == "Moirai Invisible Energy")
                {

                    barriere = npc;
                }

                if (npc != null && npc != Body && npc.Name == "Minion of Moirai" && npc.HealthPercent > 0 && npc.HealthPercent < 100)
                {
                    if (minions.Contains(npc) == false)
                    {
                ...
                if (npc.Name == "Minion of Moirai" && npc.HealthPercent > 0 && npc.HealthPercent < 100)
                {
                    bool canattack = false;
                    //Can we attack Moirai?
                    for (i = 0; i < minions.Count; i++)
                    {
                        if (i >= 2)
                        {
                            if (moirai != null && moirai.IsAlive && moirai == Body)
                            {
                                moirai.TempProperties.setProperty(Moirai.Minion_Lost_Healt, true);
                            }

                            if (Body.HealthPercent < 51 && Body.TempProperties.getProperty<bool>(Moirai.Minion_Lost_Healt))
                            {
                    ...
                    if (canattack == true && moirai.Flags == (GameNPC.eFlags)24)
                    {
                        //log.Error("Moirai ist angreifbar!");
                        CanAttack();
                        
                    }
                    //Moirai heals his Minions at <20% live of his minions
                    if (npc.HealthPercent <= 20)
                    {
                        GameObject lastTarget = null;
                        lastTarget = moirai.TargetObject;
                        moirai.TargetObject = npc;

                        if (moirai.TargetObject != null)
                        {
                            if (moirai.IsMoving)
                                moirai.StopMoving();
                            moirai.TurnTo(moirai.TargetObject);
                            moirai.CastSpell(SkillBase.GetSpellByID(heal), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), true);
                            moirai.TargetObject = lastTarget;
                            break;
                        }
                    }
                }
                if (i < 2 && moirai.TempProperties.getProperty<bool>(Moirai.Minion_Lost_Healt))
                {
                    

                    moirai.TempProperties.setProperty(Moirai.Minion_Lost_Healt, false);
                }
            }
        }
        ...
        public MoiraiBrain()
            : base()
        {
            base.m_aggroLevel = 65;
            base.m_aggroMaxRange = 2800;
            base.AggroRange = 800;
        }
        ...
        public override void Think()
        {
            ...
            if (moirai != null && moirai.Flags != (GameNPC.eFlags)24 && moirai.TempProperties.getProperty<bool>(Moirai.Minion_Lost_Healt) == false)
            {
                //log.Error("attack nicht möglich!");
                ...
                switch (moirai.Name)
                {
                    ...
                    case "Moirai":
                        {
                            if (moirai.Flags != (GameNPC.eFlags)24)
                            {
                                moirai.Flags = (GameNPC.eFlags)24;
                                moirai.Level = 65;
                                moirai.BroadcastUpdate();
                                moirai.TempProperties.setProperty(Moirai.barrier_has_disappear, false);
                            }
                            break;
                        }
                }
                ...
                //respawn of Barriere
                if (Barrierelist != null)
                {
                    for (int i = 0; i < Barrierelist.Count; i++)
                    {
                        if (string.IsNullOrEmpty(Barrierelist[i].ToString()) == false)
                        {
                            lock ((Barrierelist as ICollection).SyncRoot)
                            {
                                barriere = Barrierelist[i] as GameNPC;
                                if (Barrierelist.Contains(Barrierelist[i]))
                                    Barrierelist.RemoveAt(i);
                            }
                        }
                        if (barriere != null && barriere is GameNPC)
                        {
                            GameNPC mob = barriere as GameNPC;
            ...
            if (Body.ControlledBrain != null && Body.ControlledBrain.Body.IsAlive)// && Body.ControlledBrain.Body.InCombat == false) //if (Body.AttackState == false && Body.InCombat == false && Body != null && Body.TargetObject == null)
            {

                if (Body.Formation != GameNPC.eFormationType.Protect)
                {
                    Body.Formation = GameNPC.eFormationType.Protect;
                }

                /*
                if (Body.IsAlive && Body.InCombat == false && Body.HealthPercent == 100 && Body.X != 50779)
                {
                    PortalSpell.SetDestination(Body.CurrentRegionID, 50779, 19788, Body.Z, 23);
                    GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5613), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                }
                if (Body.HealthPercent <= 90 && Body.HealthPercent >= 69 && Body.X != 48213)
                {
                    PortalSpell.SetDestination(Body.CurrentRegionID, 48213, 20960, Body.Z, 262);
                    GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5613), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                }
                else if (Body.HealthPercent <= 70 && Body.HealthPercent >= 49 && Body.X != 47476)
                {
                    PortalSpell.SetDestination(Body.CurrentRegionID, 47476, 24035, Body.Z, 1446);
                    GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5613), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                }
                else if (Body.HealthPercent <= 50 && Body.X != 50563)
                {
                    PortalSpell.SetDestination(Body.CurrentRegionID, 50563, 23747, Body.Z, 1551);
                    GameNPCHelper.CastSpellOnOwnerAndPets(Body, Body, SkillBase.GetSpellByID(5613), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                }
                */
            }
            ...
            //const string BarriereAlive = "PETALIVEPROPERTIE";
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.AnsgarBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                ...
                if (!CheckSpells(eCheckSpellType.Offensive))
                {
                    ...
                    foreach (GamePlayer player in Body.GetPlayersInRadius((ushort)AggroRange))
                    {
                        ...
                        if (HasAggro)
                        {
                            ...
                            int RandSpell = Util.Random(1, 35);

                            if (RandSpell == 5)
                            {
                                //PBAOE Heal (Clerics "Banish Evil" Spell)
                                //Body.CastSpell(SkillBase.GetSpellByID(ThyponSpell), SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            MoiraiBrain brain = (MoiraiBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
        ...
        public override void NpcYell(GameNPC attackedNPC, GameLiving attacker)
        {
            ...
            if (attackedNPC == null || attacker == null || attackedNPC.Brain is AnsgarBrain == false)
                return;
            ...
            if (attacker != null && attackedNPC.InCombat)
            {
                ...
                foreach (GameLiving adds in attackedNPC.GetNPCsInRadius((ushort)attackedNPC.YellRange))
                {
                    ...
                    {
                        if (attackedNPC.Brain is AnsgarBrain && chanceToYellBoss && add != null && add.IsAlive && add.ObjectState == GameObject.eObjectState.Active && add.IsAvailable && add != attackedNPC && attackedNPC.BodyType == 36 && add.BodyType == 37)
                        {
                            npcFriend = add;
                ...
                else if (type == eCheckSpellType.Offensive)
                {
                    if (Body is TheurgistPet == false)
                        ...
                        foreach (Spell spell in Body.Spells)
                        {
                            ...
                            if (living != null && Body.GetSkillDisabledDuration(spell) == 0 && SpellHandler.FindEffectAndImmunityOnTarget(living, spell.SpellType) == null || Body is GameKeepGuard || spell.SpellType.ToLower().Contains("DirectDamageWithDebuff"))
                            {
...
&& Body.GetDistanceTo(Body.ControlledBrain.Body) <= spell.Range && spell.Target.ToLower() != "self")
                ...
                case "Summon":
                    Body.TargetObject = Body;
                    break;
                case "BossPetSummonMinion":
                    //If the list is null, lets make sure it gets initialized!
                    if (Body.ControlledNpcList == null)
                        Body.InitControlledBrainArray(1); //Anzahl pets
                    else
                    {
        ...
        public override IPoint3D CalcRandomWalkTarget()
        {
            ...
            double targetFlyZ = Body.SpawnPoint.Z + Util.RandomDouble(-(double)roamingRadius * GS.ServerProperties.Properties.Roam_ZSCALEFACTOR * 0.1, (double)roamingRadius * Properties.Roam_ZSCALEFACTOR * 0.1); ;
        ...
        public void SpawnAdds()
        {
            ...
            if (moirai.TempProperties.getProperty<bool>(Moirai.Add_Spawned) == false)
            {

                /*
                1 39409 22850 z16185 h2990
                2 39582 22571 z16185 h3867
                3 39891 22657 z16185 h585
                4 39925 22919 z16185 h1264
                5 39713 23077 z16185 h1981
                6 39402 22851 z16185 h3001
                */
                //Spawn
                addSpawnAdd(804, spanlevel, 39582, 22571, 16185, 3867, moirai.CurrentRegionID, 0, -1);
                addSpawnAdd(804, spanlevel, 39891, 22657, 16185, 585, moirai.CurrentRegionID, 0, -1);
                addSpawnAdd(804, spanlevel, 39925, 22919, 16185, 1264, moirai.CurrentRegionID, 0, -1);
                addSpawnAdd(804, spanlevel, 39713, 23077, 16185, 1981, moirai.CurrentRegionID, 0, -1);
                addSpawnAdd(804, spanlevel, 39402, 22851, 16185, 3001, moirai.CurrentRegionID, 0, -1);
                moirai.TempProperties.setProperty(Moirai.Add_Spawned, true);
            }
            AddSpawn = true;
        }
        ...
        protected GameNPC addSpawnAdd(int templateID, int level, int x, int y, int Z, ushort heading, ushort region, int roamingrange, int respawnInterval)
        {
            GameNPC add = null;
            ...
            try
            {
                ...
                if (m_addOrkTemplate != null)
                {
                    ...
                    add.AddToWorld();
                    new DespawnTimers2(add, add, 60 * 1000);
                }
            }

            catch
            {
                log.Warn(String.Format("Unable to get template for {0}", Body.Name));
            }
            return add;
        }
        /// </summary>
