...
namespace DOL.GS.Scripts
{
    public class Cetus : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            this.SetOwnBrain(new CetusBrain());
            Name = "Cetus";
            ...
            if (killer != null && killer != null)
            {
                ...
                if (player != null)
                {
                    ...
                    if (player is GamePlayer && IsWorthReward && (player.Group != null || player.MemberIsInBG(player) && GS.ServerProperties.Properties.BATTLEGROUP_LOOT))
                    {
                        ...
                        if (battleGroup != null && ServerProperties.Properties.BATTLEGROUP_LOOT)
                        {
                            //BattleGroup
                            foreach (GamePlayer players in battleGroup.Members.Keys)
                            {
                                if (players.IsWithinRadius(killer, 8000) == false)
                                    continue;
                                players.Out.SendMessage("You have gained Masterlevel 1.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                    ...
                    {
                        player.Out.SendMessage("You have gained Masterlevel 1.", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
...
{
    public class CetusBrain : APlayerVicinityBrain, IOldAggressiveBrain
    {
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.CetusBrain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFReinforcementsRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        {
                            CetusBrain brain = (CetusBrain)npc.Brain;
                            brain.AddToAggroList(PickTarget(attacker), 1);
                            brain.AttackMostWanted();
                            ++numAdds;
                        }
