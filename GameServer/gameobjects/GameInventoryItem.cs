...
namespace DOL.GS
{
    ...
    public class GameInventoryItem : InventoryItem, IGameInventoryItem
    {
        ...
        public virtual bool CheckValid(GamePlayer player)
        {
            if (player == null)
            {
                log.Error("GamePlayer is null in CheckValid method");
                return false; // oder throw new ArgumentNullException(nameof(player));
            }
            m_owner = player;
            return true;
        }
        ...
        public virtual bool CanEquip(GamePlayer player)
        {
            if (player == null)
            {
                log.Error("GamePlayer is null in CanEquip method");
                return false; // oder throw new ArgumentNullException(nameof(player));
            }
        ...
        public static GameInventoryItem Create<T>(T item)
        {
            if (item == null)
            {
                log.Error("Item is null in Create method");
                return null; // oder throw new ArgumentNullException(nameof(item));
            }

            GameInventoryItem playerItem = null;
            Type tItem = item.GetType();
            PropertyInfo pClassType = GetClassType(tItem);
                ...
                else
                {
                    ...
                    for (int i = 0; i < array.Length; i++)
                    {
                        ...
                        if (assembly.GetType(classType) != null)
                        {
                            try
                            {
                                playerItem = assembly.CreateInstance(classType, false, BindingFlags.CreateInstance, null, new object[] { item }, null, null)
            ...
            if (classType != null && string.IsNullOrEmpty(classType) == false)
            {
                log.WarnFormat("Failed to construct inventory item of ClassType {0}!", classType);
            }

            playerItem = Activator.CreateInstance(typeof(GameInventoryItem), new object[] { item })
                as GameInventoryItem;

            return playerItem;
        }

        private static PropertyInfo GetClassType(Type tItem)
        {
            return tItem.GetProperty("ClassType");
        }
        ...
        public virtual void OnReceive(GamePlayer player)
        {
            if (player == null)
            {
                log.Error("GamePlayer is null in OnReceive method");
                return; // Hier können Sie entscheiden, wie Sie mit dieser Situation umgehen möchten
            }
            m_owner = player;
        }
        ...
        public virtual WorldInventoryItem Drop(GamePlayer player)
        {
            ...
            Point2D itemloc = GetHeading(player);
        ...
        private static Point2D GetHeading(GamePlayer player)
        {
            return player.GetPointFromHeading(player.Heading, 30);
        }
        ...
        public virtual void OnStrikeTarget(GameLiving owner, GameObject target)
        {
            if (owner is GamePlayer)
            {
                ...
                if (IsNotLosingDur == false && ConditionPercent > 70 && Util.Chance(Properties.ITEM_CONDITION_LOSS_CHANCE))
                {
        ...
        public virtual void OnStruckByEnemy(GameLiving owner, GameLiving enemy)
        {
            if (owner is GamePlayer)
            {
                //log.Error("was hit by enemy");
                GamePlayer player = owner as GamePlayer;

                //Extended Condition Lost system by Elcotek
                InventoryItem LeftBracer = player.Inventory.GetItem(eInventorySlot.LeftBracer);
                InventoryItem RightBracer = player.Inventory.GetItem(eInventorySlot.RightBracer);
                InventoryItem LeftRing = player.Inventory.GetItem(eInventorySlot.LeftRing);
                InventoryItem RightRing = player.Inventory.GetItem(eInventorySlot.RightRing);
                InventoryItem Neck = player.Inventory.GetItem(eInventorySlot.Neck);
                InventoryItem Jewellery = player.Inventory.GetItem(eInventorySlot.Jewellery);
                InventoryItem Cloak = player.Inventory.GetItem(eInventorySlot.Cloak);
                InventoryItem Waist = player.Inventory.GetItem(eInventorySlot.Waist);
                ...
                if (IsNotLosingDur == false && ConditionPercent > 70 && Util.Chance(Properties.ITEM_CONDITION_LOSS_CHANCE))
                {
                    ...
                    switch (Util.Random(7))
                    {
                        case 0:
                            if (LeftBracer != null && IsNotLosingDur == false)
                                LeftBracer.Condition -= sub;
                            if (Condition < 0)
                                Condition = 0;
                            break;
                        case 1:
                            if (RightBracer != null && IsNotLosingDur == false)
                                RightBracer.Condition -= sub;
                            if (Condition < 0)
                                Condition = 0;
                            break;
                        case 2:
                            if (LeftRing != null && IsNotLosingDur == false)
                                LeftRing.Condition -= sub;
                            if (Condition < 0)
                                Condition = 0;
                            break;
                        case 3:
                            if (RightRing != null && IsNotLosingDur == false)
                                RightRing.Condition -= sub;
                            if (Condition < 0)
                                Condition = 0;
                            break;
                        case 4:
                            if (Neck != null && IsNotLosingDur == false)
                                Neck.Condition -= sub;
                            if (Condition < 0)
                                Condition = 0;
                            break;
                        case 5:
                            if (Jewellery != null && IsNotLosingDur == false)
                                Jewellery.Condition -= sub;
                            if (Condition < 0)
                                Condition = 0;
                            break;
                        case 6:
                            if (Cloak != null && IsNotLosingDur == false)
                                Cloak.Condition -= sub;
                            if (Condition < 0)
                                Condition = 0;
                            break;
                        case 7:
                            if (Waist != null && IsNotLosingDur == false)
                                Waist.Condition -= sub;
                            if (Condition < 0)
                                Condition = 0;
                            break;
        ...
        public virtual void Delve(List<String> delve, GamePlayer player)
        {
            if (delve == null)
            {
                log.Error("List<String> delve is null in Delve method");
                return;
            }
            if (player == null)
            {
                log.Error("GamePlayer is null in Delve method");
                return;
            }
            ...
            else if (Description != null && String.IsNullOrEmpty(Description) == false)
            {
            ...
            if ((Object_Type == (int)eObjectType.Magical || Object_Type == (int)eObjectType.Cloth) && Item_Type == (int)eInventorySlot.Cloak
                || Item_Type == (int)eInventorySlot.LeftRing || Item_Type == (int)eInventorySlot.RightBracer || Item_Type == (int)eInventorySlot.Neck
                || Item_Type == (int)eInventorySlot.Waist || Item_Type == (int)eInventorySlot.Jewellery)
        ...
        {
            if (Util.IsEmpty(AllowedClasses, true))//inventory
            {
                return;
            }
        ...
        protected virtual void WriteMagicalBonuses(IList<string> output, GameClient client, bool shortInfo)
        {
            ...
            //GM Stats display
            WriteBonusLineGM(output, client, Bonus1Type, Bonus1);
            WriteBonusLineGM(output, client, Bonus2Type, Bonus2);
            WriteBonusLineGM(output, client, Bonus3Type, Bonus3);
            WriteBonusLineGM(output, client, Bonus4Type, Bonus4);
            WriteBonusLineGM(output, client, Bonus5Type, Bonus5);
            WriteBonusLineGM(output, client, Bonus6Type, Bonus6);
            WriteBonusLineGM(output, client, Bonus7Type, Bonus7);
            WriteBonusLineGM(output, client, Bonus8Type, Bonus8);
            WriteBonusLineGM(output, client, Bonus9Type, Bonus9);
            WriteBonusLineGM(output, client, Bonus10Type, Bonus10);
            WriteBonusLineGM(output, client, ExtraBonusType, ExtraBonus);
            ...
            if (!shortInfo)
            {
                if ((ProcSpellID == 0 || ProcSpellID1 == 0 || SpellID == 0 || SpellID1 == 0) && LevelRequirement > 1)
                {
                    int requiredLevel = LevelRequirement > 0 ? LevelRequirement : Math.Min(50, Level);

                    if (requiredLevel > 1)
                    {
                        output.Add(LanguageMgr.GetTranslation(client.Account.Language, "DetailDisplayHandler.WriteMagicalBonuses.LevelRequired2", requiredLevel.ToString()));
                        output.Add(" ");
                    }
                }

               else if (ProcSpellID != 0 || ProcSpellID1 != 0 || SpellID != 0 || SpellID1 != 0)
                {
                    int requiredLevel = LevelRequirement > 0 ? LevelRequirement : Math.Min(50, Level);
            ...
            //Player Stats display
            WriteItemExtraBonusLine(output, client, Bonus1Type, Bonus1);
            WriteItemExtraBonusLine(output, client, Bonus2Type, Bonus2);
            WriteItemExtraBonusLine(output, client, Bonus3Type, Bonus3);
            WriteItemExtraBonusLine(output, client, Bonus4Type, Bonus4);
            WriteItemExtraBonusLine(output, client, Bonus5Type, Bonus5);
            WriteItemExtraBonusLine(output, client, Bonus6Type, Bonus6);
            WriteItemExtraBonusLine(output, client, Bonus7Type, Bonus7);
            WriteItemExtraBonusLine(output, client, Bonus8Type, Bonus8);
            WriteItemExtraBonusLine(output, client, Bonus9Type, Bonus9);
            WriteItemExtraBonusLine(output, client, Bonus10Type, Bonus10);
            WriteItemExtraBonusLine(output, client, ExtraBonusType, ExtraBonus);
            ...
            WriteFocusLine(output, Bonus1Type, Bonus1, client);
            WriteFocusLine(output, Bonus2Type, Bonus2, client);
            WriteFocusLine(output, Bonus3Type, Bonus3, client);
            WriteFocusLine(output, Bonus4Type, Bonus4, client);
            WriteFocusLine(output, Bonus5Type, Bonus5, client);
            WriteFocusLine(output, Bonus6Type, Bonus6, client);
            WriteFocusLine(output, Bonus7Type, Bonus7, client);
            WriteFocusLine(output, Bonus8Type, Bonus8, client);
            WriteFocusLine(output, Bonus9Type, Bonus9, client);
            WriteFocusLine(output, Bonus10Type, Bonus10, client);
            WriteFocusLine(output, ExtraBonusType, ExtraBonus, client);
            ...
            if (!shortInfo)
            {
                ...
                if (ProcSpellID != 0)
                {
                    ...
                    if (lineType != null)
                    {
                        procType = SkillBase.FindSpell(ProcSpellID, lineType);
                    }
                    ...
                    if (line != null)
                    {
                        ...
                        if (procSpell != null)
                        {
                            ...
                            else
                            {
                                ...
                                log.Error("-" + procSpell.Name + " (Spell Handler Not Implemented)");
                            }
                        ...
                        else
                        {
                            ...
                            log.Error("- Spell Not Found: " + ProcSpellID);
                        }
                    }
                    else
                    {
                ...
                if (ProcSpellID1 != 0)
                {
                    ...
                    if (lineType1 != null)
                    {
                        procType1 = SkillBase.FindSpell(ProcSpellID1, lineType1);
                    }
                    ...
                    {
                        //defensiver oder offensiver proc ?
                        if (procType1 != null && procType1.SpellType == "DefensiveProc")
                            spellNote = LanguageMgr.GetTranslation(client.Account.Language, "DetailDisplayHandler.WriteMagicalBonuses.StrikeItem");
                        else
                            spellNote = LanguageMgr.GetTranslation(client.Account.Language, "DetailDisplayHandler.WriteMagicalBonuses.StrikeEnemy");
                    }
                    ...
                    if (chargeEffectsLine != null)
                    {
                        ...
                        if (spell != null)
                        {
                            ...
                            if (spellHandler != null)
                            {
                                ...
                                {
                                    Spell m_procSpell = SkillBase.GetSpellByID((int)spell.SubSpellID);

                                    if (m_procSpell != null)
                                    {

                                        ISpellHandler procspellHandler = ScriptMgr.CreateSpellHandler(client.Player, m_procSpell, chargeEffectsLine);
                                        if (m_procSpell != null)
                                        {
                                            output.AddRange(procspellHandler.DelveInfo(client.Player));
                                            output.Add(" ");
                                ...
                                //Sub spell
                                if (spell.Value > 0 && (spell.SpellType == "OffensiveProc" || spell.SpellType == "DefensiveProc" || spell.SpellType == "ValkyrieOffensiveProc"))
                                {
...

                    {
                        ...
                        list.Add(string.Format(
                            ...
                            bonusValue.ToString("+0 ;-0 ;0 "), //Eden
                             ...
                             || bonusCat == (int)eProperty.MythicalEvade
        ...
        protected virtual void WriteFocusLine(IList<string> list, int focusCat, int focusLevel, GameClient client)
        {
        ...
        protected virtual void DelveWeaponStats(List<String> delve, GamePlayer player)
        {
            ...
            double itemSPD = SPD_ABS / 10.0;
            double effectiveDPS = (clampedDPS + DPSRR5) * Quality / 100.0 * Condition / MaxCondition;
            ...
            if (itemDPS != 0)
            {
                delve.Add("- " + effectiveDPS.ToString("0.0") + " DPS");//RR5 = 16,5 DPS
            }
        }
		...
		public virtual void WriteTechnicalInfo(List<String> delve, GameClient client)
        {
            ...
            delve.Add("");
            delve.Add(" SlotPosition: " + SlotPosition.ToString());
            if (OwnerLot != 0 || SellPrice != 0)
            {
                delve.Add("    Owner Lot: " + OwnerLot.ToString());
                delve.Add("   Sell Price: " + SellPrice.ToString());
            }
            delve.Add("");
            delve.Add("        Level: " + Level.ToString());
            ...
            delve.Add("");
            delve.Add("        Model: " + Model.ToString());
            delve.Add("    Extension: " + Extension.ToString());
            delve.Add("        Color: " + Color.ToString());
            delve.Add("       Emblem: " + Emblem.ToString());
            delve.Add("       Effect: " + Effect.ToString());
            delve.Add("");
            delve.Add("       DPS_AF: " + DPS_AF.ToString());
            delve.Add("      SPD_ABS: " + SPD_ABS.ToString());
            delve.Add("         Hand: " + Hand.ToString());
            delve.Add("  Type_Damage: " + Type_Damage.ToString());
            delve.Add("        Bonus: " + Bonus.ToString());

            if (GlobalConstants.IsWeapon(Object_Type))
            {
                delve.Add("");
                delve.Add("         Hand: " + GlobalConstants.ItemHandToName(Hand).ToString() + " (" + Hand.ToString() + ")");
                delve.Add("Damage/Second: " + (DPS_AF / 10.0f));
                delve.Add("        Speed: " + (SPD_ABS / 10.0f));
                delve.Add("  Damage type: " + GlobalConstants.WeaponDamageTypeToName(Type_Damage).ToString() + " (" + Type_Damage.ToString() + ")");
                delve.Add("        Bonus: " + Bonus);
            }
            else if (GlobalConstants.IsArmor(Object_Type))
            {
                delve.Add("");
                delve.Add("  Armorfactor: " + DPS_AF.ToString());
                delve.Add("   Absorption: " + SPD_ABS.ToString());
                delve.Add("        Bonus: " + Bonus.ToString());
            }
            else if (Object_Type == (int)eObjectType.Shield)
            {
                delve.Add("");
                delve.Add("Damage/Second: " + (DPS_AF / 10.0f).ToString());
                delve.Add("        Speed: " + (SPD_ABS / 10.0f).ToString());
                delve.Add("  Shield type: " + GlobalConstants.ShieldTypeToName(Type_Damage).ToString() + " (" + Type_Damage.ToString() + ")");
                delve.Add("        Bonus: " + Bonus);
            }
            ...
            delve.Add("Count/MaxCount: " + Count.ToString() + " / " + MaxCount.ToString());
            delve.Add("        Weight: " + (Weight / 10.0f).ToString() + "lbs");
            delve.Add("       Quality: " + Quality.ToString() + "%");
            delve.Add("    Durability: " + Durability.ToString() + "/" + MaxDurability.ToString());
            delve.Add("     Condition: " + Condition.ToString() + "/" + MaxCondition.ToString());
            delve.Add("         Realm: " + Realm.ToString());
            delve.Add("");
            delve.Add(" CraftPriceSet: " + (CraftPriceSet ? "yes" : "no"));
            delve.Add("      Dropable: " + (IsDropable ? "yes" : "no"));
            delve.Add("      Pickable: " + (IsPickable ? "yes" : "no"));
            delve.Add("      Tradable: " + (IsTradable ? "yes" : "no"));
            delve.Add("     AlwaysDUR: " + (IsNotLosingDur ? "yes" : "no"));
            delve.Add("    Indestruct: " + (IsIndestructible ? "yes" : "no"));
            delve.Add("     Stackable: " + (IsStackable ? "yes (" + MaxCount.ToString() + ")" : "no"));
            delve.Add("");
            delve.Add("   ProcSpellID: " + ProcSpellID.ToString());
            delve.Add("  ProcSpellID1: " + ProcSpellID1.ToString());
            delve.Add("    ProcChance: " + ProcChance.ToString());
            delve.Add("       SpellID: " + SpellID.ToString() + " (" + Charges.ToString() + "/" + MaxCharges.ToString() + ")");
            delve.Add("      SpellID1: " + SpellID1.ToString() + " (" + Charges1 + "/" + MaxCharges1 + ")");
            delve.Add(" PoisonSpellID: " + PoisonSpellID.ToString() + " (" + PoisonCharges.ToString() + "/" + PoisonMaxCharges.ToString() + ") ");
            delve.Add("");
            delve.Add("AllowedClasses: " + AllowedClasses);
            delve.Add(" LevelRequired: " + LevelRequirement.ToString());
            delve.Add("    BonusLevel: " + BonusLevel.ToString());
            delve.Add(" ");
            delve.Add("              Flags: " + Flags.ToString());
            delve.Add("     SalvageYieldID: " + SalvageYieldID.ToString());
