...
namespace DOL.GS
{
    public class SpellCrafting : AdvancedCraftingSkill
    {
        ...
        /// <summary>
        /// FocusSpellAllowed for Staffs only
        /// </summary>
        /// <param name="item"></param>
        /// <param name="currentItem"></param>
        /// <returns></returns>
        public virtual bool FocusSpellAllowed(InventoryItem item, InventoryItem currentItem)
        {
            if (item.Object_Type != 8 && item.Object_Type != 28 && item.Object_Type != 26)
            {
                switch (currentItem.Bonus1Type)
                {
                    case 120: // Focus_Darkness
                    case 121: //Focus_Suppression =
                    case 122: //Focus_Runecarving
                    case 123: //Focus_Spirit =
                    case 124: //Focus_Fire
                    case 125: //Focus_Air 
                    case 126: //Focus_Cold 
                    case 127: //Focus_Earth
                    case 128: //Focus_Light
                    case 129: //Focus_Body
                    case 130: //Focus_Matter
                              // 131 Available
                    case 132: //Focus_Mind
                    case 133: //Focus_Void
                    case 134: //Focus_Mana
                    case 135: //Focus_Enchantments
                    case 136: //Focus_Mentalism
                    case 137: //Focus_Summoning
                    case 138: //Focus_BoneArmy
                    case 139: //Focus_PainWorking
                    case 140: //Focus_DeathSight
                    case 141: //Focus_DeathServant
                    case 142: //Focus_Verdant
                    case 143: //Focus_CreepingPath
                    case 144: //Focus_Arboreal
                    case 157: //Focus_EtherealShriek
                    case 158: //Focus_PhantasmalWail
                    case 159: //Focus_SpectralForce
                    case 160: //Focus_Cursing
                    case 161: //Focus_Hexing
                    case 162: //Focus_Witchcraft
                    case 165: //AllFocusLevels
            ...
            else
            {
                ...
                lock (player.TradeWindow.Sync)
                {
                    for (int i = 0; i < player.TradeWindow.ItemsCount; i++)
                    {
                        ...
                        ///Fucus darf nur auf stäbe gehen
                        if (FocusSpellAllowed(item, currentItem) == false) //currentItem.Bonus1Type == 165 && item.Object_Type != 8 && item.Object_Type != 28 && item.Object_Type != 26)
                        {
                            //Dieser Wert ist nur für Stäbe geeignet
                            player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "SpellCrafting.IsAllowedToCombine.WrongBonus"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            return false;
                        }
                        ...
                        if (bonusToApply.Count >= 4 && (item.Bonus5Type == 0 || item.ExtraBonusType == 0))
                        {
                        ...
                        if (bonusToApply.Count > 5)
                        {
                            player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "SpellCrafting.IsAllowedToCombine.DifferentTypes", item.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            return false;
                        }
                        if (bonusToApply.Count == 5 && (item.Bonus5Type == 0 || item.ExtraBonusType == 0))
                        {
                            player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "SpellCrafting.IsAllowedToCombine.DifferentTypes", item.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            return false;
                        }
        ...
        public virtual void ShowSpellCraftingInfos(GamePlayer player, InventoryItem item)
        {
            ...
            if (item.Bonus1Type != 0)
            {
                spellcraftInfos.Add("\t" + SkillBase.GetPropertyName((eProperty)item.Bonus1Type, player.Client.Account.Language) + ": " + item.Bonus1 + " " + ((item.Bonus1Type >= (int)eProperty.Resist_First && item.Bonus1Type <= (int)eProperty.Resist_Last) ? "%" : " pts"));
                if (item.Bonus2Type != 0)
                {
                    spellcraftInfos.Add("\t" + SkillBase.GetPropertyName((eProperty)item.Bonus2Type, player.Client.Account.Language) + ": " + item.Bonus2 + " " + ((item.Bonus2Type >= (int)eProperty.Resist_First && item.Bonus2Type <= (int)eProperty.Resist_Last) ? "%" : "pts"));
                    if (item.Bonus3Type != 0)
                    {
                        spellcraftInfos.Add("\t" + SkillBase.GetPropertyName((eProperty)item.Bonus3Type, player.Client.Account.Language) + ": " + item.Bonus3 + " " + ((item.Bonus3Type >= (int)eProperty.Resist_First && item.Bonus3Type <= (int)eProperty.Resist_Last) ? "%" : "pts"));
                        if (item.Bonus4Type != 0)
                        {
                            spellcraftInfos.Add("\t" + SkillBase.GetPropertyName((eProperty)item.Bonus4Type, player.Client.Account.Language) + ": " + item.Bonus4 + " " + ((item.Bonus4Type >= (int)eProperty.Resist_First && item.Bonus4Type <= (int)eProperty.Resist_Last) ? "%" : "pts"));
                        }
                    }
                }
            }
            ...
            lock (player.TradeWindow.Sync)
            {
                for (int i = 0; i < player.TradeWindow.ItemsCount; i++)
                {
                    ...
                    spellcraftInfos.Add("\t" + currentGem.Name + " - " + SkillBase.GetPropertyName((eProperty)currentGem.Bonus1Type, player.Client.Account.Language) + ": (" + GetGemImbuePoints(currentGem.Bonus1Type, currentGem.Bonus1) + ") " + currentGem.Bonus1 + " " + ((currentGem.Bonus1Type >= (int)eProperty.Resist_First && currentGem.Bonus1Type <= (int)eProperty.Resist_Last) ? "%" : "pts"));
                }
            }
